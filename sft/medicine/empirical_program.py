"""Sealed Medicine laws and post-seal external authority validation."""
...
from sft.medicine.generated_law import MEDICINE_BLUEPRINTS, MedicineBlueprint
from sft.medicine.external_bindings import BINDING_BY_CLAIM
from sft.medicine.sources import SOURCE_BY_ID, source_corpus, validate_sources
...
PRE_SOURCE_SEAL_PATH = "experiments/sealed_predictions/medicine_foundation_complete_pre_source.json"


@dataclass(frozen=True)
class EmpiricalMedicineSpec:
    ...
    def validate(self) -> None:
        ...
        if binding is None:
            raise ValueError("Medicine claim lacks a post-seal source binding")
        ...
        if self.source_ids != expected or not self.source_ids:
            raise ValueError("Medicine source identities differ from the registered binding")
        if self.target_id != self.claim_id.lower() + "-authority-correspondence":
            raise ValueError("Medicine target identity is invalid")


def _spec(blueprint: MedicineBlueprint) -> EmpiricalMedicineSpec:
    binding = BINDING_BY_CLAIM[blueprint.claim_id]
    return EmpiricalMedicineSpec(
        blueprint=blueprint,
...
MEDICINE_SPECS = tuple(_spec(row) for row in MEDICINE_BLUEPRINTS)
...
def validate_pre_source_seal(root: Path) -> str:
    ...
    if sha256_identity(payload) != claimed:
        raise ValueError("Medicine pre-source seal payload changed")
    if payload.get("external_source_identities_selected") is not False or payload.get("external_outcomes_opened") is not False:
        raise ValueError("Medicine derivation was not sealed before source selection")
    if payload.get("required_claim_count") != len(MEDICINE_SPECS):
        raise ValueError("Medicine pre-source claim count changed")
    if payload.get("candidate_count") != len(MEDICINE_SPECS) * 256:
        raise ValueError("Medicine pre-source candidate count changed")
    prediction_set = tuple((row.claim_id, row.exact_result, row.expected_observation_label) for row in MEDICINE_SPECS)
    if payload.get("claim_prediction_set_hash") != sha256_identity(prediction_set):
        raise ValueError("Medicine sealed prediction set changed")
    for path_key, hash_key in (("inventory_path", "inventory_file_hash"), ("obligations_path", "obligations_hash"), ("generated_law_path", "generated_law_hash"), ("structural_counts_path", "structural_counts_hash")):
        if hash_file(root / payload[path_key]) != payload[hash_key]:
            raise ValueError(f"Medicine sealed source changed: {payload[path_key]}")
    return claimed


class GeneratedEmpiricalMedicineProgram(GeneratedEmpiricalPhysicsProgram):
    @property
...
def prediction_program_document(spec: EmpiricalMedicineSpec) -> dict[str, object]:
    return {
        "schema": "sft-v3-fold-program/1",
        "program_id": spec.experiment_id + "-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["medicine-observation", spec.expected_observation_label]},
...
def experiment_registration_record(root: Path, spec: EmpiricalMedicineSpec) -> dict[str, object]:
...
def _source_derived_target(root: Path, spec: EmpiricalMedicineSpec) -> tuple[str, str]:
    ...
    for requirement in BINDING_BY_CLAIM[spec.claim_id].requirements:
        ...
        if requirement.fragment.casefold() not in corpus:
            raise ValueError(f"required Medicine feature absent: {requirement.source_id} :: {requirement.fragment}")
...
class BlindMedicineAuthorityValidator:
    def __init__(self, root: Path, spec: EmpiricalMedicineSpec):
    ...
    def validate(self, sealed) -> EmpiricalValidation:
        ...
        if sha256_identity(audited_program) != execution.program_hash or not package_audit.passed:
            raise ValueError("Medicine prediction differs after hostile-package audit")
        release = vault.release(prediction_seal)
        CrossPlatformCustodyExchange.verify(vault.commitment, release, prediction_seal)
        boundary.measurement_context(release.targets)
        prediction = execution.output
        if not isinstance(prediction, HeldLabel) or prediction.family != "medicine-observation":
            raise ValueError("prediction emitted an invalid Medicine label")
...
for _row in MEDICINE_SPECS:
    _row.validate()


__all__ = ("BlindMedicineAuthorityValidator", "EmpiricalMedicineSpec", "GeneratedEmpiricalMedicineProgram", "MEDICINE_SPECS", "PRE_SOURCE_SEAL_PATH", "experiment_registration_record", "prediction_program_document", "validate_pre_source_seal")
