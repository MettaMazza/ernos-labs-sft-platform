"""Execution bindings for the complete FORMX-001 through FORMX-022 family."""
import json
import sys
from pathlib import Path

from sft.computation.formx_001_022_external_v1 import FormalExecutionValidator, REGISTRY, VECTOR
from sft.computation.formx_001_022_laws_v1 import FormalExtensionProgram, SPECS
...
def build_execution(root: Path, claim_id: str, execution_file: Path):
    ...
    source_files = [
        ...
        root / "sft/computation/formx_001_022_laws_v1.py",
        root / "sft/computation/formx_001_022_external_v1.py",
        root / "sft/computation/formx_001_022_execution_v1.py",
        root / REGISTRY,
        root / VECTOR,
        execution_file,
    ]
    ...
    independent = ExternalCommandValidator(
        "sft-classical-computation-formx-001-022-independent-python/1",
        (sys.executable, str(validator), claim_id, str(root)),
        validator.parent,
        (validator,),
    )
    return ClaimExecution(
        FormalExtensionProgram(spec, source_hash),
        independent,
        source_files,
        FormalExecutionValidator(root, spec),
    )
