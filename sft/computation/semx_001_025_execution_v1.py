"""Execution bindings for the complete SEMX-001 through SEMX-025 family."""
import json
import sys
from pathlib import Path

from sft.computation.semx_001_025_external_v1 import REGISTRY, VECTOR, SemanticsObservationValidator
from sft.computation.semx_001_025_laws_v1 import SPECS, SemanticsExtensionProgram
...
def build_execution(root: Path, claim_id: str, execution_file: Path):
    ...
    source_files = [
        ...
        root / "sft/computation/semx_001_025_laws_v1.py",
        root / "sft/computation/semx_001_025_external_v1.py",
        root / "sft/computation/semx_001_025_execution_v1.py",
        root / REGISTRY,
        root / VECTOR,
        execution_file,
    ]
    ...
    independent = ExternalCommandValidator(
        "sft-classical-computation-semx-001-025-independent-python/1",
        (sys.executable, str(validator), claim_id, str(root)),
        validator.parent,
        (validator,),
    )
    return ClaimExecution(SemanticsExtensionProgram(spec, source_hash), independent, source_files, SemanticsObservationValidator(root, spec))
