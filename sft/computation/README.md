# Classical computational science — archived v1 claim set; reconciliation open

Formal computation, computability, complexity, algorithms, semantics,
concurrency, distributed systems, security, learning and scientific computation.

This branch reconstructs science and mathematics of computation rather than
software-management practice.

The archived inventory contains 113 dependency-ordered claims across nine
sub-branches and 28,928 generated candidates. Every claim has exactly one
survivor, depth-independent closure, four passing adverse controls and an
implementation-distinct certificate. The authoritative inventory is
`publications/inventories/computation.json`; the exhaustive local manuscript is
`publications/current/computation/AFTER_TURING_THE_FOLD_MACHINE.md`.

The v1 paper is published. A current successor is not authorized or eligible
until every Computation-owned prior result has a same-strength V3 disposition
and the current-compliance gate passes.
