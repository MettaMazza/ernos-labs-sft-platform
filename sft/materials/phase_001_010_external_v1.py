"""Post-registry authoritative correspondence for Materials PHASE-001--010."""
...
from sft.materials.phase_001_010_laws_v1 import SPECS


REGISTRY = "census/materials_phase_001_010_target_registry_v1.json"
MANIFEST = "experiments/external_sources/materials/phase_001_010_v1/source_custody_manifest.json"
VECTOR = "experiments/external_sources/materials/phase_001_010_v1/complete_evidence_vector_v1.json"
...
def load_complete_vector(root: Path):
    ...
    if registry["target_content_present"] is not False or registry["target_count"] != len(SPECS):
        raise ValueError("PHASE target registry contains target content or changed membership")
    if manifest["target_registry_identity"] != registry_identity:
        raise ValueError("PHASE target/source custody identity changed")
    for source in manifest["documents"]:
        if hash_file(root / source["snapshot_path"]) != source["snapshot_hash"]:
            raise ValueError("PHASE source changed: " + source["source_id"])
    ...
    if vector["source_custody_manifest_identity"] != manifest_identity or vector["target_registry_identity"] != registry_identity:
        raise ValueError("PHASE complete vector custody changed")
    for row in vector["pdf_text_reconstructions"]:
        if hash_file(root / row["text_path"]) != row["text_hash"]:
            raise ValueError("PHASE text reconstruction changed: " + row["source_id"])
...
def experiment_registration(spec):
    return {
        ...
        "target_id": f"MATERIALS-PHASE-{spec.number}-COMPLETE-EXTERNAL-RECORD",
        "target_identity_registry": REGISTRY,
        "expected_observation_label": spec.exact_result,
        "falsification_condition": "Reject if any registered source identity, source byte, available fragment, condition boundary, phase distinction, prediction seal, custody identity, candidate, control or independent reconstruction is missing or changed.",
    }


def prediction_document(spec):
    return {"schema": "sft-v3-fold-program/1", "program_id": f"SFT-EXP-MAT-PHASE-{spec.number}-V1-prediction", "instructions": [{"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]}, {"opcode": "label", "destination": "prediction", "arguments": ["materials-phase", spec.exact_result]}, {"opcode": "pair", "destination": "bound", "arguments": ["premise", "prediction"]}, {"opcode": "emit", "destination": "", "arguments": ["prediction"]}]}


class PhaseExternalValidator:
    ...
    def validate(self, sealed):
        ...
        if self.spec.claim_id not in rows or vector["claim_count"] != len(SPECS):
            raise ValueError("PHASE family row missing")
        row = rows[self.spec.claim_id]
        if not row["all_comparisons_preserved"] or not row["all_registered_fragments_present"]:
            raise ValueError("PHASE external evidence row incomplete")
        sources = tuple(dict.fromkeys(item["source_id"] for item in row["comparisons"]))
        measurements = tuple(f"{item['source_id']}: {item['source_status']}; {item['first_registered_fragment']}; {item['second_registered_fragment']}; first={item['first_fragment_present']}; second={item['second_fragment_present']}" for item in row["comparisons"])
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("PHASE capability audit failed")
