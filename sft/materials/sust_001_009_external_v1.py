"""Post-registry authoritative correspondence for Materials SUST-001--009."""
...
from sft.materials.sust_001_009_laws_v1 import SPECS

REGISTRY = "census/materials_sust_001_009_target_registry_v1.json"
MANIFEST = "experiments/external_sources/materials/sust_001_009_v1/source_custody_manifest.json"
VECTOR = "experiments/external_sources/materials/sust_001_009_v1/complete_evidence_vector_v1.json"
...
def load_complete_vector(root):
    ...
    if registry["target_content_present"] is not False or registry["target_count"] != len(SPECS) or manifest["target_registry_identity"] != registry_identity:
        raise ValueError("SUST target custody changed")
    for source in manifest["documents"]:
        if hash_file(root / source["snapshot_path"]) != source["snapshot_hash"]:
            raise ValueError("SUST source changed")
    ...
    for row in vector["claims"]:
        for comparison in row["comparisons"]:
            ...
            if "text_reconstruction_path" in comparison and hash_file(root / comparison["text_reconstruction_path"]) != comparison["text_reconstruction_hash"]:
                raise ValueError("SUST text reconstruction changed")
...
def experiment_registration(spec):
    return {
        "experiment_id": f"SFT-EXP-MAT-SUST-{spec.number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"MATERIALS-SUST-{spec.number}-SUSTLETE-SUSTERNAL-RECORD",
...
def prediction_document(spec):
    return {
        ...
        "program_id": f"SFT-EXP-MAT-SUST-{spec.number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["sustainable materials and lifecycle custody", spec.exact_result]},
...
class SustExternalValidator:
    ...
    def validate(self, sealed):
        ...
        if vector["claim_count"] != len(SPECS) or not row["all_comparisons_preserved"] or not row["all_registered_fragments_present"]:
            raise ValueError("SUST evidence incomplete")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("SUST capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("materials-sust-external-v1", self.spec.claim_id)),
        ...
        return EmpiricalValidation(
            ...
            measurements=(f"SUST-{self.spec.number}: {row['comparison_count']} comparisons", f"family sources: {vector['captured_source_count']}", f"exact correspondence: {match}; omission rejected: {omission_rejected}") + measurements,
...
__all__ = ("SustExternalValidator", "REGISTRY", "MANIFEST", "VECTOR", "load_complete_vector", "experiment_registration")
