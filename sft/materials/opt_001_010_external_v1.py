"""Post-registry authoritative correspondence for Materials OPT-001--010."""
import hashlib
import json
import platform
...
from sft.materials.opt_001_010_laws_v1 import SPECS

REGISTRY = "census/materials_opt_001_010_target_registry_v1.json"
MANIFEST = "experiments/external_sources/materials/opt_001_010_v1/source_custody_manifest.json"
VECTOR = "experiments/external_sources/materials/opt_001_010_v1/complete_evidence_vector_v1.json"
ADDENDUM_REGISTRY = "census/materials_opt_003_quantum_yield_source_addendum_v1.json"
ADDENDUM_MANIFEST = "experiments/external_sources/materials/opt_001_010_v1/opt_003_quantum_yield_source_addendum_v1.json"
...
def load_complete_vector(root):
    ...
    addendum_registry, addendum_registry_identity = load_identity(root, ADDENDUM_REGISTRY, "addendum_identity")
    addendum, addendum_identity = load_identity(root, ADDENDUM_MANIFEST, "manifest_identity")
    if registry["target_content_present"] is not False or registry["target_count"] != len(SPECS) or manifest["target_registry_identity"] != registry_identity:
        raise ValueError("OPT target custody changed")
    for source in manifest["documents"]:
        if hash_file(root / source["snapshot_path"]) != source["snapshot_hash"]:
            raise ValueError("OPT source changed")
    if addendum_registry["base_target_registry_identity"] != registry_identity or addendum["source_registry_identity"] != addendum_registry_identity or addendum["base_target_registry_identity"] != registry_identity or hash_file(root / addendum["snapshot_path"]) != addendum["snapshot_hash"]:
        raise ValueError("OPT source addendum changed")
    vector, vector_identity = load_identity(root, VECTOR, "complete_vector_identity")
    if vector["source_custody_manifest_identity"] != manifest_identity or vector["target_registry_identity"] != registry_identity or vector["source_addendum_registry_identity"] != addendum_registry_identity or vector["source_addendum_manifest_identity"] != addendum_identity:
        raise ValueError("OPT evidence custody changed")
    for reconstruction in vector["pdf_text_reconstructions"]:
        if hash_file(root / reconstruction["text_path"]) != reconstruction["text_hash"]:
            raise ValueError("OPT text reconstruction changed")
    vector.update(complete_vector_identity=vector_identity, source_custody_manifest_identity=manifest_identity, target_registry_identity=registry_identity, source_addendum_registry_identity=addendum_registry_identity, source_addendum_manifest_identity=addendum_identity)
    return vector

def experiment_registration(spec):
    return {
        "experiment_id": f"SFT-EXP-MAT-OPT-{spec.number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"MATERIALS-OPT-{spec.number}-COMPLETE-EXTERNAL-RECORD",
...
def prediction_document(spec):
    return {"schema": "sft-v3-fold-program/1", "program_id": f"SFT-EXP-MAT-OPT-{spec.number}-V1-prediction", "instructions": [
        {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
        {"opcode": "label", "destination": "prediction", "arguments": ["materials-optical and photonic", spec.exact_result]},
        {"opcode": "pair", "destination": "bound", "arguments": ["premise", "prediction"]},
        {"opcode": "emit", "destination": "", "arguments": ["prediction"]},
    ]}

class OptExternalValidator:
    ...
    def validate(self, sealed):
        ...
        if vector["claim_count"] != len(SPECS) or not row["all_comparisons_preserved"] or not row["all_registered_fragments_present"]:
            raise ValueError("OPT evidence incomplete")
        sources = tuple(dict.fromkeys(comparison["source_id"] for comparison in row["comparisons"]))
        measurements = tuple(f"{comparison['source_id']}: {comparison['first_registered_fragment']}; {comparison['second_registered_fragment']}" for comparison in row["comparisons"])
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("OPT capability audit failed")
        ...
        return EmpiricalValidation(validated_seal_hash=sealed.seal_hash, experiment_registration_hash=registration_hash, isolation_certificate=isolation, target_custody_certificate=custody, evaluator_verified_seal=True, target_opened_after_seal=True, all_rows_preserved=True, data_source_ids=sources, measurements=(f"OPT-{self.spec.number}: {row['comparison_count']} comparisons", f"family sources: {vector['captured_source_count']}", f"exact correspondence: {match}; omission rejected: {omission_rejected}") + measurements, measurement_receipt_hash=sha256_identity(payload), falsification_condition=registration["falsification_condition"], passed=bool(match and omission_rejected and row["all_registered_fragments_present"]))

__all__ = ("OptExternalValidator", "REGISTRY", "MANIFEST", "VECTOR", "ADDENDUM_REGISTRY", "ADDENDUM_MANIFEST", "load_complete_vector", "experiment_registration")
