"""Post-registry authoritative correspondence for Materials CLASS-001--012."""
import hashlib
import json
import platform
...
from sft.materials.class_001_012_laws_v1 import SPECS

REGISTRY = "census/materials_class_001_012_target_registry_v1.json"
MANIFEST = "experiments/external_sources/materials/class_001_012_v1/source_custody_manifest.json"
VECTOR = "experiments/external_sources/materials/class_001_012_v1/complete_evidence_vector_v1.json"
...
def load_complete_vector(root):
    ...
    if registry["target_content_present"] is not False or registry["target_count"] != len(SPECS) or manifest["target_registry_identity"] != registry_identity:
        raise ValueError("CLASS target custody changed")
    ...
    if vector["source_custody_manifest_identity"] != manifest_identity or vector["target_registry_identity"] != registry_identity:
        raise ValueError("CLASS evidence custody changed")
...
def experiment_registration(spec):
    return {"experiment_id": f"SFT-EXP-MAT-CLASS-{spec.number}-V1", "claim_id": spec.claim_id, "target_id": f"MATERIALS-CLASS-{spec.number}-COMPLETE-EXTERNAL-RECORD", "target_identity_registry": REGISTRY, "expected_observation_label": spec.exact_result, "falsification_condition": "Reject if any registered source, fragment, condition, path, seal, custody identity, candidate, control or independent reconstruction is missing or changed."}

def prediction_document(spec):
    return {"schema": "sft-v3-fold-program/1", "program_id": f"SFT-EXP-MAT-CLASS-{spec.number}-V1-prediction", "instructions": [{"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]}, {"opcode": "label", "destination": "prediction", "arguments": ["materials classes", spec.exact_result]}, {"opcode": "pair", "destination": "bound", "arguments": ["premise", "prediction"]}, {"opcode": "emit", "destination": "", "arguments": ["prediction"]}]}

class ClassExternalValidator:
    ...
    def validate(self, sealed):
        ...
        if vector["claim_count"] != len(SPECS) or not row["all_comparisons_preserved"] or not row["all_registered_fragments_present"]:
            raise ValueError("CLASS evidence incomplete")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("CLASS capability audit failed")
