"""Post-registry authoritative correspondence for Materials MAGSC-001--007."""
import hashlib
import json
import platform
...
from sft.materials.magsc_001_012_laws_v1 import SPECS

REGISTRY = "census/materials_magsc_001_012_target_registry_v1.json"
MANIFEST = "experiments/external_sources/materials/magsc_001_012_v1/source_custody_manifest.json"
VECTOR = "experiments/external_sources/materials/magsc_001_012_v1/complete_evidence_vector_v1.json"
...
def load_complete_vector(root):
    ...
    if registry["target_content_present"] is not False or registry["target_count"] != len(SPECS) or manifest["target_registry_identity"] != registry_identity:
        raise ValueError("MAGSC target custody changed")
    for source in manifest["documents"]:
        if hash_file(root / source["snapshot_path"]) != source["snapshot_hash"]:
            raise ValueError("MAGSC source changed")
...
def experiment_registration(spec):
    return {
        ...
        "target_id": f"MATERIALS-MAGSC-{spec.number}-COMPLETE-EXTERNAL-RECORD",
...
def prediction_document(spec):
    return {"schema": "sft-v3-fold-program/1", "program_id": f"SFT-EXP-MAT-MAGSC-{spec.number}-V1-prediction", "instructions": [
        {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
        {"opcode": "label", "destination": "prediction", "arguments": ["materials-magnetic and superconducting", spec.exact_result]},
        {"opcode": "pair", "destination": "bound", "arguments": ["premise", "prediction"]},
        {"opcode": "emit", "destination": "", "arguments": ["prediction"]},
    ]}

class MagscExternalValidator:
    ...
    def validate(self, sealed):
        ...
        if vector["claim_count"] != len(SPECS) or not row["all_comparisons_preserved"] or not row["all_registered_fragments_present"]:
            raise ValueError("MAGSC evidence incomplete")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("MAGSC capability audit failed")
