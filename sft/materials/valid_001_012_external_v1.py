...
from sft.materials.valid_001_012_laws_v1 import REGISTRY,SPECS
VECTOR="experiments/external_sources/materials/valid_001_012_complete_empirical_vector_v1.json"
...
def load_vector(root):
 path=root/VECTOR
 if hash_file(path)!=REGISTRY["evidence_vector_file_hash"]:raise ValueError("Materials VALID vector file changed")
 d=json.loads(path.read_text());body=dict(d);identity=body.pop("complete_vector_identity")
 if canonical(body)!=identity or identity!=REGISTRY["evidence_vector_identity"]:raise ValueError("Materials VALID vector identity changed")
 return d
def check(root,spec):
...
def registration(spec):return {"experiment_id":f"SFT-EXP-MAT-VALID-{spec.number}-V1","claim_id":spec.claim_id,"target_id":f"MATERIALS-VALID-{spec.number}-COMPLETE-VECTOR-V1","identity_registry":"census/materials_valid_001_012_dependency_registry_v1.json","expected_label":f"complete-materials-valid-vector-{spec.number}-retained","falsification_condition":"Reject if any registered member, receipt, control, evidence file, result class, closure scope, formal-only boundary or source-custody row is missing, changed, duplicated or selected after outcome access."}
def document(spec):return {"schema":"sft-v3-fold-program/1","program_id":f"SFT-EXP-MAT-VALID-{spec.number}-V1-prediction","instructions":[{"opcode":"input","destination":"premise","arguments":["registered-premise"]},{"opcode":"label","destination":"prediction","arguments":["materials-validation-vector",f"complete-materials-valid-vector-{spec.number}-retained"]},{"opcode":"pair","destination":"bound","arguments":["premise","prediction"]},{"opcode":"emit","destination":"","arguments":["prediction"]}]}
class MaterialsValidationVectorValidator:
