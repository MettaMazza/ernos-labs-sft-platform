"""Shared generated-grammar kernel for the twelve information-science laws.
...
class LawSpec:
    ...
    def validate(self) -> None:
        if not self.claim_id.startswith("SFT-INFO-"):
            raise ValueError("information-science claim identity is invalid")
        if not self.dependencies or not self.dimensions or not self.witnesses:
            raise ValueError("information-science law lacks dependencies, dimensions or witnesses")
        keys = tuple(dimension.key for dimension in self.dimensions)
        if len(set(keys)) != len(keys):
            raise ValueError("information-science law contains duplicate dimensions")
        ...
        if not all(witness.passed for witness in self.witnesses):
            ...
            raise ValueError("information-science operational witness failed: " + ", ".join(failed))
...
class GeneratedInformationProgram:
