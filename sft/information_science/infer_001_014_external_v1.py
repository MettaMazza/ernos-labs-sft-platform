"""Post-registry exact observation validator for INFER-001--014."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_infer_001_014_target_registry_v1.json"
REGISTRY_HASH = "sha256:281ebee51324fdcc5654060183daf15755ff1fec04395e071f675f27a6a2053d"
VECTOR = "experiments/external_sources/information_science/infer_001_014_observation_vector_v1.json"
VECTOR_HASH = "sha256:f19e3d07f621ea68e19634480176ccb8e012faccc223756ef0b44a02a933e440"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("INFER frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("INFER frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-INFER-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-INFER-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-infer-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete inference census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-INFER-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["infer-observation", f"complete-infer-{number}-observation-retained"]},
...
class InferObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("INFER observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("INFER capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-infer-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"INFER-{number}: {row['observation_name']}",
            ...
            "complete finite hypothesis, evidence, support, decision, error-class, stopping and sensor-record enumeration supplies the observation; no imported prior, fitted likelihood, chosen threshold or hidden hypothesis enters",
        )
        payload = {
