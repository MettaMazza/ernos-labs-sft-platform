"""Post-registry exact observation validator for COMP-001--014."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_comp_001_014_target_registry_v1.json"
REGISTRY_HASH = "sha256:85d2bf0f0314b48cc633a8587384d7aac21f8f4ced53f7bd2ff6ff29e7826b12"
VECTOR = "experiments/external_sources/information_science/comp_001_014_observation_vector_v1.json"
VECTOR_HASH = "sha256:c23439eafc37d01a048feec609b5e27d0a146d535f07a179b53ffad244f9485d"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("COMP frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("COMP frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-COMP-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-COMP-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-comp-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete comp census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-COMP-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["comp-observation", f"complete-comp-{number}-observation-retained"]},
...
class CompressionObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("COMP observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("COMP capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-comp-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"COMP-{number}: {row['observation_name']}",
            ...
            "complete finite source, codeword, dictionary, recurrence, reconstruction, distortion, layer and side-information enumeration supplies the observation; no imported entropy optimum, fitted dictionary or erased predecessor enters",
        )
        payload = {
