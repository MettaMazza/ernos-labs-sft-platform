"""Post-registry exact observation validator for CODE-001--018."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_code_001_018_target_registry_v1.json"
REGISTRY_HASH = "sha256:f654d667875def75082b077105b19f9d00e2643c1c4257f0e20b0d66e3b8ef46"
VECTOR = "experiments/external_sources/information_science/code_001_018_observation_vector_v1.json"
VECTOR_HASH = "sha256:3aa064418f0ef3fcb39b706c074e6d3183eed0271691b50a21389531f36616a2"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("CODE frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("CODE frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-CODE-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-CODE-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-code-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete code census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-CODE-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["code-observation", f"complete-code-{number}-observation-retained"]},
...
class CodeObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("CODE observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("CODE capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-code-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"CODE-{number}: {row['observation_name']}",
            ...
            "complete finite source, codeword, error-mask, check, predecessor, decoder, path and rate-part enumeration supplies the observation; no imported stochastic error law, likelihood decoder, fitted threshold or hidden codeword enters",
        )
        payload = {
