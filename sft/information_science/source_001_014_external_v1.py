"""Post-registry exact observation validator for SOURCE-001--014."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_source_001_014_target_registry_v1.json"
REGISTRY_HASH = "sha256:f9729b1c672c5ff8ad3549ed4e57b5cfbb7e13aeba15bbd04c2a09f602c0281d"
VECTOR = "experiments/external_sources/information_science/source_001_014_observation_vector_v1.json"
VECTOR_HASH = "sha256:2e7ef44021acd83f728a3460a23c5614ad83ec9ece0153acdf7534d8b73e6aa6"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("SOURCE frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("SOURCE frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-SOURCE-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-SOURCE-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-source-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete source census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-SOURCE-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["source-observation", f"complete-source-{number}-observation-retained"]},
...
class SourceObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("SOURCE observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("SOURCE capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-source-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"SOURCE-{number}: {row['observation_name']}",
            ...
            "complete finite support, sequence, transition, spatial, network and joint-source enumeration supplies the observation; no imported stochastic cause, hidden path or omitted possibility enters",
        )
        payload = {
