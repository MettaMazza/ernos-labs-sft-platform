"""Post-registry exact observation validator for SYMREP-001--014."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_symrep_001_014_target_registry_v1.json"
REGISTRY_HASH = "sha256:2d6d97c01f6d8c6489f26964d3cd14f5371842711856d8365329cc626ade124a"
VECTOR = "experiments/external_sources/information_science/symrep_001_014_observation_vector_v1.json"
VECTOR_HASH = "sha256:2bc1d1c7621cc2a20ccf4461f3eef82ef446130a596455347057ebf638598e06"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("SYMREP frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("SYMREP frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-SYMREP-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-SYMREP-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-symrep-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete representation census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-SYMREP-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["symrep-observation", f"complete-symrep-{number}-observation-retained"]},
...
class RepresentationObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("SYMREP observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("SYMREP capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-symrep-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"SYMREP-{number}: {row['observation_name']}",
            ...
            "complete finite alphabet, code, parse, grammar and relation enumeration supplies the observation; no imported coding axiom, semantic prior or hidden parse enters",
        )
        payload = {
