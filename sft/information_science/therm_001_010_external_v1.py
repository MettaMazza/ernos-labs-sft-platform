"""Post-registry exact observation validator for THERM-001--010."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_therm_001_010_target_registry_v1.json"
REGISTRY_HASH = "sha256:fbb787db24b7bc68c67dc56a1802c2d6165c158132f67d671b2add733f4b82c0"
VECTOR = "experiments/external_sources/information_science/therm_001_010_observation_vector_v1.json"
VECTOR_HASH = "sha256:9f51bc1b74b6bd4fec6be384cf47cceba67fb29130c6252b77c464875b18e18a"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("THERM frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("THERM frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-THERM-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-THERM-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-therm-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete record/transition/predecessor custody census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-THERM-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["therm-observation", f"complete-therm-{number}-observation-retained"]},
...
class ThermObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("THERM observation membership changed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-therm-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"THERM-{number}: {row['observation_name']}",
            ...
            "complete logical-record, merge, predecessor, held-label, measurement, reset and cycle enumeration supplies the observation; no imported physical constant, fitted energy value, erased provenance or information-energy identity enters",
        )
        payload = {
