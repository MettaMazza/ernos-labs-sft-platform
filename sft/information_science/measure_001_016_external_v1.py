"""Post-registry exact observation validator for MEASURE-001--016."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_measure_001_016_target_registry_v1.json"
REGISTRY_HASH = "sha256:1b863672f75904f34a57bb92f70275e9f99b9ae438f45558d0ec1db8109c367d"
VECTOR = "experiments/external_sources/information_science/measure_001_016_observation_vector_v1.json"
VECTOR_HASH = "sha256:aee669e0a6493eb7c206a497162501b09f2a2279c9b15d7546a9cd839aa11865"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("MEASURE frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("MEASURE frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-MEASURE-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-MEASURE-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-measure-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete measure census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-MEASURE-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["measure-observation", f"complete-measure-{number}-observation-retained"]},
...
class MeasureObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("MEASURE observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("MEASURE capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-measure-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"MEASURE-{number}: {row['observation_name']}",
            ...
            "complete finite support, partition, description-program, unit and retained/closed-ledger enumeration supplies the observation; no imported entropy formula, continuum logarithm, universal oracle or fitted threshold enters",
        )
        payload = {
