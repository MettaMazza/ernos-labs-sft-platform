"""Post-registry exact observation validator for RETR-001--012."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_retr_001_012_target_registry_v1.json"
REGISTRY_HASH = "sha256:4759221d8edb5df800552e142b2db354654ee572a086d72fd412df84eb1b0781"
VECTOR = "experiments/external_sources/information_science/retr_001_012_observation_vector_v1.json"
VECTOR_HASH = "sha256:0eff9f0d9b8f8efc9d0eb9b24c8d91bb567660b2d29b71eae6550647353a88cb"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("RETR frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("RETR frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-RETR-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-RETR-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-retr-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete source-index-query-result census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-RETR-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["retr-observation", f"complete-retr-{number}-observation-retained"]},
...
class RetrObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("RETR observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("RETR capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-retr-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"RETR-{number}: {row['observation_name']}",
            ...
            "complete source, key, location, query, result, order-class, relevance and reference-edge enumeration supplies the observation; no imported relevance judgment, fitted rank score, silent tie break or stale source version enters",
        )
        payload = {
