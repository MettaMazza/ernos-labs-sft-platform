"""Post-registry exact observation validator for SEM-001--012."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_sem_001_012_target_registry_v1.json"
REGISTRY_HASH = "sha256:5869cc974c211c3ebb71e40920421385d8cc2182ace5c7248423dae38a919821"
VECTOR = "experiments/external_sources/information_science/sem_001_012_observation_vector_v1.json"
VECTOR_HASH = "sha256:bf39fa18ae005823ab00d41228967b2ed7d7ce96959a38faa53bdeb2dc0405a2"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("SEM frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("SEM frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-SEM-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-SEM-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-sem-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete symbol-reference-context-handoff census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-SEM-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["sem-observation", f"complete-sem-{number}-observation-retained"]},
...
class SemObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("SEM observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("SEM capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-sem-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"SEM-{number}: {row['observation_name']}",
            ...
            "complete symbol, reference, context, interpretation, translation, ownership, handoff and provenance enumeration supplies the observation; no imported meaning, fitted referent, inferred quale, silent cross-domain substitution or target-selected result enters",
        )
        payload = {
