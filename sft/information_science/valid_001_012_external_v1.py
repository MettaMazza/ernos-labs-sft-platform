...
REGISTRY = "census/information_science_valid_001_012_target_registry_v1.json"
REGISTRY_HASH = "sha256:f478504bd9cf0ccb1b6e210a1bfb5c79b6b0a1f63f92abe05c523d0386b7dd1d"
VECTOR = "experiments/external_sources/information_science/valid_001_012_observation_vector_v1.json"
VECTOR_HASH = "sha256:8c68a43f4a9997b521307efaf8b8861c6630996187863c22e7531e4f14b15067"
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-VALID-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-VALID-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-valid-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, frozen 244-receipt reconciliation, complete controls and disposition vector, exact observation, candidate survivor, source identity, seal, custody record or independent reconstruction is missing, stale, changed, duplicated, selectively omitted or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-VALID-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["valid-observation", f"complete-valid-{number}-observation-retained"]},
...
class ValidObservationValidator:
    ...
    def validate(self, sealed):
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-valid-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            ...
            "the frozen 244-receipt reconciliation, 976 controls, current certificates, complete disposition custody and implementation-distinct reconstruction supply the observation; no favorable-only selection, fitted tolerance, stale receipt, erased halt or edited authority enters",
        )
        payload = {
