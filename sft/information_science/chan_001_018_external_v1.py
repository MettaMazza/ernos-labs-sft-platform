"""Post-registry exact observation validator for CHAN-001--018."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_chan_001_018_target_registry_v1.json"
REGISTRY_HASH = "sha256:6364aa1816371241f64435e8120193f629463f792eb719d54a47796c71bc32ab"
VECTOR = "experiments/external_sources/information_science/chan_001_018_observation_vector_v1.json"
VECTOR_HASH = "sha256:b80e18e8bd39de35cd4f40b5cb4495b6b288a899c79d6299b6cd7db90fad5897"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("CHAN frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("CHAN frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-CHAN-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-CHAN-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-chan-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete channel census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-CHAN-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["chan-observation", f"complete-chan-{number}-observation-retained"]},
...
class ChannelObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("CHAN observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("CHAN capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-chan-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"CHAN-{number}: {row['observation_name']}",
            ...
            "complete finite input, output, relation, observation, codebook, use-history, route, cut and simulation enumeration supplies the observation; no imported stochastic channel, logarithmic capacity formula or fitted distribution enters",
        )
        payload = {
