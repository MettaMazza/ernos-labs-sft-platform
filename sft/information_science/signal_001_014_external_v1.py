"""Post-registry exact observation validator for SIGNAL-001--014."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_signal_001_014_target_registry_v1.json"
REGISTRY_HASH = "sha256:74aa9296554a83ca09c542543056af5183e0c519c12e088cdb34ff1c95a690e6"
VECTOR = "experiments/external_sources/information_science/signal_001_014_observation_vector_v1.json"
VECTOR_HASH = "sha256:d09003b8d6dfa052f7e04c6ab61dbf07f0e5b1148e2b4d0a38ee15735d89eed2"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("SIGNAL frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("SIGNAL frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-SIGNAL-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-SIGNAL-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-signal-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete signal census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-SIGNAL-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["signal-observation", f"complete-signal-{number}-observation-retained"]},
...
class SignalObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("SIGNAL observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("SIGNAL capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-signal-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"SIGNAL-{number}: {row['observation_name']}",
            ...
            "complete finite position, amplitude, sample, alias, quantization, reconstruction, transform and provenance enumeration supplies the observation; no imported continuum theorem, hidden predecessor or fitted threshold enters",
        )
        payload = {
