"""Post-registry exact observation validator for REL-001--014."""
import hashlib
import json
import platform
...
REGISTRY = "census/information_science_rel_001_014_target_registry_v1.json"
REGISTRY_HASH = "sha256:1c1ff64108cab8dd0cbf84dcbd0ba259ef485299b4ec6b5c9fd9f3b5c95896ab"
VECTOR = "experiments/external_sources/information_science/rel_001_014_observation_vector_v1.json"
VECTOR_HASH = "sha256:d68f3366af09606db631bef38a27bcbefe0806a31e47e1d56b1aed9ab4c5acdb"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("REL frozen observation files changed")
    ...
    if (
        ...
        raise ValueError("REL frozen identities changed")
...
def registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-INFO-REL-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"INFO-REL-{number}-EXACT-OBSERVATION-V1",
        "expected_label": f"complete-rel-{number}-observation-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete relational census, exact observation, candidate survivor, source identity, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-INFO-REL-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["rel-observation", f"complete-rel-{number}-observation-retained"]},
...
class RelObservationValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("REL observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("REL capability audit failed")
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(
            ...
            comparison_implementation_identity_hash=sha256_identity(("information-science-rel-observer-v1", self.spec.claim_id)),
        ...
        measurements = (
            f"REL-{number}: {row['observation_name']}",
            ...
            "complete finite joint, conditional, shared, directed, causal-path and representation enumeration supplies the observation; no imported distribution, signed-information premise, fitted allocation or hidden joint row enters",
        )
        payload = {
