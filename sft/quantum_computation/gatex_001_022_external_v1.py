"""Post-registry exact execution validator for GATEX-001 through GATEX-022."""
import hashlib
import json
import platform
...
REGISTRY = "census/quantum_gatex_001_022_target_registry_v1.json"
REGISTRY_HASH = "sha256:ae5d95fa839ebef95ed02d960ba27538f00cc92a5320433cd1080a1392482c49"
VECTOR = "experiments/external_sources/quantum_computation/gatex_001_022_observation_vector_v1.json"
VECTOR_HASH = "sha256:90d8357470c20023edd488af5d2833b14dbc833c23039b0fc08f23d8c7d353ad"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("GATEX frozen observation files changed")
    ...
    if canonical(registry_body) != registry_identity or canonical(vector_body) != vector_identity or vector["registry_identity"] != registry_identity or registry["target_content_present"] is not False:
        raise ValueError("GATEX frozen identities changed")
    return registry, vector


def experiment_registration(spec):
    number = spec.claim_id.rsplit("-", 1)[-1]
    return {"experiment_id": f"SFT-EXP-QUANTUM-GATEX-{number}-V1", "claim_id": spec.claim_id, "target_id": f"QUANTUM-GATEX-{number}-EXACT-EXECUTION-V1", "expected_label": f"complete-gatex-{number}-execution-retained", "registry": REGISTRY, "falsification_condition": "Reject if the value-free registry, complete gate/circuit grammar, exact branchwise execution, source identity, candidate survivor, seal, custody record, control or independent reconstruction is missing, changed, duplicated or opened out of order."}


def program_document(spec):
    number = spec.claim_id.rsplit("-", 1)[-1]
    return {"schema": "sft-v3-fold-program/1", "program_id": f"SFT-EXP-QUANTUM-GATEX-{number}-V1-prediction", "instructions": [
        {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
        {"opcode": "label", "destination": "prediction", "arguments": ["gatex-execution", f"complete-gatex-{number}-execution-retained"]},
        {"opcode": "pair", "destination": "bound", "arguments": ["premise", "prediction"]},
        {"opcode": "emit", "destination": "", "arguments": ["prediction"]},
    ]}


class GateCircuitExecutionValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("GATEX observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("GATEX capability audit failed")
        ...
        measurements = (f"GATEX-{number}: {row['observation_name']}", "exact gate/circuit execution: " + json.dumps(row["exact_observation"], sort_keys=True, separators=(",", ":")), f"complete gate/circuit custody: {vector['record_count']} records; all rows preserved", "complete finite support, gate, inverse, circuit, observation and resource execution supplies the observation; physical timing, gaps, error rates and device outcomes remain downstream measurements")
