"""Post-registry exact execution validator for QLEARNX-001 through QLEARNX-022."""
...
REGISTRY = "census/quantum_qlearnx_001_022_target_registry_v1.json"
REGISTRY_HASH = "sha256:f2155bfbb0eba50cabd48d9da6d4deb4ed8b2d1bab50479e04da7113d5e61cd9"
VECTOR = "experiments/external_sources/quantum_computation/qlearnx_001_022_observation_vector_v1.json"
VECTOR_HASH = "sha256:a348a2e81b0157e88553f79b176671813b2dcae8c1038f6e46a991d6bf380840"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("QLEARNX frozen observation files changed")
    ...
    if canonical(registry_body) != registry_identity or canonical(vector_body) != vector_identity or vector["registry_identity"] != registry_identity or registry["target_content_present"] is not False:
        raise ValueError("QLEARNX frozen identities changed")
...
def experiment_registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-QUANTUM-QLEARNX-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"QUANTUM-QLEARNX-{number}-EXACT-EXECUTION-V1",
        "expected_label": f"complete-qlearnx-{number}-execution-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete learning grammar, held-out custody, exact execution or explicit handoff, source identity, candidate survivor, seal, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def program_document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-QUANTUM-QLEARNX-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["qlearnx-execution", f"complete-qlearnx-{number}-execution-retained"]},
...
class QuantumLearningExecutionValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("QLEARNX observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("QLEARNX capability audit failed")
        ...
        measurements = (
            f"QLEARNX-{number}: {row['observation_name']}",
            "exact quantum-learning execution: " + json.dumps(row["exact_observation"], sort_keys=True, separators=(",", ":")),
            f"complete learning and held-out custody: {vector['record_count']} records; all rows preserved",
            "exact finite learning rows supply formal observations; physical advantage, device behavior and application targets remain explicit owning-domain measurement handoffs",
        )
