"""Post-registry exact execution validator for QLIMITX-001 through QLIMITX-022."""
...
REGISTRY = "census/quantum_qlimitx_001_022_target_registry_v1.json"
REGISTRY_HASH = "sha256:07a46b8dbc3e86dbd7654d1baad32e1f2da7e0d9c165bd3903047c18781242be"
VECTOR = "experiments/external_sources/quantum_computation/qlimitx_001_022_observation_vector_v1.json"
VECTOR_HASH = "sha256:332e35bff0cb66601fcd8d2319bd10afc0db3c29279b840a91f458c5e1917339"
...
def load(root):
    ...
    if hash_file(registry_path) != REGISTRY_HASH or hash_file(vector_path) != VECTOR_HASH:
        raise ValueError("QLIMITX frozen observation files changed")
    ...
    if canonical(registry_body) != registry_identity or canonical(vector_body) != vector_identity or vector["registry_identity"] != registry_identity or registry["target_content_present"] is not False:
        raise ValueError("QLIMITX frozen identities changed")
...
def experiment_registration(spec):
    ...
    return {
        "experiment_id": f"SFT-EXP-QUANTUM-QLIMITX-{number}-V1",
        "claim_id": spec.claim_id,
        "target_id": f"QUANTUM-QLIMITX-{number}-EXACT-EXECUTION-V1",
        "expected_label": f"complete-qlimitx-{number}-execution-retained",
        "registry": REGISTRY,
        "falsification_condition": "Reject if the value-free registry, complete correspondence/limit grammar, exact witness or explicit physical handoff, source identity, candidate survivor, seal, control or independent reconstruction is missing, changed, duplicated or opened out of order.",
    }
...
def program_document(spec):
    ...
    return {
        ...
        "program_id": f"SFT-EXP-QUANTUM-QLIMITX-{number}-V1-prediction",
        "instructions": [
            {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
            {"opcode": "label", "destination": "prediction", "arguments": ["qlimitx-execution", f"complete-qlimitx-{number}-execution-retained"]},
...
class QuantumLimitsExecutionValidator:
    ...
    def validate(self, sealed):
        ...
        if row["claim_id"] != self.spec.claim_id or not row["all_rows_preserved"]:
            raise ValueError("QLIMITX observation membership changed")
        ...
        if sha256_identity(audited) != execution.program_hash or not audit.passed:
            raise ValueError("QLIMITX capability audit failed")
        ...
        measurements = (
            f"QLIMITX-{number}: {row['observation_name']}",
            "exact classical-quantum correspondence/limit execution: " + json.dumps(row["exact_observation"], sort_keys=True, separators=(",", ":")),
            f"complete limit and handoff custody: {vector['record_count']} records; all rows preserved",
            "exact finite limit witnesses supply formal observations; physical speedup, energy, timing, fidelity and thresholds remain explicit owning-domain measurements",
        )
