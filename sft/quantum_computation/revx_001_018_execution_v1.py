"""Execution bindings for the complete REVX-001 through REVX-018 family."""
import json
import sys
...
from sft.quantum_computation.revx_001_018_external_v1 import ReversibleExecutionValidator, REGISTRY, VECTOR
from sft.quantum_computation.revx_001_018_laws_v1 import ReversibleExtensionProgram, SPECS
...
def build_execution(root: Path, claim_id: str, execution_file: Path):
    ...
    source_files = [
        ...
        root / "sft/quantum_computation/revx_001_018_laws_v1.py",
        root / "sft/quantum_computation/revx_001_018_external_v1.py",
        root / "sft/quantum_computation/revx_001_018_execution_v1.py",
        root / REGISTRY,
        root / VECTOR,
        execution_file,
    ]
    ...
    independent = ExternalCommandValidator(
        "sft-quantum-computation-revx-001-018-independent-python/1",
        (sys.executable, str(validator), claim_id, str(root)),
        validator.parent,
        (validator,),
    )
    return ClaimExecution(
        ReversibleExtensionProgram(spec, source_hash),
        independent,
        source_files,
        ReversibleExecutionValidator(root, spec),
    )
