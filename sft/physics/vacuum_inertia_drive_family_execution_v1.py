"""Execution builder for the formal vacuum/inertia drive family."""
...
def build_execution(root: Path, claim_id: str, execution_file: Path) -> ClaimExecution:
    spec = SPECS[claim_id]
    fixed = (
        root / "sft/physics/vacuum_inertia_drive_family_law_v1.py",
        root / "sft/physics/vacuum_inertia_drive_family_execution_v1.py",
    ...
    validator = root / "generated/physics/vacuum_inertia_drive_family_validator_v1.py"
    return ClaimExecution(
        StructuralPhysicsProgram(spec, source_hash),
        ExternalCommandValidator(
            "sft-physics-vacuum-inertia-drive-family-independent-python/1",
