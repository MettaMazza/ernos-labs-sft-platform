...
from sft.chemistry.hand_001_006_laws_v2 import REGISTRY, REGISTRY_FILE_HASH

VECTOR = "experiments/external_sources/chemistry/hand_001_006_complete_handoff_vector_v2.json"
VECTOR_FILE_HASH = "sha256:99ad257a1b887c1423f80683275a6adb770aaa900026fd0482ff4b3ad4207d77"
...
def load_surface(root):
    ...
    if hash_file(registry_path) != REGISTRY_FILE_HASH or hash_file(vector_path) != VECTOR_FILE_HASH:
        raise ValueError("Chemistry HAND frozen files changed")
    ...
    if canonical(rb) != ri or canonical(vb) != vi or vector["registry_identity"] != ri:
        raise ValueError("Chemistry HAND frozen identities changed")
...
def check(root, spec):
    ...
    if spec.number == "006":
        ...
        if tuple(row["claim_id"] for row in rows) != spec.paired_claim_ids:
            raise ValueError("Chemistry HAND pair changed")
        if tuple(row["owner"] for row in rows) != tuple(registry["paired_owner_vectors"][spec.number]):
            raise ValueError("Chemistry HAND owner vector changed")
        for row in rows:
...
def registration(spec):
    return {
        "experiment_id": f"SFT-EXP-CHEM-HAND-{spec.number}-V2",
        "claim_id": spec.claim_id,
        "target_id": f"CHEMISTRY-HAND-{spec.number}-EXACT-OWNER-BOUNDARY-V2",
        "identity_registry": "census/chemistry_hand_001_006_dependency_registry_v2.json",
        "expected_label": f"complete-chemistry-hand-{spec.number}-retained",
...
def document(spec):
    return {"schema": "sft-v3-fold-program/1", "program_id": f"SFT-EXP-CHEM-HAND-{spec.number}-V2-prediction", "instructions": [
        {"opcode": "input", "destination": "premise", "arguments": ["registered-premise"]},
        {"opcode": "label", "destination": "prediction", "arguments": ["chemistry-handoff", f"complete-chemistry-hand-{spec.number}-retained"]},
        {"opcode": "pair", "destination": "bound", "arguments": ["premise", "prediction"]},
        {"opcode": "emit", "destination": "", "arguments": ["prediction"]},
    ]}


class ChemistryHandoffValidator:
    ...
    def validate(self, sealed):
        ...
        isolation = seal_isolation_certificate(unsealed_isolation_certificate(executor_id=reg["experiment_id"] + "-executor", host_platform=platform.system() or "host", python_implementation=platform.python_implementation(), interpreter_hash=sha256_identity(CapabilityClosedFoldInterpreter.interpreter_id), program_hash=execution.program_hash, input_manifest_hash=execution.input_manifest_hash, registered_target_identity_hash=tid, comparison_implementation_identity_hash=sha256_identity(("chemistry-hand-v2", self.spec.claim_id)), prediction_seal_hash=prediction.seal_hash, output_hash=execution.output_hash, trace_hash=execution.trace_hash))
