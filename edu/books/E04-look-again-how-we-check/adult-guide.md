...
E04 is a read-aloud story and activity book for Early Years / Foundation. It
continues directly from E03 at the garden gate. The child helps Mia, Sol, Tavi
...
Book Four and Level Four must make sense to a child who has just finished Level
...
- Claim title: **Complete replayable derivation trace**
- Receipt:
  `sha256:457414dd7e3a14e44b200b1ac329cdc9e2bb3527abe901c3255d7fb6f9233c5e`
- Status: model admitted, depth-independent, independently replicated
- Exact role in E04: an admitted finite derivation has one complete ordered
  source- and dependency-bound record that deterministically replays every
  registered operation, intermediate and terminal result.

The four picture-placement cards translate the experience of keeping steps in
order. They are not derivation identities, and replaying a classroom sign does
not prove the formal claim.

### `SFT-FOUNDATION-MEASURED-VALUE-BOUNDARY-001`

- Claim title: **One-way derivation-to-measurement boundary**
- Receipt:
  `sha256:a4e409f6bb8b203de746bd0c21d3f2566e8052003219490c657249e11f6f2086`
- Status: model admitted, depth-independent, independently replicated
- Exact role in E04: the exact source-bound law and target question are fixed
  before measurement is opened; measurement preserves every row and cannot
  select or mutate the law.

The WIDTH and HEIGHT ribbons translate the one-question boundary. A width
result answers width only. It cannot establish picture order, height, beauty,
truth or SFT authority. A measuring tool answers only the question we chose and
declared.

### `SFT-FOUNDATION-ADMISSION-ENFORCEMENT-001`

- Claim title: **Single fail-closed SFT admission law**
- Receipt:
  `sha256:0e21ffcb217271ddfab4000901691761c2b7fd423b93d27f81ab757f0300c58c`
- Status: model admitted, depth-independent, independently replicated
- Exact role in E04: the complete fail-closed authority path preserves adverse,
  rejected and unresolved dispositions; an incomplete route does not gain
  authority.

Sol's first rebuild, its changed step and the width-only conclusion remain in
the checking book. The closed gate is an educational image of a halt: it opens
only after every declared classroom checkpoint has a visible record. The gate
is not the formal admission engine.

### `SFT-ENG-INDEPENDENT-CHECK-001`

- Claim title: **Implementation-distinct check**
- Receipt:
  `sha256:927d1b0f871cb92e06ae180a9c1fbcf6b2fe0a024135fb487fbf41a9b082b364`
- Status: model admitted, empirically tested and independently reconstructed
- Branch: Engineering Translation
- Exact bounded role in E04: an independent check reconstructs through a
  separately identified implementation path while preserving specifications,
  results, disagreements and provenance.

Ivo uses the original source, fresh pieces and a separate work area without
seeing the team's answer. This is a child-scale translation of a fresh path. It
does not claim that two children constitute formal independent certification.

## What the activities do not prove

A picture plan, memory game, measuring ribbon, garden gate or friend's answer
is a bounded educational demonstration. None derives, validates or changes an
SFT law. The book does not assert that agreement makes a statement true, that
disagreement makes it false, or that a physical fit confers scientific status.

The following remain excluded:

- measurement or observed success selecting a derivation;
- one tool answering a property it was not chosen to measure;
- self-checking relabelled as independent checking;
- deletion of wrong, missing, unresolved or inconvenient attempts;
- guessing, voting, adult confidence or praise as evidence;
- binary digits or signed proof labels;
- semantic numerical zero;
- floating-point equality;
- completed infinity or an ungenerated continuum; and
- outside models, standards or curricula as authority in SFT derivation.

## Story cause-chain

Keep the mission audible: **rebuild and check the Garden Welcome Sign before
the morning visitors arrive**.
...
Use large cards that cannot be swallowed, a light paper or cardboard board and
a soft cloth. Do not hang a heavy sign, use a working gate, give a child a glass
lens, or ask them to stretch a rigid or retracting metal tape. The game's lens
is fictional; a real activity should use a cardboard viewing ring. Adult hands
operate scissors, fasteners and elevated displays.

## Accessibility

- Every picture has a name above it and a distinct shape; colour is secondary.
...
- Reduced-motion mode removes decorative movement without removing state.
- Local narration must match captions exactly. Sound and colour are never
  required to understand an answer.

## Companion Level Four contract

Level Four retains Mia, Sol and Tavi as the permanent trio and introduces only
Ivo. It opens with a magical recap and invitation, then presents nine connected
stages. Each stage has a genuinely different replayable mechanic:

1. sweep a magnifying lens over a picture plan and tag the requested observed
   picture while decoys move between rounds;
...
Every stage includes meaningful success, recoverable mistakes, feedback,
retry, replay and bounded variation. No stage is a nested sequence of “Next”
buttons. Characters move within each scene, and the active tool or record is
large enough for phone touch.

Level Four has its own gentle garden-checking score, distinct from Levels One
through Three. It may use soft wooden taps, leaf rustles and short lens chimes,
but music must never signal an answer. Music, narration and effects have
separate remembered controls. App switching pauses all audio; returning resumes
the current visual state without replaying an old or completed line.

The level stores stage, round, source board, exact failed attempt, trace,
measurement questions and completed checks. Returning to the menu and resuming
must restore those exact records. Completion provides replay, level select and
the sequential next-learning option when such a reviewed level exists.

## Connecting the book and Level Four

The book and game teach the same caused story in distinct ways. The book gives
time for challenge-and-reveal comparison. The game lets the child operate each
tool, make a recoverable mistake and replay with a new bounded arrangement.
...
## Optional picture codes

Codes never reveal an answer, skip an attempt or grant learning progress.

- **`LOOKCLOSE` - page 8:** Ivo's lens reflects the four corner symbols.
- **`SIGNMAKER` - page 11:** the completed frame grows a harmless flower vine.
- **`KEEPFIRST` - page 14:** Sol adds a “useful try” ribbon to the saved board.
- **`SPOTCHANGE` - page 16:** bee and boot perform a cosmetic place-swap replay.
- **`FOLLOWSTEPS` - page 19:** the four trace cards make four soft leaf sounds.
- **`WIDTHONLY` - page 22:** the width ribbon rolls itself into Ivo's satchel.
- **`FRIENDCHECK` - page 25:** matching picture pairs exchange a glow.
- **`MOREWORK` - page 27:** Mia and Ivo share a friendly checking wave.
- **`ALLCHECKED` - page 30:** the open gate plays a short garden greeting.

## External references

The UK Early Years Foundation Stage framework may calibrate language length,
safe activity, communication and accessibility. Publisher conventions and
other educational games may inform usability comparisons. None is authority
for SFT claims, topic order or derivation.
