#!/usr/bin/env python3
"""Generate E04's semantic, scalable-text student edition from canonical JSON."""
...
OUTPUT = BOOK_DIR / "accessible" / "student-book-v1.0.0.html"
...
def main() -> None:
    ...
    for page in pages:
        ...
        if page.get("choices"):
            choice_items = "".join(
                f"<li>{html.escape(choice.replace('-', ' ').title())}</li>"
...
<head>
  ...
  <title>{html.escape(book['title'])} - accessible review 1.0.0</title>
  <style>
