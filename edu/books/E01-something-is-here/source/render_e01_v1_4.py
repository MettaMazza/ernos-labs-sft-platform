#!/usr/bin/env python3
"""Render the coherent 3D-stage E01 review edition 1.4.0."""
...
SOURCE = BOOK_DIR / "source" / "book-v1.4.0.json"
GAME = ROOT / "edu" / "games" / "companion-adventures"
RELEASE_DIR = ROOT / "output" / "pdf" / "edu" / "SFT-EDU-E01-SOMETHING-IS-HERE" / "1.4.0"
STUDENT_PDF = RELEASE_DIR / "SFT-E01-Something-Is-Here-v1.4.0.pdf"
ADULT_PDF = RELEASE_DIR / "SFT-E01-Adult-Guide-v1.4.0.pdf"
ACCESSIBLE_HTML = BOOK_DIR / "accessible" / "student-book-v1.4.0.html"
...
SPRITES = {name: GAME / "public" / "art" / "characters" / "individual" / f"{name}.png" for name in ("tavi", "sol", "vee", "moss", "nori", "luma")}
...
def load_book() -> dict:
    ...
    if len(book["pages"]) != 32:
        raise ValueError("E01 1.4.0 must contain exactly 32 pages")
...
def draw_stage(c: canvas.Canvas, page: dict) -> None:
    ...
    cast = page.get("cast", [])
    positions = {1: [0.12], 2: [0.08, 0.69], 3: [0.03, 0.38, 0.73]}.get(len(cast), [])
...
def draw_cover(c: canvas.Canvas, page: dict) -> None:
    c.drawImage(image(ART["keyart"]), 0, 0, width=PAGE, height=PAGE, preserveAspectRatio=False, mask="auto")
    ...
    c.drawString(18 * mm, 14 * mm, "Maria Smith · Review edition 1.4.0")
...
def render_student(book: dict) -> None:
    ...
    c.setSubject("SFT Early Years coherent 3D story adventure, review 1.4.0")
...
def render_accessible(book: dict) -> None:
    ...
    ACCESSIBLE_HTML.write_text(f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{html.escape(book['title'])} · accessible 1.4.0</title><style>body{{font:1.15rem/1.65 system-ui,sans-serif;max-width:52rem;margin:auto;padding:2rem;color:#20314a;background:#fff9ea}}section{{padding:2rem 0;border-bottom:2px solid #c9b98d}}figure{{margin:1rem 0;padding:1rem;border-left:.35rem solid #2f8f76;background:#fff}}h1,h2{{line-height:1.15}}nav a{{margin-right:.7rem}}</style></head><body><header><h1>{html.escape(book['title'])}</h1><p>{html.escape(book['subtitle'])} · Review 1.4.0</p></header><nav aria-label="Page links">{''.join(f'<a href="#page-{n}">{n}</a>' for n in range(1,33))}</nav><main>{''.join(sections)}</main></body></html>''', encoding="utf-8")
...
def adult_footer(c: canvas.Canvas, doc: SimpleDocTemplate) -> None:
    ...
    c.setFillColor(GREY)
    c.setFont(base.font("regular"), 8)
    c.drawString(20 * mm, 9 * mm, "SFT E01 Adult Guide · Review version 1.4.0")
    c.drawRightString(A4[0] - 20 * mm, 9 * mm, f"Page {doc.page}")
    c.restoreState()


def render_adult() -> None:
    doc = SimpleDocTemplate(str(ADULT_PDF), pagesize=A4, rightMargin=20*mm, leftMargin=20*mm, topMargin=19*mm, bottomMargin=20*mm, title="Adult Guide · E01 · Review 1.4.0", author="Maria Smith")
