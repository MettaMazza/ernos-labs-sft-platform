...
- output: 28 MP3 files at 24 kHz in `public/audio/e01/`
...
`bm_daniel`; Nori uses `bf_isabella`; Vee uses `bf_lily`; Moss uses
`bm_george`; and Luma uses `bf_alice`. This mapping is versioned in the
manifest so returning characters retain recognisable voices.
