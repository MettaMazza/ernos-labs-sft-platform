...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `source-field-matter-signal-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `compact-inference-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `compact-state-to-observable-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `position-mass-spin-environment-method-and-errors-held` — The complete required record is held.
