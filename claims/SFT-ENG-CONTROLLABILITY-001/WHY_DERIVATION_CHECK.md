...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `initial-target-action-sequence-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `reachable-set-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `bounded-input-to-state-transition-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `states-actions-limits-duration-path-and-failures-held` — The complete reconstructible engineering record is retained.
