...
def build_execution(root: Path):
    ...
    files = (
        root / "sft/chemistry/inorganic_acid_base_redox_network_law_v1.py",
        root / "sft/chemistry/inorganic_acid_base_redox_network_batch_v1.py",
        root / "sft/chemistry/inorganic_acid_base_redox_network_validation_v1.py",
        ...
        *(root / path for path in snapshots),
        root / "claims/SFT-CHEM-INORGANIC-ACID-BASE-REDOX-NETWORK-017/execution.py",
    )
    files = tuple(dict.fromkeys(files))
    source_hash = build_source_manifest(root, files).manifest_hash
    independent = root / "claims/SFT-CHEM-INORGANIC-ACID-BASE-REDOX-NETWORK-017/independent_validator.py"
    return ClaimExecution(
        GeneratedObservationalChemistryProgram(INORGANIC_ACID_BASE_REDOX_NETWORK_SPEC, source_hash),
        ExternalCommandValidator(
            "sft-chem-inorganic-acid-base-redox-network-017-independent-python/1",
            (sys.executable, str(independent)),
            independent.parent,
            (independent,),
        ),
        files,
        InorganicAcidBaseRedoxNetworkValidator(root),
    )
