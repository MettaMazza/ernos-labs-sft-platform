...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `artifact-specification-evidence-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `verification-baseline-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `artifact-to-specified-condition-comparison` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `version-requirement-method-result-deviation-and-review-held` — The complete reconstructible engineering record is retained.
