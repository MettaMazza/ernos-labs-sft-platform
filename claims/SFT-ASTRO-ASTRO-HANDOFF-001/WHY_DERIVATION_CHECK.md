...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `law-instrument-earth-life-social-boundary-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `branch-ownership-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `typed-branch-handoff-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `source-owner-dependency-evidence-and-use-held` — The complete required record is held.
