"""Official execution binding for terminal charged-lepton measured-value validation."""
...
def build_execution(root: Path) -> ClaimExecution:
    claim_id = TERMINAL_VALIDATION_SPEC.claim_id
    source_files = (
    ...
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(TERMINAL_VALIDATION_SPEC, source_hash),
        independent_validator=ExternalCommandValidator(
            "sft-phys-validation-charged-lepton-terminal-002-independent-python/1",
            (sys.executable, str(validator)), validator.parent, (validator,),
        ),
        source_files=source_files,
        empirical_validator=ChargedLeptonTerminalValidationExternalValidator(root),
    )
