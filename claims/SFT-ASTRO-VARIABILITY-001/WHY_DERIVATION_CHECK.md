...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `time-series-source-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `time-series-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `state-to-signal-change-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `cadence-window-band-errors-and-nondetections-held` — The complete required record is held.
