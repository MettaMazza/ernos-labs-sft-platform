...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `input-operation-output-purpose-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `function-domain-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `declared-transformation-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `inputs-outputs-state-loss-context-and-failure-held` — The complete reconstructible engineering record is retained.
