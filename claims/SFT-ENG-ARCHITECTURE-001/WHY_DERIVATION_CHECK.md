...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `components-interfaces-allocation-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `architecture-scope-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `composition-and-dependency-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `parts-links-responsibilities-failures-versions-and-rationale-held` — The complete reconstructible engineering record is retained.
