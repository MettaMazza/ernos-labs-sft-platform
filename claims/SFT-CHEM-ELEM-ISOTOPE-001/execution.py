"""Official execution binding for SFT-CHEM-ELEM-ISOTOPE-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in ELEMENTS_PERIODICITY_BATCH_1_SPECS if item.claim_id == 'SFT-CHEM-ELEM-ISOTOPE-001')
    source_files = (
    ...
    validator = root / "claims/SFT-CHEM-ELEM-ISOTOPE-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-elem-isotope-001' + "-independent-python/1",
