...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `target-allowed-region-measurement-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `tolerance-purpose-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `measurement-to-acceptance-region-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `nominal-bounds-unit-uncertainty-rationale-and-version-held` — The complete reconstructible engineering record is retained.
