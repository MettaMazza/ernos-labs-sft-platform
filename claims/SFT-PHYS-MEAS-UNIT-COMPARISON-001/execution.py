"""Official execution binding for SFT-PHYS-MEAS-UNIT-COMPARISON-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in MEASUREMENT_SPECS if item.claim_id == 'SFT-PHYS-MEAS-UNIT-COMPARISON-001')
    source_files = (
        root / "sft/physics/generated_empirical_law.py",
        root / "sft/physics/measurement_laws.py",
        root / "claims/SFT-PHYS-MEAS-UNIT-COMPARISON-001/execution.py",
    ...
    validator = root / "claims/SFT-PHYS-MEAS-UNIT-COMPARISON-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-phys-meas-unit-comparison-001' + "-independent-python/1",
