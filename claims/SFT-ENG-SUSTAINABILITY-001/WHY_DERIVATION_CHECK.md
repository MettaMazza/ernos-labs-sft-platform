...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-lifecycle-resources-affected-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `declared-lifecycle-boundary` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `lifecycle-input-output-impact-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `boundary-resources-emissions-labour-distribution-uncertainty-and-tradeoffs-held` — The complete reconstructible engineering record is retained.
