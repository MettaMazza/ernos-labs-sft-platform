...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `source-event-multiband-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `transient-class-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `progenitor-to-transient-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `localization-time-bands-selection-afterglow-and-errors-held` — The complete required record is held.
