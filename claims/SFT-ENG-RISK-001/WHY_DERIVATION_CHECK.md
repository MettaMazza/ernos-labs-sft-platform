...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `hazard-population-consequence-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `risk-model-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `scenario-to-consequence-comparison` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `scenarios-exposure-severity-frequency-uncertainty-and-values-held` — The complete reconstructible engineering record is retained.
