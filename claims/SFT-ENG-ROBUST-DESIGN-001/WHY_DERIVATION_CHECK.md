...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `design-condition-family-performance-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `registered-variation-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `condition-variation-to-performance-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `conditions-uncertainties-failures-margins-and-boundary-held` — The complete reconstructible engineering record is retained.
