...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `plant-sensor-controller-actuator-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `loop-configuration-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `output-record-to-next-action-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `components-delays-gains-bounds-noise-and-failures-held` — The complete reconstructible engineering record is retained.
