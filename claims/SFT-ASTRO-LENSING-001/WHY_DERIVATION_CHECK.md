...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `source-lens-observer-image-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `lensing-model-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `multiple-path-distortion-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `redshifts-geometry-selection-mass-model-and-errors-held` — The complete required record is held.
