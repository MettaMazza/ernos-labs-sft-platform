# Why SFT-MATH-OAI26-MULTICOLOUR-RAMSEY-007 requires a derivation check

The frozen target is `ErdosProblems.MulticolourTriangleRamsey.erdos_problem_183_explicit`. Its exact source text and quantifier order are in `source_statement.json`; its exact SFT-native proposition is in `translation.json`.
