"""Official execution binding for SFT-MAT-MICRO-DIFFUSION-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in MATERIALS_SPECS if item.claim_id == 'SFT-MAT-MICRO-DIFFUSION-001')
    source_files = (
        ...
        root / "claims/SFT-MAT-MICRO-DIFFUSION-001/execution.py",
    ...
    validator = root / "claims/SFT-MAT-MICRO-DIFFUSION-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalMaterialsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-mat-micro-diffusion-001' + "-independent-python/1",
