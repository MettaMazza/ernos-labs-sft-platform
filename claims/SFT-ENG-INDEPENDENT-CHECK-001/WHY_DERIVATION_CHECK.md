...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `claim-first-implementation-second-implementation-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `independence-criterion-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `independent-reconstruction-comparison` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `specifications-codepaths-results-disagreements-and-provenance-held` — The complete reconstructible engineering record is retained.
