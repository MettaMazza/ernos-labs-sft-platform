...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `need-requirement-design-test-result-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `trace-baseline-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `bidirectional-evidence-chain-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `source-versions-links-changes-gaps-and-owners-held` — The complete reconstructible engineering record is retained.
