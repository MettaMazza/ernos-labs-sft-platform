"""Official execution binding for SFT-CHEM-MEAS-SUBSTANCE-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in CHEMISTRY_SPECS if item.claim_id == 'SFT-CHEM-MEAS-SUBSTANCE-001')
    source_files = (
        root / "sft/chemistry/generated_law.py",
        root / "sft/chemistry/catalog.py",
        root / "sft/chemistry/obligations.py",
        root / "claims/SFT-CHEM-MEAS-SUBSTANCE-001/execution.py",
    ...
    validator = root / "claims/SFT-CHEM-MEAS-SUBSTANCE-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-meas-substance-001' + "-independent-python/1",
