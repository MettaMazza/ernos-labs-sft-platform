...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `model-input-assumption-output-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `model-and-resolution-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `declared-model-to-computed-result-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `equations-discretization-inputs-assumptions-errors-and-comparison-held` — The complete reconstructible engineering record is retained.
