"""Official execution binding for SFT-PHYS-WAVE-DISPERSION-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in WAVE_SPECS if item.claim_id == 'SFT-PHYS-WAVE-DISPERSION-001')
    source_files = (
        root / "sft/physics/generated_empirical_law.py",
        root / 'sft/physics/wave_laws.py',
        root / "claims/SFT-PHYS-WAVE-DISPERSION-001/execution.py",
    ...
    validator = root / "claims/SFT-PHYS-WAVE-DISPERSION-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-phys-wave-dispersion-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
