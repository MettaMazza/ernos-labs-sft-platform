...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `component-condition-transition-consequence-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `failure-definition-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `condition-to-loss-of-function-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `cause-detection-state-effect-severity-and-recovery-held` — The complete reconstructible engineering record is retained.
