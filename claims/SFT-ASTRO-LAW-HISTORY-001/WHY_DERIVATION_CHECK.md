# Universal law and cosmic history distinction
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `law-initial-condition-observation-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `ownership-boundary-declared` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `lawful-history-generation-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `law-receipts-initial-records-and-observations-held` — The complete required record is held.
