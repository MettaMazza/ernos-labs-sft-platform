...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `implementation-condition-metric-result-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `performance-test-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `test-condition-to-performance-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `artifact-version-metric-boundary-results-failures-and-uncertainty-held` — The complete reconstructible engineering record is retained.
