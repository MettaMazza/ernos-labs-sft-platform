"""Official execution binding for SFT-CHEM-MOL-NETWORK-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in MOLECULAR_STRUCTURE_BATCH_2_SPECS if item.claim_id == 'SFT-CHEM-MOL-NETWORK-001')
    source_files = (
    ...
    validator = root / "claims/SFT-CHEM-MOL-NETWORK-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-mol-network-001' + "-independent-python/1",
