...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-fault-safe-state-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `declared-fault-set-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `detected-or-bounded-fault-to-safe-transition-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `faults-detection-actions-safe-state-residual-harm-and-test-held` — The complete reconstructible engineering record is retained.
