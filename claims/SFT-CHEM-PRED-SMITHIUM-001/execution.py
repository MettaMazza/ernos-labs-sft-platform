"""Official execution binding for SFT-CHEM-PRED-SMITHIUM-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in GBLOCK_PREDICTION_SPECS if item.claim_id == 'SFT-CHEM-PRED-SMITHIUM-001')
    source_files = (
        ...
        root / "claims/SFT-CHEM-PRED-SMITHIUM-001/execution.py",
    ...
    validator = root / "claims/SFT-CHEM-PRED-SMITHIUM-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-pred-smithium-001' + "-independent-python/1",
