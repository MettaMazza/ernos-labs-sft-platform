...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `ordered-radiation-matter-state-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `thermal-reconstruction-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `dilution-interaction-transition-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `temperature-scale-species-epoch-and-evidence-held` — The complete required record is held.
