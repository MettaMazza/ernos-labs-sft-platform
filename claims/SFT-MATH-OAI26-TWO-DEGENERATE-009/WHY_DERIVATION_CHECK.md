# Why SFT-MATH-OAI26-TWO-DEGENERATE-009 requires a derivation check

The frozen target is `TwoDegenerateGraphs.twoDegenerateExtremalCounterexample`. Its exact source text and quantifier order are in `source_statement.json`; its exact SFT-native proposition is in `translation.json`.
