...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `internal-state-output-history-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `observation-depth-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `state-distinction-to-record-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `state-set-sensors-history-resolution-and-ambiguity-held` — The complete reconstructible engineering record is retained.
