# Periodicity and transient distinction
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `time-ordered-event-or-cycle-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `temporal-class-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `recurrence-or-one-event-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `epochs-phase-window-baseline-and-alternatives-held` — The complete required record is held.
