# Astronomical model-observation boundary
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `law-state-instrument-model-output-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `model-purpose-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `conditional-simulation-comparison-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `inputs-parameters-observations-errors-and-output-held` — The complete required record is held.
