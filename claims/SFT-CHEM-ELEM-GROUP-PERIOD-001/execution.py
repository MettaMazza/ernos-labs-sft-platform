"""Official execution binding for SFT-CHEM-ELEM-GROUP-PERIOD-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in ELEMENTS_PERIODICITY_BATCH_2_SPECS if item.claim_id == 'SFT-CHEM-ELEM-GROUP-PERIOD-001')
    source_files = (
        root / "sft/chemistry/generated_law.py",
        root / "sft/chemistry/generated_periodic_law.py",
        root / "sft/chemistry/elements_periodicity_batch_2.py",
        root / "claims/SFT-CHEM-ELEM-GROUP-PERIOD-001/execution.py",
    ...
    validator = root / "claims/SFT-CHEM-ELEM-GROUP-PERIOD-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-elem-group-period-001' + "-independent-python/1",
