...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `requirement-test-result-decision-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `acceptance-test-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `registered-criterion-to-result-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `requirement-method-threshold-uncertainty-result-and-decision-held` — The complete reconstructible engineering record is retained.
