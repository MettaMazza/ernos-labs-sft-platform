...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-service-end-assets-record-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `retirement-scope-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `active-to-retired-state-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `authority-users-data-materials-hazards-recovery-and-custody-held` — The complete reconstructible engineering record is retained.
