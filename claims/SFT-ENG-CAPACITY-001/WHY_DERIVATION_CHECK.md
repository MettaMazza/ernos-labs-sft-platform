...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-demand-capability-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `load-profile-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `load-to-service-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `demand-capacity-duration-queue-degradation-and-failure-held` — The complete reconstructible engineering record is retained.
