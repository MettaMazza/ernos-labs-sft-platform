...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-use-context-user-evidence-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `validation-use-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `implemented-system-to-intended-use-comparison` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `users-context-tasks-results-harms-limitations-and-review-held` — The complete reconstructible engineering record is retained.
