...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `requirement-generated-design-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `alternative-grammar-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `design-to-evaluation-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `generator-assumptions-resources-results-failures-and-rejection-held` — The complete reconstructible engineering record is retained.
