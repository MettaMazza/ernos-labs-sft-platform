...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `source-ejecta-medium-generation-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `enrichment-history-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `nucleosynthesis-transfer-mixing-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `species-yields-sources-time-mixing-and-losses-held` — The complete required record is held.
