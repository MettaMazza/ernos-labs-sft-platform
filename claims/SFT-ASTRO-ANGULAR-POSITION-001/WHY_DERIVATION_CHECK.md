# Angular position and reference frame
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `direction-coordinate-frame-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `astrometric-frame-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `frame-transformation-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `epoch-origin-basis-motion-and-uncertainty-held` — The complete required record is held.
