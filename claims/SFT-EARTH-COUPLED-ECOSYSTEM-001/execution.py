"""Official execution binding for SFT-EARTH-COUPLED-ECOSYSTEM-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in EARTH_SPECS if item.claim_id == 'SFT-EARTH-COUPLED-ECOSYSTEM-001')
    source_files = (
        ...
        root / "claims/SFT-EARTH-COUPLED-ECOSYSTEM-001/execution.py",
    ...
    validator = root / "claims/SFT-EARTH-COUPLED-ECOSYSTEM-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalEarthProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-earth-coupled-ecosystem-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
