...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-claim-argument-evidence-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `safety-case-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `evidence-to-bounded-safety-claim-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `claim-scope-hazards-controls-evidence-gaps-and-review-held` — The complete reconstructible engineering record is retained.
