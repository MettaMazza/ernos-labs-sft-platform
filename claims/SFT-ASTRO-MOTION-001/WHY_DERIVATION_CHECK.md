...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `time-indexed-position-spectrum-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `motion-observation-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `position-or-feature-change-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `frame-epochs-baseline-components-and-errors-held` — The complete required record is held.
