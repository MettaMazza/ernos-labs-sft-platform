...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `stars-gas-dust-dark-joint-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `galaxy-observation-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `bound-structure-and-history-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `components-distance-frame-environment-and-time-held` — The complete required record is held.
