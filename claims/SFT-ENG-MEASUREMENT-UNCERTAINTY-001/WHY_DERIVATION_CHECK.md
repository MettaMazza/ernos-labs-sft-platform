...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `result-source-variation-bound-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `uncertainty-model-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `sources-to-declared-result-region-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `components-method-correlation-coverage-boundary-and-version-held` — The complete reconstructible engineering record is retained.
