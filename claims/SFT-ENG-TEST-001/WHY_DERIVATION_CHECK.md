...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `article-condition-procedure-observation-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `test-protocol-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `stimulus-to-response-comparison` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `article-version-setup-procedure-raw-data-failures-and-deviations-held` — The complete reconstructible engineering record is retained.
