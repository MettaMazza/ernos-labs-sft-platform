...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `engineering-anomaly-owner-question-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `disciplinary-ownership-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `validated-deviation-to-categorical-handoff-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `upstream-receipts-implementation-audit-controls-data-and-open-question-held` — The complete reconstructible engineering record is retained.
