...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `artifact-environment-procedure-result-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `reproduction-environment-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `declared-rebuild-to-comparable-result-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `source-dependencies-platform-commands-outputs-failures-and-hashes-held` — The complete reconstructible engineering record is retained.
