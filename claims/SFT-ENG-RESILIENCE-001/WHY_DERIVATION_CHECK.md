...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-disruption-service-recovery-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `disruption-family-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `disruption-to-bounded-recovery-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `service-loss-adaptation-resources-time-residual-and-learning-held` — The complete reconstructible engineering record is retained.
