...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `actor-goal-system-context-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `use-context-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `action-to-system-response-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `actors-preconditions-path-outcomes-misuse-and-harm-held` — The complete reconstructible engineering record is retained.
