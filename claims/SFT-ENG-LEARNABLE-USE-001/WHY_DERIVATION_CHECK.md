...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `new-user-task-guidance-feedback-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `declared-audience-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `instruction-and-feedback-to-correct-use-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `audience-prerequisites-steps-errors-examples-and-tests-held` — The complete reconstructible engineering record is retained.
