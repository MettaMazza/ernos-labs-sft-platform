...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-condition-action-result-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `maintenance-event-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `observed-condition-to-restorative-action-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `condition-diagnosis-parts-action-tests-downtime-and-result-held` — The complete reconstructible engineering record is retained.
