"""Official execution binding for SFT-PHYS-VALIDATION-PROTON-ELECTRON-003."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in VALIDATION_SPECS if item.claim_id == 'SFT-PHYS-VALIDATION-PROTON-ELECTRON-003')
    source_files = (
        ...
        root / "claims/SFT-PHYS-VALIDATION-PROTON-ELECTRON-003/execution.py",
    ...
    validator = root / "claims/SFT-PHYS-VALIDATION-PROTON-ELECTRON-003/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-phys-validation-proton-electron-003' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
