...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `molecule-gas-dust-bound-region-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `tracer-method-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `chemistry-cooling-collapse-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `species-density-temperature-motion-and-boundary-held` — The complete required record is held.
