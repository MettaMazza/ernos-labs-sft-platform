...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `source-redshift-distance-time-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `expansion-probe-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `distance-redshift-comparison-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `standardization-calibration-selection-covariance-and-era-held` — The complete required record is held.
