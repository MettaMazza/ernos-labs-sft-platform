# Multiverse admissibility boundary
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `declared-domain-hypothesis-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `observable-consequence-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `observable-consequence-or-nonadmission-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `premises-domain-predictions-controls-and-access-held` — The complete required record is held.
