...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `baseline-change-dependency-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `dependency-graph-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `changed-element-to-affected-evidence-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `proposal-links-risks-tests-results-and-rollback-held` — The complete reconstructible engineering record is retained.
