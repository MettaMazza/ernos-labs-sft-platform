...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-operation-resource-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `resource-accounting-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `operation-to-consumption-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `resource-stock-flow-units-time-loss-and-source-held` — The complete reconstructible engineering record is retained.
