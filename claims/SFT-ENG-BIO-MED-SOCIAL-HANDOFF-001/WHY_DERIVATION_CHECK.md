# Biological, medical and social handoff
...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `organism-user-population-device-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `human-use-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `science-and-context-to-intervention-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `biological-clinical-social-receipts-users-ethics-harms-and-access-held` — The complete reconstructible engineering record is retained.
