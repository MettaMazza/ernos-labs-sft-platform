"""Official execution binding for SFT-MED-PREVENTION-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in MEDICINE_SPECS if item.claim_id == 'SFT-MED-PREVENTION-001')
    source_files = (
        ...
        root / "claims/SFT-MED-PREVENTION-001/execution.py", root / "sft/physics/generated_empirical_law.py",
    ...
    validator = root / "claims/SFT-MED-PREVENTION-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalMedicineProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-med-prevention-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
