...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `measurand-instrument-method-record-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `measurement-procedure-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `property-to-record-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `definition-unit-instrument-condition-uncertainty-and-custody-held` — The complete reconstructible engineering record is retained.
