...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `design-version-baseline-change-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `configuration-boundary-declared` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `proposal-to-approved-version-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `baseline-rationale-review-impact-tests-and-rollback-held` — The complete reconstructible engineering record is retained.
