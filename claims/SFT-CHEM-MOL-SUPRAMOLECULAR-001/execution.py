"""Official execution binding for SFT-CHEM-MOL-SUPRAMOLECULAR-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in MOLECULAR_STRUCTURE_BATCH_2_SPECS if item.claim_id == 'SFT-CHEM-MOL-SUPRAMOLECULAR-001')
    source_files = (
        ...
        root / "claims/SFT-CHEM-MOL-SUPRAMOLECULAR-001/execution.py",
    ...
    validator = root / "claims/SFT-CHEM-MOL-SUPRAMOLECULAR-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-mol-supramolecular-001' + "-independent-python/1",
