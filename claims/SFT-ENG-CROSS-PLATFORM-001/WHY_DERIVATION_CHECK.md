...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `artifact-platform-environment-result-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `declared-platform-matrix` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `same-specification-across-platforms-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `platform-version-dependencies-commands-results-and-differences-held` — The complete reconstructible engineering record is retained.
