...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `all-sky-frequency-temperature-polarization-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `cmb-release-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `background-anisotropy-spectrum-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `frequency-calibration-mask-beam-foregrounds-and-covariance-held` — The complete required record is held.
