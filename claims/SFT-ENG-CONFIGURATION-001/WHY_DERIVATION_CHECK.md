...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `artifact-version-dependency-environment-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `configuration-baseline-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `configuration-to-reproduced-behaviour-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `source-dependencies-settings-platform-hashes-and-date-held` — The complete reconstructible engineering record is retained.
