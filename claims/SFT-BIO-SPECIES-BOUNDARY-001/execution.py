"""Official execution binding for SFT-BIO-SPECIES-BOUNDARY-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in BIOLOGY_SPECS if item.claim_id == 'SFT-BIO-SPECIES-BOUNDARY-001')
    source_files = (
        ...
        root / "claims/SFT-BIO-SPECIES-BOUNDARY-001/execution.py", root / "sft/physics/generated_empirical_law.py",
    ...
    validator = root / "claims/SFT-BIO-SPECIES-BOUNDARY-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalBiologyProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-bio-species-boundary-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
