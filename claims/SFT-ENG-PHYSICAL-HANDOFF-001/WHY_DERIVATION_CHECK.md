...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `physical-law-component-environment-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `physical-design-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `law-to-physical-implementation-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `law-receipts-materials-geometry-boundary-measurements-and-failures-held` — The complete reconstructible engineering record is retained.
