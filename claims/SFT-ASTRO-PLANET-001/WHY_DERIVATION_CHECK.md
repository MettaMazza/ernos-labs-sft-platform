# Planet and satellite classification
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `body-orbit-mass-shape-context-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `classification-version-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `feature-and-dynamical-class-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `host-orbit-mass-radius-composition-and-method-held` — The complete required record is held.
