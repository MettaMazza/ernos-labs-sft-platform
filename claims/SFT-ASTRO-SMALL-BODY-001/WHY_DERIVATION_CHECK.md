...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `selection-bounded-comet-asteroid-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `small-body-survey-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `orbit-size-composition-population-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `survey-selection-orbits-activity-and-completeness-held` — The complete required record is held.
