"""Official execution binding for SFT-PHYS-VALIDATION-RELATIVISTIC-FIELDS-003."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in VALIDATION_SPECS if item.claim_id == 'SFT-PHYS-VALIDATION-RELATIVISTIC-FIELDS-003')
    source_files = (
    ...
    validator = root / "claims/SFT-PHYS-VALIDATION-RELATIVISTIC-FIELDS-003/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-phys-validation-relativistic-fields-003' + "-independent-python/1",
