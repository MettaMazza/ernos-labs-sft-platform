...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `host-signal-planet-candidate-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `detection-method-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `transit-radial-imaging-or-timing-inference` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `method-selection-host-activity-period-and-errors-held` — The complete required record is held.
