"""Official execution binding for SFT-PHYS-VALIDATION-ATOMIC-CUBIC-SUPPORT-004."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in VALIDATION_SPECS if item.claim_id == 'SFT-PHYS-VALIDATION-ATOMIC-CUBIC-SUPPORT-004')
    source_files = (
        ...
        root / "claims/SFT-PHYS-VALIDATION-ATOMIC-CUBIC-SUPPORT-004/execution.py",
    ...
    validator = root / "claims/SFT-PHYS-VALIDATION-ATOMIC-CUBIC-SUPPORT-004/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-phys-validation-atomic-cubic-support-004' + "-independent-python/1",
