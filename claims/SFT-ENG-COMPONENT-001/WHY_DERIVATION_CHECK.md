...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `bounded-part-state-capability-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `component-boundary-declared` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `part-to-declared-function-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `identity-version-input-output-state-and-failure-held` — The complete reconstructible engineering record is retained.
