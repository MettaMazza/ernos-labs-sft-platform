...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-state-target-actuator-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `control-envelope-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `observed-state-to-bounded-action-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `target-state-measurement-action-limit-time-and-effect-held` — The complete reconstructible engineering record is retained.
