...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `prototype-version-purpose-test-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `prototype-purpose-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `partial-implementation-to-bounded-learning-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `omissions-fidelity-setup-results-failures-and-next-decision-held` — The complete reconstructible engineering record is retained.
