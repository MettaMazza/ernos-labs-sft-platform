"""Implementation-distinct product validator for SFT-EARTH-STATE-001."""
from itertools import product
import json
import sys
CLAIM_ID = 'SFT-EARTH-STATE-001'
DOMAINS = (('answer-only-or-unbounded-carrier', 'time-indexed-complete-reservoir-carrier'), ('boundary-or-interface-erased', 'registered-state-vector-bounded'), ('imported-fitted-or-missing-relation', 'stock-relation-state-assignment'), ('favourable-output-with-incomplete-record', 'stocks-relations-missingness-and-time-held'), ('model-proxy-forecast-or-observation-conflated', 'evidence-class-explicit'), ('prior-target-consensus-or-application-selected', 'root-bound-forward-forcing'), ('one-favourable-case-with-erased-alternatives', 'positive-finite-successor-retains-all-rows'), ('free-parameter-exception-or-opaque-oracle', 'no-extra-rule'))
SURVIVOR = 'time-indexed-complete-reservoir-carrier__registered-state-vector-bounded__stock-relation-state-assignment__stocks-relations-missingness-and-time-held__evidence-class-explicit__root-bound-forward-forcing__positive-finite-successor-retains-all-rows__no-extra-rule'
def main():
