# SFT-derived self-hosted V4 handoff
...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `v3-corpus-language-compiler-runtime-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `future-rebuild-authority-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `admitted-semantics-to-self-hosted-rebuild-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `all-upstream-receipts-specification-bootstrap-tests-divergences-and-provenance-held` — The complete reconstructible engineering record is retained.
