"""Official execution binding for SFT-CHEM-REDOX-OXIDATION-STATE-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in REDOX_BATCH_1_SPECS if item.claim_id == 'SFT-CHEM-REDOX-OXIDATION-STATE-001')
    source_files = (
    ...
    validator = root / "claims/SFT-CHEM-REDOX-OXIDATION-STATE-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-chem-redox-oxidation-state-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
