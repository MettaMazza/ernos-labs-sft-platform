...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `progenitor-transition-remnant-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `endpoint-class-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `mass-composition-loss-collapse-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `progenitor-event-remnant-ejecta-and-uncertainty-held` — The complete required record is held.
