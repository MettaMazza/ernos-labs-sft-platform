"""Official execution binding for SFT-CHEM-STOICH-MIXTURE-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in COMPOSITION_STOICHIOMETRY_BATCH_1_SPECS if item.claim_id == 'SFT-CHEM-STOICH-MIXTURE-001')
    source_files = (
        root / "sft/chemistry/generated_law.py",
        root / 'sft/chemistry/composition_stoichiometry_batch_1.py',
        root / "claims/SFT-CHEM-STOICH-MIXTURE-001/execution.py",
    ...
    validator = root / "claims/SFT-CHEM-STOICH-MIXTURE-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-stoich-mixture-001' + "-independent-python/1",
