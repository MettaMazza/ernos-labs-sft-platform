...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `requirement-test-evidence-decision-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `acceptance-protocol-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `condition-to-acceptance-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `criterion-method-result-uncertainty-failure-and-approver-held` — The complete reconstructible engineering record is retained.
