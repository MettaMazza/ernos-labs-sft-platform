# Portable data and record format
...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `record-format-reader-platform-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `format-version-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `serialized-record-to-equivalent-reading-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `schema-encoding-version-units-errors-and-migrations-held` — The complete reconstructible engineering record is retained.
