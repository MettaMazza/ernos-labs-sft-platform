...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-demand-time-population-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `reliability-population-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `required-service-over-trials-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `units-demands-duration-failures-censoring-and-repair-held` — The complete reconstructible engineering record is retained.
