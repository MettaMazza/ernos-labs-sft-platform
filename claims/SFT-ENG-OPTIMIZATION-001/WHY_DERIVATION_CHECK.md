...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `finite-design-set-objective-constraint-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `finite-search-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `enumeration-to-admissible-optimum-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `designs-objective-constraints-ties-failures-and-boundary-held` — The complete reconstructible engineering record is retained.
