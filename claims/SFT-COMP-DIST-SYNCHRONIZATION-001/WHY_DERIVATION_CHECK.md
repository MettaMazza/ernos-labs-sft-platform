...
> The structural One supplies one canonical jointly enabled process transitions form with its identity trace and no unrecorded predecessor.

Depth-independent successor:

> Appending one generated jointly enabled process transitions form adds its exact carrier row, relations, trace positions and new pair/interface distinctions while preserving every earlier record.
...
- `distributed_computation-witness-1` — distributed_computation exact operational witness 1 for SFT-COMP-DIST-SYNCHRONIZATION-001
- `distributed_computation-witness-2` — distributed_computation exact operational witness 2 for SFT-COMP-DIST-SYNCHRONIZATION-001
- `distributed_computation-witness-3` — distributed_computation exact operational witness 3 for SFT-COMP-DIST-SYNCHRONIZATION-001
...
Conventional terms are downstream correspondence only: synchronization, rendezvous.
