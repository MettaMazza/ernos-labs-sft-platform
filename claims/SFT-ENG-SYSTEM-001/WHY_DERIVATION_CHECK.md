...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `parts-relations-boundary-purpose-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `system-purpose-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `joint-operation-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `parts-environment-state-resources-users-and-failures-held` — The complete reconstructible engineering record is retained.
