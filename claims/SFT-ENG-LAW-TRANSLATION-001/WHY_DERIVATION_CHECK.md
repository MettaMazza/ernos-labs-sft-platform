...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `admitted-law-requirement-design-test-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `translation-purpose-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `law-to-bounded-implementation-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `upstream-receipt-assumptions-choices-boundary-results-and-failures-held` — The complete reconstructible engineering record is retained.
