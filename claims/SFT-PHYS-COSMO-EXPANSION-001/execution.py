"""Official execution binding for SFT-PHYS-COSMO-EXPANSION-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in COSMOLOGICAL_BOUNDARY_SPECS if item.claim_id == 'SFT-PHYS-COSMO-EXPANSION-001')
    source_files = (
        root / "sft/physics/generated_empirical_law.py",
        root / 'sft/physics/cosmological_boundary_laws.py',
        
        root / "claims/SFT-PHYS-COSMO-EXPANSION-001/execution.py",
    ...
    validator = root / "claims/SFT-PHYS-COSMO-EXPANSION-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-phys-cosmo-expansion-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
