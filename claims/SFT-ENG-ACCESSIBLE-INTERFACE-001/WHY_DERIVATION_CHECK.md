...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `user-task-interface-context-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `user-and-task-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `action-to-understandable-result-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `users-input-modes-output-modes-errors-guidance-and-testing-held` — The complete reconstructible engineering record is retained.
