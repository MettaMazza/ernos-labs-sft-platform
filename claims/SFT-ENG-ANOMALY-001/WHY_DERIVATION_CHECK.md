...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `artifact-expected-observed-context-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `anomaly-protocol-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `registered-expectation-to-deviation-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `versions-setup-raw-data-controls-repeats-and-uncertainty-held` — The complete reconstructible engineering record is retained.
