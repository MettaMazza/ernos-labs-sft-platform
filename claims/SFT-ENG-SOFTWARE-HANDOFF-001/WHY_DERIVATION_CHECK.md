# Software and computation handoff
...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `formal-process-implementation-platform-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `software-implementation-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `semantics-to-executable-artifact-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `specification-source-build-platform-tests-and-resource-record-held` — The complete reconstructible engineering record is retained.
