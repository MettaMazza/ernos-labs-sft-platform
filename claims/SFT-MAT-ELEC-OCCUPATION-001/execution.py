"""Official execution binding for SFT-MAT-ELEC-OCCUPATION-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in MATERIALS_SPECS if item.claim_id == 'SFT-MAT-ELEC-OCCUPATION-001')
    source_files = (
        ...
        root / "claims/SFT-MAT-ELEC-OCCUPATION-001/execution.py",
    ...
    validator = root / "claims/SFT-MAT-ELEC-OCCUPATION-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalMaterialsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-mat-elec-occupation-001' + "-independent-python/1",
