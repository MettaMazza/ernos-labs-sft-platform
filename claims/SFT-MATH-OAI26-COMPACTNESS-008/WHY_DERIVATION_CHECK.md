# Why SFT-MATH-OAI26-COMPACTNESS-008 requires a derivation check

The frozen target is `CompactnessConjecture.quantitativeCompactnessCounterexample`. Its exact source text and quantifier order are in `source_statement.json`; its exact SFT-native proposition is in `translation.json`.
