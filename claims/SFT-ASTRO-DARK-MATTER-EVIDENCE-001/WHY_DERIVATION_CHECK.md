...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `visible-tracer-gravitational-residual-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `dark-inference-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `observed-motion-lensing-growth-comparison` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `baryons-geometry-selection-alternatives-and-errors-held` — The complete required record is held.
