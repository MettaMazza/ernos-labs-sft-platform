# Causal order and accessibility cone

Claim: `SFT-PHYS-SPACETIME-CAUSAL-ORDER-001`
...
`complete-fold-carrier__limiting-path-reachability-order__source-bound-proof-trace__capability-closed-prediction__separate-measurement-record__complete-registered-rows__one-successor-closure__no-extra-rule`

The exact result is:

> Causal order is exact generated path reachability bounded by limiting propagation support.

The base certificate is: One event is one generated state transition with one held recurrence count, spatial relation and causal predecessor trace.

The successor certificate is: Adding one event preserves every prior event, interval, causal and source relation and appends exactly its generated adjacent paths and observation records.
