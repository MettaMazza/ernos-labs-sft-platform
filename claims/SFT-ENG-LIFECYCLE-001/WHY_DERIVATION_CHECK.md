...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `need-design-build-use-retire-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `lifecycle-boundary-declared` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `versioned-lifecycle-transition-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `resources-decisions-configurations-failures-impacts-and-ownership-held` — The complete reconstructible engineering record is retained.
