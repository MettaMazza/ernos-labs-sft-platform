# Why SFT-QUANTUM-OAI26-PARALLEL-REPETITION-001 requires a derivation check

The frozen target is `QuantumParallelRepetition.distributionUniformExponential`. Its exact source text and quantifier order are in `source_statement.json`; its exact SFT-native proposition is in `translation.json`.
