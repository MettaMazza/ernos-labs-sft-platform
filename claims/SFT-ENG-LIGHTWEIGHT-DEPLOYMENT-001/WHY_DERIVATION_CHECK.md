...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `artifact-minimal-runtime-install-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `installation-environment-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `declared-package-to-working-system-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `runtime-dependencies-size-permissions-network-and-failures-held` — The complete reconstructible engineering record is retained.
