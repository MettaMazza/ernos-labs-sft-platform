...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `angular-spectrum-feature-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `spectrum-analysis-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `bounded-harmonic-recurrence-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `multipoles-peaks-covariance-foregrounds-and-model-boundary-held` — The complete required record is held.
