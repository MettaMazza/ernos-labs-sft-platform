...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `design-condition-failure-record-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `implementation-version-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `implementation-path-to-failed-result-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `requirements-choices-build-state-test-cause-and-uncertainty-held` — The complete reconstructible engineering record is retained.
