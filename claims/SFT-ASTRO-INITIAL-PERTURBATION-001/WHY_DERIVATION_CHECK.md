...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `background-anisotropy-seed-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `seed-inference-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `initial-distinction-to-later-structure-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `scale-spectrum-phase-covariance-and-transfer-held` — The complete required record is held.
