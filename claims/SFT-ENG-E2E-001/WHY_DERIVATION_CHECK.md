...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `source-input-complete-pipeline-output-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `pipeline-boundary-declared` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `declared-entry-to-final-result-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `versions-stages-intermediates-failures-duration-and-receipts-held` — The complete reconstructible engineering record is retained.
