# Groups, clusters and environment
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `member-galaxy-gas-potential-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `cluster-definition-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `membership-binding-environment-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `selection-redshift-membership-mass-method-and-errors-held` — The complete required record is held.
