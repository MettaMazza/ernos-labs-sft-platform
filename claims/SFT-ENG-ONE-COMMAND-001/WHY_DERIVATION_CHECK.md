...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `repository-public-command-result-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `repository-version-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `single-entry-to-complete-verification-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `command-platform-stages-progress-duration-failures-and-receipt-held` — The complete reconstructible engineering record is retained.
