...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-partition-interface-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `decomposition-boundary-declared` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `decomposition-recomposition-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `partition-contract-coupling-state-loss-and-tests-held` — The complete reconstructible engineering record is retained.
