...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `alternatives-criteria-context-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `tradeoff-context-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `criteria-wise-comparison-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `criteria-units-evidence-conflicts-stakeholders-and-losses-held` — The complete reconstructible engineering record is retained.
