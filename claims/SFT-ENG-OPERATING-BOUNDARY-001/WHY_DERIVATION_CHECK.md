...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-environment-state-limit-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `declared-operation-envelope` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `condition-to-valid-operation-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `conditions-duration-loads-transients-exceptions-and-failures-held` — The complete reconstructible engineering record is retained.
