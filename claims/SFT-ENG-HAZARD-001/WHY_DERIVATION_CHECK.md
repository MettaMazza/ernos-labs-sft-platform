...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `source-condition-exposed-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `hazard-scenario-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `hazard-to-harm-path-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `source-path-exposure-severity-likelihood-and-uncertainty-held` — The complete reconstructible engineering record is retained.
