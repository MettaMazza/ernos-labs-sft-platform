...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-fault-sensor-response-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `fault-family-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `fault-evidence-to-bounded-response-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `fault-detection-latency-action-state-recovery-and-misses-held` — The complete reconstructible engineering record is retained.
