"""Official execution binding for SFT-CHEM-AB-BUFFER-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in ACID_BASE_BATCH_1_SPECS if item.claim_id == 'SFT-CHEM-AB-BUFFER-001')
    source_files = (
        ...
        root / "claims/SFT-CHEM-AB-BUFFER-001/execution.py",
    ...
    validator = root / "claims/SFT-CHEM-AB-BUFFER-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-ab-buffer-001' + "-independent-python/1",
