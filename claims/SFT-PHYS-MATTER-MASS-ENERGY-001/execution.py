"""Official execution binding for SFT-PHYS-MATTER-MASS-ENERGY-001."""
...
from sft.physics.matter_nuclear_laws import VALUE_SPECS
from sft.verification import ClaimExecution
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in MATTER_NUCLEAR_SPECS if item.claim_id == 'SFT-PHYS-MATTER-MASS-ENERGY-001')
    source_files = (
        root / "sft/physics/generated_empirical_law.py",
        root / 'sft/physics/matter_nuclear_laws.py',
        
        root / "sft/physics/measured_value.py",
        root / "claims/SFT-PHYS-MATTER-MASS-ENERGY-001/execution.py",
    ...
    validator = root / "claims/SFT-PHYS-MATTER-MASS-ENERGY-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalPhysicsProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-phys-matter-mass-energy-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
