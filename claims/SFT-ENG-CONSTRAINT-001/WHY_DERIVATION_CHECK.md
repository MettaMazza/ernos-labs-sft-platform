...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `design-variable-limit-source-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `constraint-authority-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `variable-to-admissible-region-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `source-units-tolerance-context-version-and-conflict-held` — The complete reconstructible engineering record is retained.
