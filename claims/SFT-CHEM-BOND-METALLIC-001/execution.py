"""Official execution binding for SFT-CHEM-BOND-METALLIC-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in BONDING_MOLECULAR_BATCH_1_SPECS if item.claim_id == 'SFT-CHEM-BOND-METALLIC-001')
    source_files = (
        root / "sft/chemistry/generated_law.py",
        root / "sft/chemistry/generated_goldbook_extended_law.py",
        root / "sft/chemistry/bonding_molecular_batch_1.py",
        root / "claims/SFT-CHEM-BOND-METALLIC-001/execution.py",
    ...
    validator = root / "claims/SFT-CHEM-BOND-METALLIC-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalChemistryProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator(
            'sft-chem-bond-metallic-001' + "-independent-python/1",
