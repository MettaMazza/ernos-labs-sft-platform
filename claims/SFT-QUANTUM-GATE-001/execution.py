"""Official execution binding for SFT-QUANTUM-GATE-001."""
from pathlib import Path
import sys
from sft.engine import ExternalCommandValidator
from sft.engine.source import build_source_manifest
from sft.quantum_computation.generated_law import GeneratedQuantumProgram
from sft.quantum_computation.gate.law import SPEC
...
def build_execution(root: Path) -> ClaimExecution:
    source_files = (
        ...
        root / "sft/quantum_computation/gate/law.py",
        root / "claims/SFT-QUANTUM-GATE-001/execution.py",
    )
    source_hash = build_source_manifest(root, source_files).manifest_hash
    validator = root / "claims/SFT-QUANTUM-GATE-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedQuantumProgram(SPEC, source_hash),
        independent_validator=ExternalCommandValidator(
            "sft-quantum-gate-001-independent-python/1",
