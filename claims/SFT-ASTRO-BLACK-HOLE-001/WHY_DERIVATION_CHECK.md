# Black-hole candidate and horizon evidence
...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `source-orbit-accretion-wave-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `horizon-evidence-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `compactness-and-horizon-consequence-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `mass-spin-environment-messenger-and-alternatives-held` — The complete required record is held.
