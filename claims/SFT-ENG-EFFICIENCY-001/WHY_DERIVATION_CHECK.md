...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `input-useful-output-loss-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `efficiency-boundary-declared` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `accounted-output-to-input-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `boundary-input-output-loss-time-and-uncertainty-held` — The complete reconstructible engineering record is retained.
