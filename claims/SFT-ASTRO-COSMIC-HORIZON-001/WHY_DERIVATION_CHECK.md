...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `observer-signal-reach-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `horizon-definition-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `finite-history-causal-access-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `observer-epoch-propagation-law-and-boundary-held` — The complete required record is held.
