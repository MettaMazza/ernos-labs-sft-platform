...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `fuel-reaction-product-radiation-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `nuclear-network-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `reaction-network-to-output-ledger` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `species-rates-losses-time-and-neutrino-photon-record-held` — The complete required record is held.
