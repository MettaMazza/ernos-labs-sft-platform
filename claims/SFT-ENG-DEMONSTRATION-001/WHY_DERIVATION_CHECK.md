...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `artifact-scenario-observer-result-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `demonstration-scenario-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `declared-scenario-to-visible-behaviour-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `version-setup-scenario-observations-failures-and-limits-held` — The complete reconstructible engineering record is retained.
