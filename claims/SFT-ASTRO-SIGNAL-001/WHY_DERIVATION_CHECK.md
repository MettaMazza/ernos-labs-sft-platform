...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `source-path-detector-signal-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `signal-channel-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `propagation-and-detection-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `emission-path-band-arrival-and-quality-held` — The complete required record is held.
