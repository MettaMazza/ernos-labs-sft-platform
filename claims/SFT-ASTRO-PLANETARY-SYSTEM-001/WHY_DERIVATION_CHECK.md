...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `host-orbiting-body-joint-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `system-detection-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `bound-orbit-and-interaction-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `host-bodies-orbits-masses-epochs-and-errors-held` — The complete required record is held.
