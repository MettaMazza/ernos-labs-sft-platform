...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `components-build-sequence-system-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `integration-build-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `assembly-to-joint-behaviour-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `versions-order-interfaces-tests-failures-and-rollbacks-held` — The complete reconstructible engineering record is retained.
