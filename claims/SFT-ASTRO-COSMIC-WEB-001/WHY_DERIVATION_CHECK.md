...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `tracer-node-filament-void-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `web-extraction-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `adjacency-density-history-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `tracer-selection-scale-redshift-method-and-errors-held` — The complete required record is held.
