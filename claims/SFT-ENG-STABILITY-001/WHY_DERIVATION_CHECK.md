...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-perturbation-bound-return-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `stability-region-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `perturbed-trajectory-to-operating-region-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `initial-state-disturbance-time-bound-overshoot-and-failure-held` — The complete reconstructible engineering record is retained.
