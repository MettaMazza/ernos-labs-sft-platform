...
- `carrier`: reject `answer-only-or-unversioned-artifact`; preserve `system-service-time-state-carrier` — The claim-specific engineering carrier is retained.
- `boundary`: reject `operating-test-or-lifecycle-boundary-erased`; preserve `availability-window-bounded` — The operating, test, user and lifecycle boundary remains explicit.
- `relation`: reject `target-fitted-status-or-application-selected-relation`; preserve `serviceable-time-relation` — Only generated relations and admitted upstream laws act.
- `record`: reject `successful-output-only`; preserve `uptime-downtime-cause-maintenance-demand-and-window-held` — The complete reconstructible engineering record is retained.
