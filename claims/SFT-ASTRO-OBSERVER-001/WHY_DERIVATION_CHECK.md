...
- `carrier`: reject `answer-only-or-unbounded-carrier`; preserve `site-platform-instrument-observer-carrier` — The claim-specific carrier is retained.
- `boundary`: reject `frame-time-or-selection-erased`; preserve `observer-frame-bounded` — The exact observational boundary is explicit.
- `relation`: reject `imported-fitted-or-missing-relation`; preserve `observer-to-record-relation` — Only generated relations and admitted dependencies act.
- `record`: reject `favourable-output-incomplete-record`; preserve `site-platform-clock-calibration-and-frame-held` — The complete required record is held.
