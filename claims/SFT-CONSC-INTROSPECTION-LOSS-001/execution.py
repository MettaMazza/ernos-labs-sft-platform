"""Official execution binding for SFT-CONSC-INTROSPECTION-LOSS-001."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in CONSCIOUSNESS_SPECS if item.claim_id == 'SFT-CONSC-INTROSPECTION-LOSS-001')
    source_files = (
        ...
        root / "claims/SFT-CONSC-INTROSPECTION-LOSS-001/execution.py",
    ...
    validator = root / "claims/SFT-CONSC-INTROSPECTION-LOSS-001/independent_validator.py"
    return ClaimExecution(
        program=GeneratedEmpiricalConsciousnessProgram(spec, source_hash),
        independent_validator=ExternalCommandValidator('sft-consc-introspection-loss-001' + "-independent-python/1", (sys.executable, str(validator)), validator.parent, (validator,)),
