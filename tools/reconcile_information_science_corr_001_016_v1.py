...
PREVIOUS = ROOT / "census/information_science_discipline_current_reconciliation_v16.json"
OUT = ROOT / "census/information_science_discipline_current_reconciliation_v17.json"
AUDIT = ROOT / "audits/INFORMATION_SCIENCE_CORR_001_016_COMPLETION_2026-07-29.json"

from sft.information_science.corr_001_016_laws_v1 import IDS
...
def main():
    ...
    if canonical(frozen_body) != frozen_identity or canonical(previous_body) != previous_identity or previous["current_closed_count"] != 216:
        raise SystemExit("CORR reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        obligation = f"SFT-INFO-OBL-CORR-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["information_science_obligation"] != obligation:
            raise SystemExit("CORR reconciliation halt: " + claim_id)
        rows.append({
    ...
    families["CORR"] = rows
    value = {
        "schema": "sft-v3-information-science-discipline-current-reconciliation/17",
        "date": "2026-07-29",
        ...
        "current_open_count": 30,
        "current_completion_fraction": "232/262",
        "current_completion_percent": "88.5%",
    ...
    audit = {
        "schema": "sft-v3-information-science-corr-completion/1",
        "date": "2026-07-29",
        "family": "CORR-001--016",
        ...
        "current_information_science_progress": "232/262",
        "current_information_science_percent": "88.5%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 232, "open": 30, "percent": "88.5%", "identity": value["reconciliation_identity"]}, indent=2))
