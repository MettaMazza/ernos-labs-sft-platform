#!/usr/bin/env python3
"""Admit and materialize omitted matter/flavour completion laws."""
...
def load_execution(claim_id: str):
    ...
    definition = importlib.util.spec_from_file_location("sft_physics_completion_" + claim_id.replace("-", "_"), path)
...
def main() -> None:
    ...
    receipts = {}
    for position, item in enumerate(COMPLETION_SPECS, 1):
        if item.claim_id in existing:
            receipt = read_receipt(ROOT / existing[item.claim_id]["receipt_path"])
            print(f"[{position}/{len(COMPLETION_SPECS)}] retained {item.claim_id}: {receipt.receipt_hash}")
        else:
            ...
            existing = {row["claim_id"]: row for row in json.loads(census_path.read_text(encoding="utf-8"))["claims"]}
            print(f"[{position}/{len(COMPLETION_SPECS)}] admitted {item.claim_id}: {receipt.receipt_hash}")
    ...
    for position, item in enumerate(COMPLETION_SPECS, 1):
        ...
        print(f"[{position}/{len(COMPLETION_SPECS)}] materialized {item.claim_id}")
