#!/usr/bin/env python3
"""Admit the complete SCIX-001 through SCIX-025 family sequentially."""
...
from sft.computation.scix_001_025_laws_v1 import IDS, SPECS
...
def seals():
    for tool, status in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Classical Computation SCIX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("scix_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        row = next(item for item in json.loads(claims_path.read_text())["claims"] if item["claim_id"] == claim_id)
        sealed, independent, empirical = captured["sealed"], captured["independent"], captured["empirical"]
        package = ROOT / "claims" / claim_id
        certificate = {
            "claim_id": claim_id,
            "classical_computation_obligation": f"SFT-COMP-OBL-SCIX-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/computation_scix_001_025_target_registry_v1.json",
                "empirical_protocol": "post-registry exact scientific execution custody",
