"""Materialize the pre-source-sealed first acid/base batch."""
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in ACID_BASE_BATCH_1_SPECS if item.claim_id == {spec.claim_id!r})
    source_files = (
...
def main() -> None:
    ...
    for spec in ACID_BASE_BATCH_1_SPECS:
        ...
        registration["pre_source_derivation_seal"] = "experiments/sealed_predictions/chemistry_acid_base_batch_1_pre_source.json"
        ...
        record["pre_source_derivation_seal"] = "experiments/sealed_predictions/chemistry_acid_base_batch_1_pre_source.json"
        record["authority_support_registry"] = "experiments/external_sources/chemistry/observations_acid_base_batch_1.json"
        write(experiment / "registration.json", json.dumps(record, indent=2, sort_keys=True) + "\n")
