#!/usr/bin/env python3
"""Admit and materialize terminal hadron multiplet and Regge closure."""
...
from sft.physics.hadron_regge_successor_laws_v1 import HADRON_REGGE_SPEC  # noqa: E402
from sft.physics.hadron_regge_successor_validation_v1 import (  # noqa: E402
...
def load_execution():
    path = ROOT / "claims" / HADRON_REGGE_SPEC.claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_hadron_regge_terminal", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load terminal hadron/Regge execution")
...
def experiment_registration() -> dict[str, object]:
    spec = HADRON_REGGE_EMPIRICAL_SPEC
    ...
    return {
        ...
        "dimension_unit_boundary": {
            ...
            "external_reference_protocol": "PDG MeV inscriptions remain external records; the formal theorem uses exact normalized fixed-carrier support.",
            "proof_value_policy": "positive-generated-counts-exact-fractions-held-labels-and-empty-form-only",
            "measurement_record_policy": "external-records-never-become-proof-scalars-formal-survivor-selectors-or-fitted-tensions",
        },
        "prediction_protocol": {
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("exact-hadron-Regge-comparison", spec.experiment_id, spec.falsification_condition)),
            "metrics": [{
                "metric_id": "complete-multiplet-and-five-state-trajectory-vector",
                "definition": "Compare both complete multiplet partitions and every registered spin, parity, mass, uncertainty, squared-mass step and listing-status row exactly.",
                "unit_protocol": "External MeV and GeV-squared inscriptions remain source records; all interval propagation uses exact positive fractions.",
                "all_rows": True,
            }],
            "acceptance_condition": "Both exact multiplet counts match, the complete trajectory remains ordered, the adverse empty common-step intersection is retained and no fitted correction is introduced.",
            "falsification_condition": spec.falsification_condition,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "Incomplete flavour, exchange or tube support is rejected."},
            {"control_id": "TAMPERED-SOURCE", "kind": "tampered_source", "expected_rejection": "Any changed source record, mass, uncertainty or status is rejected."},
            {"control_id": "TAMPERED-ARTIFACT", "kind": "tampered_artifact", "expected_rejection": "Changed prediction or trace is rejected."},
            {"control_id": "BOUNDARY", "kind": "boundary", "expected_rejection": "Target access, forbidden values or a fitted tension/residual is rejected."},
            {"control_id": "UNFAVORABLE-MEASUREMENT", "kind": "unfavorable_measurement", "expected_rejection": "Concealing the empty exact-step intersection or rho5 omission status fails."},
        ],
        "custody_protocol": {
        ...
        "stop_condition": "Halt after every multiplet, trajectory, interval, status and custody row is evaluated once, or immediately on any violation.",
...
def main() -> None:
    spec = HADRON_REGGE_SPEC
    empirical = HADRON_REGGE_EMPIRICAL_SPEC
    ...
    write_json(package / "registration.json", {
        ...
        "intended_certificate": "All 1,024 typed forms, one survivor, independent reconstruction, hostile controls and the complete PDG multiplet/five-state trajectory vector including adverse exact-spacing evidence.",
    ...
    why = [
        ...
        "Observation informed the explicit law; target-inaccessible enumeration and sealing precede release of the complete PDG vector.", "",
    ...
    why.extend((
        ...
        "## CHECK", "",
        "- Independent code reconstructs all ordered words, exchange classes and the exact affine successor without source access.",
        "- PDG independently records nine as eight plus one and twenty-seven as ten plus eight plus eight plus one.",
        "- Exact interval propagation retains every one of the five inherited natural-parity mass rows.",
        "- All five mass intervals rise with spin, but all four squared-mass step intervals have no common intersection; the old literal-exact measured reading is therefore closed as false, not hidden.",
        "- The exact normalized fixed-carrier theorem remains sealed, and no free correction is introduced.",
        "", "## EXCLUSIONS", "",
    ))
