#!/usr/bin/env python3
"""Admit all ten OPT claims sequentially through the untouched engine."""
...
MANIFEST_ID = "sha256:73a556f40d26940ece49ca60e593d96ae2b4370e1017dea6434cd0f2cebd3478"
VECTOR_ID = "sha256:644deb845d38c749f205cf4d9355e1314e986523fe3bd97d7159076b9b11f99e"
ADDENDUM_REGISTRY_ID = "sha256:1000c8841bce992c6b5ea41cf1303f5b6cb3ff6159c14fa9b302d9eaf8578408"
ADDENDUM_MANIFEST_ID = "sha256:0c5be1a28b12d7b29a2bd862abeebaef7267842498f63c5d4d11fc6d51e3998f"
...
def seals():
    for tool, expected in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or payload.get("status") != expected:
            raise SystemExit("Materials OPT admission halted: protected seal invalid")

def execution(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("materials_opt_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    if present:
        raise SystemExit("Materials OPT admission refuses replay: " + ", ".join(present))
    for index, claim_id in enumerate(ORDER, 1):
        ...
        if missing:
            raise SystemExit(f"Materials OPT admission halted: missing dependencies {claim_id}: {missing}")
        ...
        comparison_count = next(item["comparison_count"] for item in json.loads((ROOT / "experiments/external_sources/materials/opt_001_010_v1/complete_evidence_vector_v1.json").read_text())["claims"] if item["claim_id"] == claim_id)
        write_json(package / "certificate.json", {
            ...
            "source_addendum_registry_identity": ADDENDUM_REGISTRY_ID,
            "source_addendum_manifest_identity": ADDENDUM_MANIFEST_ID,
            "complete_evidence_vector_identity": VECTOR_ID,
            "captured_authoritative_source_count": 11,
