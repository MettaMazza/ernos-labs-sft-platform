#!/usr/bin/env python3
"""Admit and materialize the first same-strength Physics lineage batch."""
...
from sft.physics.lineage_particle_laws import LINEAGE_PARTICLE_SPECS  # noqa: E402
...
def load_execution(claim_id: str):
    ...
    definition = importlib.util.spec_from_file_location("sft_physics_lineage_" + claim_id.replace("-", "_"), path)
...
def main() -> None:
    ...
    receipts = {}
    for position, spec in enumerate(LINEAGE_PARTICLE_SPECS, 1):
        if spec.claim_id in existing:
            receipt = read_receipt(ROOT / existing[spec.claim_id]["receipt_path"])
            print(f"[{position}/{len(LINEAGE_PARTICLE_SPECS)}] retained {spec.claim_id}: {receipt.receipt_hash}")
        else:
            ...
            print(f"[{position}/{len(LINEAGE_PARTICLE_SPECS)}] admitted {spec.claim_id}: {receipt.receipt_hash}")
            census = json.loads(census_path.read_text(encoding="utf-8"))
            existing = {row["claim_id"]: row for row in census["claims"]}
    ...
    for position, spec in enumerate(LINEAGE_PARTICLE_SPECS, 1):
        ...
        (package / "STATUS.md").write_text(
            ...
            "- Empirical status: exact formal Physics result sealed; authoritative comparison is separate\n"
        ...
        print(f"[{position}/{len(LINEAGE_PARTICLE_SPECS)}] materialized {spec.claim_id}")
