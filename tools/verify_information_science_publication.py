#!/usr/bin/env python3
"""Build and verify the local Information Science paper evidence bundle.

This command is branch-specific and fail-closed. It checks the frozen inventory,
admitted receipts, claim evidence, standalone paper, rendered PDF and complete
evidence map. It never pushes, uploads or publishes anything.
"""
...
INVENTORY_PATH = ROOT / "publications/inventories/information_science.json"
CENSUS_PATH = ROOT / "census/claims.json"
PAPER_PATH = ROOT / "publications/current/information_science/FROM_DISTINCTION_TO_INFORMATION.md"
PDF_PATH = ROOT / "output/pdf/from-distinction-to-information-branch-paper-001-v1.2.pdf"
EVIDENCE_MAP_PATH = ROOT / "publications/current/information_science/evidence_map.json"
MANIFEST_PATH = ROOT / "publications/current/information_science/manifest.json"
PUBLICATION_RECEIPT_PATH = ROOT / "publications/current/information_science/publication_receipt.json"

PAPER_SECTIONS = {
    "SFT-INFO-SYMBOL-DISTINCTION-001": "6",
    "SFT-INFO-ENCODING-DECODING-001": "7",
    "SFT-INFO-QUANTITY-001": "8",
    "SFT-INFO-ENTROPY-UNCERTAINTY-001": "9",
    "SFT-INFO-COMPRESSION-REDUNDANCY-001": "10",
    "SFT-INFO-CHANNEL-CAPACITY-001": "11",
    "SFT-INFO-NOISE-ERROR-001": "12",
    "SFT-INFO-CODING-001": "13",
    "SFT-INFO-MUTUAL-CONDITIONAL-001": "14",
    "SFT-INFO-CONSERVATION-LOSS-001": "15",
    "SFT-INFO-CLASSICAL-PROBABILISTIC-001": "16",
    "SFT-INFO-QUANTUM-CORRESPONDENCE-001": "17",
}
...
def load_inventory() -> BranchInventory:
    ...
    if calculated != payload["inventory_hash"]:
        raise ValueError("information-science inventory identity does not match its content")
...
def build_evidence_map() -> dict[str, Any]:
    ...
    if tuple(census) != inventory.required_claim_ids:
        raise ValueError("information-science census order does not equal the frozen inventory")
    ...
    return {
        "schema": "sft-v3-information-science-paper-evidence-map/1",
        "branch_id": BRANCH_ID,
        "inventory": {
...
def evidence_map_controls(value: Mapping[str, Any], inventory: BranchInventory) -> None:
    ...
    if not value["publication_action_authorized"]:
        raise ValueError("authorized release must carry publication authorization")
...
def bundle_objects(evidence_map: Mapping[str, Any]):
    inventory = load_inventory()
    census = branch_census_entries()
    receipts = {
    ...
    return inventory, paper, receipt, manifest
...
def verify_written_bundle() -> dict[str, Any]:
    ...
    _, paper, receipt, expected_manifest = bundle_objects(actual_map)
    if read_json(MANIFEST_PATH) != expected_manifest:
        raise ValueError("information-science publication manifest is stale or altered")
    expected_receipt = json.loads(json.dumps(asdict(receipt)))
    if read_json(PUBLICATION_RECEIPT_PATH) != expected_receipt:
        raise ValueError("information-science publication receipt is stale or altered")
    return {
...
def build_bundle() -> dict[str, Any]:
    ...
    _, _, receipt, manifest = bundle_objects(evidence_map)
...
def main() -> int:
    result = build_bundle()
    print("SFT INFORMATION SCIENCE LOCAL PUBLICATION GATE: PASS")
