#!/usr/bin/env python3
"""Exact read-only replay of all twelve admitted EQN receipts."""
import json,sys
...
from sft.mathematics.eqn_001_012_laws_v1 import IDS as EQN
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("EQN replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in EQN);authority=AuthorityLedger()
 ...
 for cid in EQN:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("EQN exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(EQN)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
