#!/usr/bin/env python3
"""Exact read-only replay of all ten admitted ALEXT receipts."""
import json,sys
...
from sft.mathematics.alext_001_010_laws_v1 import IDS as ORDER
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("ALEXT replay halted: census and manifest differ")
 ...
 for cid in ORDER:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("ALEXT exact replay mismatch: "+cid)
