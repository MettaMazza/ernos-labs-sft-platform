#!/usr/bin/env python3
"""Exact read-only replay of the nine admitted Materials MICRO receipts."""
...
from sft.materials.micro_001_009_laws_v1 import ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("MICRO replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        index = indices[claim_id]; row = census[index]; execution = _load_execution(ROOT, manifest[index])
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("MICRO exact replay mismatch: " + claim_id)
        authority.admit(receipt); results.append({"claim_id": claim_id, "receipt_hash": receipt.receipt_hash, "exact_replay": True})
