#!/usr/bin/env python3
"""Exact read-only replay of all eight admitted Materials SURF receipts."""
import json
...
from sft.materials.surf_001_008_laws_v1 import ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("SURF replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("SURF exact replay mismatch: " + claim_id)
