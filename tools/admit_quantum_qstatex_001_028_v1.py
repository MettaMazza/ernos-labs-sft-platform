#!/usr/bin/env python3
"""Admit the complete QSTATEX-001 through QSTATEX-028 family sequentially."""
...
from sft.quantum_computation.qstatex_001_028_laws_v1 import IDS, SPECS
...
def seals():
    for tool, status in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Quantum QSTATEX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("qstatex_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "quantum_computation_obligation": f"SFT-QUANTUM-OBL-QSTATEX-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                "$schema": "../../governance/claim.schema.json", "branch": "quantum_computation", "claim_id": claim_id,
                "title": spec.title, "statement": spec.statement, "dependencies": list(spec.dependencies),
                "root_theorems": ["SFT-ROOT-THERE-IS-NO-NOTHING"], "axioms": [], "free_parameters": [],
                ...
                "pre_source_target_registry": "census/quantum_qstatex_001_028_target_registry_v1.json",
                "empirical_protocol": "post-registry exact quantum-state execution custody",
                "registered_by": "Maria Smith", "registration_date": "2026-07-29",
                "required_controls": ["false_premise", "tampered_source", "tampered_artifact", "boundary"], "status": "empirically_tested",
            },
        }
