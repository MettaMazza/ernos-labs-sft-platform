#!/usr/bin/env python3
"""Admit the dark/Smithion/LFV post-seal comparison through the sealed engine."""
...
from sft.physics.dark_smithion_lfv_empirical_v1 import CLAIM_ID, EXPERIMENT_ID, SOURCE_FILES, SOURCE_HASH, SOURCE_IDS, SOURCE_PATH, SPEC
from sft.physics.dark_smithion_lfv_validation_v1 import TARGET_IDS, source_hashes
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\ndark/Smithion/LFV empirical admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_dark_smithion_lfv_empirical_062", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load empirical execution")
...
def experiment_registration():
    ...
    return {
        ...
        "development_observations": [{"source_id": "V1-V2-DARK-SMITHION-LFV-CONTEXT", "role": "development_only", "content_hash": sha256_identity("prior-obligation-only")}],
        ...
        "inputs": [{"input_id": "DARK-SMITHION-LFV-FORMAL-RECEIPT", "value_kind": "admitted-formal-receipt", "content_hash": record["formal_receipt_hash"]}],
        "withheld_targets": [{"target_id": TARGET_IDS[0], "source_id": SOURCE_IDS[0], "content_withheld_from_prediction": True}],
        "dimension_unit_boundary": {"derived_dimension_carriers": ["27/5 abundance ratio", "four exact cubic spectra", "3:5:20 LFV relative weights"], "external_reference_protocol": "Density values, galaxy records, search limits and measurement status remain post-seal.", "proof_value_policy": "positive-generated-counts-and-exact-ratios-only", "measurement_record_policy": "upper-limits-and-unobserved-status-never-become-measured-proof-scalars"},
        ...
        "evaluation_protocol": {"evaluator_id": EXPERIMENT_ID + "-post-seal-evaluator", "comparison_implementation_hash": sha256_identity(("exact-dark-smithion-lfv-comparator/1", SPEC.falsification_condition)), "metrics": [{"metric_id": "DARK-DENSITY", "definition": "Exact ratio and absolute central transport inside complete Planck intervals.", "unit_protocol": "exact rational interval", "all_rows": True}, {"metric_id": "STANDING-TEST-CUSTODY", "definition": "Galaxy, LFV upper-limit and Smithion measurement-status rows retained without evidential relabelling.", "unit_protocol": "source-bound complete vector", "all_rows": True}], "acceptance_condition": "Every presently measurable comparison passes and every unmeasured prediction remains explicitly unmeasured.", "falsification_condition": SPEC.falsification_condition},
        "controls": [{"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "A law not descending from Claim 061 rejects."}, {"control_id": "TAMPERED-SOURCE", "kind": "tampered_source", "expected_rejection": "Any source or row change rejects."}, {"control_id": "TAMPERED-ARTIFACT", "kind": "tampered_artifact", "expected_rejection": "A changed census, trace or certificate rejects."}, {"control_id": "BOUNDARY", "kind": "boundary", "expected_rejection": "Target access, fitting or evidential relabelling rejects."}, {"control_id": "DISPLACED-MEASUREMENT", "kind": "unfavorable_measurement", "expected_rejection": "A displaced density or fabricated nonzero LFV rate rejects."}],
        ...
        "target_access_policy": "structurally-denied-before-seal", "row_retention_policy": "retain-every-registered-measurement-and-unmeasured-status-row", "stop_condition": "Halt without admission on changed evidence, failed measured comparison, omitted row, target access, fitting, evidential relabelling or failed control.",
...
def main():
    ...
    write_json(package / "registration.json", {"$schema": "../../governance/claim.schema.json", "branch": "physics", "candidate_grammar": {"boundary": SPEC.grammar_boundary, "completeness_certificate": "generated by the untouched admission engine from the complete declared product", "generator": SPEC.generation_rule}, "claim_id": CLAIM_ID, "dependencies": list(SPEC.dependencies), "excluded_inputs": list(SPEC.exclusions), "empirical_protocol": f"experiments/physics/{EXPERIMENT_ID}/registration.json", "intended_certificate": "Complete 256-form census, exact Planck comparison, SPARC discriminator, complete LFV/search-status custody and hostile controls.", "provenance_classes": ["observational_derivation"], "registered_by": "Maria Smith", "registration_date": "2026-07-25", "required_controls": ["false_premise", "tampered_source", "tampered_artifact", "boundary", "unfavorable_measurement"], "statement": SPEC.exact_result, "status": "empirically_tested", "title": SPEC.title})
    ...
    (package / "STATUS.md").write_text("\n".join((f"# {CLAIM_ID}", "", "Status: `empirically_tested_and_independently_replicated`", "", "- `27/5` and absolute transport `0.12096` pass the complete Planck comparisons.", "- SPARC's 175-galaxy record requires additional gravitating support under the derived inverse-square law.", "- All three LFV rows are null-search upper limits, not measured relative rates.", "- All twelve Smithion ratios remain exact standing predictions, not fabricated measurements.", "- Tampered-density and evidential-relabelling controls reject.", f"- Closure: `{certificate['closure_scope']}`", f"- Post-seal empirical validation: `{certificate['empirical_validation_hash']}`", f"- Engine receipt: `{row['receipt_hash']}`", "")), encoding="utf-8")
