...
PREVIOUS = ROOT / "census/information_science_discipline_current_reconciliation_v14.json"
OUT = ROOT / "census/information_science_discipline_current_reconciliation_v15.json"
AUDIT = ROOT / "audits/INFORMATION_SCIENCE_PRIV_001_010_COMPLETION_2026-07-29.json"

from sft.information_science.priv_001_010_laws_v1 import IDS
...
def main():
    ...
    if canonical(frozen_body) != frozen_identity or canonical(previous_body) != previous_identity or previous["current_closed_count"] != 196:
        raise SystemExit("PRIV reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        obligation = f"SFT-INFO-OBL-PRIV-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["information_science_obligation"] != obligation:
            raise SystemExit("PRIV reconciliation halt: " + claim_id)
        rows.append({
    ...
    families["PRIV"] = rows
    value = {
        "schema": "sft-v3-information-science-discipline-current-reconciliation/15",
        "date": "2026-07-29",
        ...
        "current_closed_count": 206,
        "current_open_count": 56,
        "current_completion_fraction": "206/262",
        "current_completion_percent": "78.6%",
    ...
    audit = {
        "schema": "sft-v3-information-science-priv-completion/1",
        "date": "2026-07-29",
        "family": "PRIV-001--010",
        ...
        "current_information_science_progress": "206/262",
        "current_information_science_percent": "78.6%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 206, "open": 56, "percent": "78.6%", "identity": value["reconciliation_identity"]}, indent=2))
