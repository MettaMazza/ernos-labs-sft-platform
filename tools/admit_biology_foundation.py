#!/usr/bin/env python3
"""Admit and materialize the 75 foundational Biology laws in order."""
...
from sft.biology.generated_law import BIOLOGY_SPECS, PRE_SOURCE_SEAL_PATH  # noqa: E402
from sft.engine import EngineRepository  # noqa: E402
from tools.scaffold_biology_foundation_claims import EXACT_COUNT_CLAIMS  # noqa: E402


CHECKPOINT = ROOT / "census/biology_continuation_checkpoint.json"


def load_execution(claim_id: str):
    path = ROOT / "claims" / claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_biology_" + claim_id.replace("-", "_"), path)
...
def update_checkpoint(last_claim_id: str | None, receipt_hash: str | None, admitted_count: int, next_operation: str, status: str = "in_progress") -> None:
    write_json(CHECKPOINT, {
        "schema": "sft-v3-biology-continuation-checkpoint/1",
        "branch": "biology",
        "foundation_required_claim_count": len(BIOLOGY_SPECS),
        "admitted_claim_count": admitted_count,
        "remaining_claim_count": len(BIOLOGY_SPECS) - admitted_count,
...
def materialize(spec, execution, receipt, captured) -> None:
    ...
    experiment_path = ROOT / "experiments/biology" / spec.experiment_id / "registration.json"
    ...
    (package / "STATUS.md").write_text(
        ...
        f"- Post-seal biological correspondence: `{certificate['empirical_validation_hash']}`\n"
        ...
        f"- Exact structural value validated: `{exact_value or 'conditional biological relation; no universal specimen magnitude'}`\n"
...
def main() -> None:
    ...
    ordered_ids = [row.claim_id for row in BIOLOGY_SPECS]
    prefix = 0
    ...
    if any(claim_id in already for claim_id in ordered_ids[prefix + 1:]):
        raise RuntimeError("Biology receipts are not a continuous dependency prefix")
    ...
    for index, spec in enumerate(BIOLOGY_SPECS[prefix:], prefix + 1):
        ...
        print(f"[{index}/{len(BIOLOGY_SPECS)}] admitted {spec.claim_id}: {receipt.receipt_hash}", flush=True)
