#!/usr/bin/env python3
"""Exact read-only replay of all ten admitted Materials NANO receipts."""
...
from sft.materials.nano_001_010_laws_v1 import ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("NANO replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("NANO exact replay mismatch: " + claim_id)
