#!/usr/bin/env python3
"""Admit and materialize the atomic-spectrum post-seal comparison group."""
...
from sft.physics.atomic_spectra_postseal_validation_v1 import VALIDATION_SPECS  # noqa: E402


def load_execution(claim_id: str):
    path = ROOT / "claims" / claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_atomic_postseal_" + claim_id.replace("-", "_"), path)
...
def main() -> None:
    ...
    for index, spec in enumerate(VALIDATION_SPECS, 1):
        ...
        print(f"[{index}/{len(VALIDATION_SPECS)}] admitted {spec.claim_id}: {receipt.receipt_hash}")
    ...
    for index, spec in enumerate(VALIDATION_SPECS, 1):
        ...
        (package / "STATUS.md").write_text(
            ...
            f"- Post-seal external comparison: `{certificate['empirical_validation_hash']}`\n"
        ...
        print(f"[{index}/{len(VALIDATION_SPECS)}] materialized {spec.claim_id}")
