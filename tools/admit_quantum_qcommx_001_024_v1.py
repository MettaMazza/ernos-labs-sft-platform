#!/usr/bin/env python3
"""Admit QCOMMX-001 through QCOMMX-024 sequentially."""
...
def seals():
    for tool, status in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or data.get("status") != status: raise SystemExit("Quantum QCOMMX admission halted: protected seal invalid")
def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"; module_spec = importlib.util.spec_from_file_location("qcommx_" + claim_id.replace("-", "_"), path); module = importlib.util.module_from_spec(module_spec); module_spec.loader.exec_module(module); return module.build_execution(ROOT)
def main():
    from sft.engine import EngineRepository
    seals(); claims_path, manifest_path = ROOT / "census/claims.json", ROOT / "census/execution_manifest.json"; existing = {row["claim_id"] for row in json.loads(claims_path.read_text())["claims"]}
