#!/usr/bin/env python3
"""Submit exactly one NUCHEM-009–012 claim per invocation in dependency order."""
...
from sft.chemistry.nuchem_terminal_batch_v1 import FISSION_PRODUCT_SPEC, RADIOLYSIS_SPEC, RADIOTRACER_SPEC, SEPARATION_SPEC
from sft.chemistry.nuchem_terminal_validation_v1 import exact_analysis
from sft.engine import EngineRepository


CONFIG = {
    "009": (RADIOTRACER_SPEC, "SFT-CHEM-OBL-NUCHEM-009", "radiotracer_custody_receipt.json", "model_admitted_forced_radiotracer_custody_inference_law_empirically_tested_and_independently_replicated", "eight tracer-chemistry rows, detector and residence-time support, 70–75% cobalt recovery, about 5% short circuit and nearly 30% blocked volume"),
    "010": (SEPARATION_SPEC, "SFT-CHEM-OBL-NUCHEM-010", "radiochemical_separation_receipt.json", "model_admitted_forced_radiochemical_separation_decontamination_law_empirically_tested_and_independently_replicated", "fourteen species, nine processes, 92% and 71% recoveries, no-detect scandium and the adverse 1% run"),
    "011": (FISSION_PRODUCT_SPEC, "SFT-CHEM-OBL-NUCHEM-011", "fission_product_distribution_receipt.json", "model_admitted_forced_fission_product_chemical_distribution_law_empirically_tested_and_independently_replicated", "complete chemical groups, five phase/location and five sample supports, 45–71% iodine inventory and all unresolved balances"),
    "012": (RADIOLYSIS_SPEC, "SFT-CHEM-OBL-NUCHEM-012", "radiolysis_network_receipt.json", "model_admitted_forced_radiation_chemistry_reaction_network_law_empirically_tested_and_independently_replicated", "six products, four reactions, four preferred yields, two high-dose rows, five rare-gas rows and twelve adverse/limit rows"),
}
