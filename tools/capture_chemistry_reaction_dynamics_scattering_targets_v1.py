#!/usr/bin/env python3
"""Open the complete KIN-013 target surface after identity registration."""
...
OUTPUT = ROOT / "experiments/external_sources/chemistry/reaction_dynamics_scattering_withheld_targets_v1.json"
...
def main() -> None:
    ...
    pdf_text = {
        ...
        for name in ("article.pdf", "supplementary-information.pdf", "transparent-peer-review.pdf")
    }
    ...
    for identity in identity_document["rows"]:
        ...
        if record_class == "complete-article-landing-record":
            ...
            raise ValueError(f"unknown KIN-013 source record class: {record_class}")
        rows.append(identity | {"target_payload": target})
    payload = {
        "schema": "sft-v3-reaction-dynamics-scattering-withheld-targets/1",
