#!/usr/bin/env python3
"""Admit and materialize expanded scientific calculator claim 005 only."""
...
from sft.mathematics.calculator.law_v3 import SPEC  # noqa: E402
...
def verify_engine() -> None:
    ...
    if actual != ENGINE_TREE or dirty != 0:
        raise SystemExit("frozen engine identity changed; expanded calculator admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_math_calculator_v3_admission", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load expanded calculator execution")
...
def registration() -> dict[str, object]:
    return {
        ...
        "intended_certificate": (
            "Complete 8,192-form semantic census; core, scientific-function, angle, session, terminal and desktop "
            "witnesses; four adverse controls; and implementation-distinct regeneration."
        ),
        ...
        "extends": "SFT-MATH-SCIENTIFIC-CALCULATOR-004",
        "title": SPEC.title,
    }
...
def main() -> None:
    ...
    (package / "WHY_DERIVATION_CHECK.md").write_text(
        ...
        "Claim 005 does not replace exact SFT mathematics with a familiar-looking interface. It extends the admitted "
        "004 core, exhausts the three additional coverage/state/application choices and requires every terminal or "
        "desktop action to invoke the same exact evaluator.\n\n"
        "## WHY\n\n" + SPEC.why + "\n\n## DERIVATION\n\n" + SPEC.derivation + "\n\n## CHECK\n\n" + SPEC.check + "\n",
        encoding="utf-8",
    )
    (package / "STATUS.md").write_text(
        "\n".join((
            f"# {CLAIM_ID}", "", "Status: `independently_replicated`", "",
            "- Active full calculator/app law; extends the corrected exact core 004.",
            "- Ordinary use: `1+1=` returns exact forward-held two.",
            "- Scientific surface: powers, roots, circular/inverse/hyperbolic, logs, exponentials, combinatorics and statistics.",
            "- Interaction: RAD/DEG/GRAD, Ans, memory and ordered history.",
            "- App: editable display, complete keypad, history, exact detail, trace and embedded SFT guide.",
            "- Runtime: Python standard library only; terminal and Tk desktop views share one evaluator.",
