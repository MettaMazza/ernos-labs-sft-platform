#!/usr/bin/env python3
"""Admit the complete FORMX-001 through FORMX-022 family sequentially."""
...
from sft.computation.formx_001_022_laws_v1 import IDS, SPECS
...
def seals():
    for tool, status in (
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Classical Computation FORMX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("formx_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "classical_computation_obligation": f"SFT-COMP-OBL-FORMX-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/computation_formx_001_022_target_registry_v1.json",
                "empirical_protocol": "post-registry exact formal-computation execution custody",
