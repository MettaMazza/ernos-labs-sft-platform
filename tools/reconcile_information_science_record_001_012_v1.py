...
PREVIOUS = ROOT / "census/information_science_discipline_current_reconciliation_v2.json"
OUT = ROOT / "census/information_science_discipline_current_reconciliation_v3.json"
AUDIT = ROOT / "audits/INFORMATION_SCIENCE_RECORD_001_012_COMPLETION_2026-07-29.json"

from sft.information_science.record_001_012_laws_v1 import IDS
...
def main():
    ...
    if canonical(frozen_body) != frozen_identity or canonical(previous_body) != previous_identity or previous["current_closed_count"] != 26:
        raise SystemExit("RECORD reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        obligation = f"SFT-INFO-OBL-RECORD-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["information_science_obligation"] != obligation:
            raise SystemExit("RECORD reconciliation halt: " + claim_id)
        rows.append({
    ...
    families["RECORD"] = rows
    value = {
        "schema": "sft-v3-information-science-discipline-current-reconciliation/3",
        "date": "2026-07-29",
        ...
        "current_closed_count": 38,
        "current_open_count": 224,
        "current_completion_fraction": "38/262",
        "current_completion_percent": "14.5%",
    ...
    audit = {
        "schema": "sft-v3-information-science-record-completion/1",
        "date": "2026-07-29",
        "family": "RECORD-001--012",
        ...
        "current_information_science_progress": "38/262",
        "current_information_science_percent": "14.5%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 38, "open": 224, "percent": "14.5%", "identity": value["reconciliation_identity"]}, indent=2))
