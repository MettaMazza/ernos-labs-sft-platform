#!/usr/bin/env python3
"""Exact read-only replay of all ten admitted PRIV receipts."""
import json
import sys
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("PRIV replay halted: census and manifest differ")
    ...
    for claim_id in IDS:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("PRIV exact replay mismatch: " + claim_id)
