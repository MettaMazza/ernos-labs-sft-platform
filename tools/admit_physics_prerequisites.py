"""Execute, admit and materialize the three Physics empirical prerequisites."""
...
def load_execution(claim_id: str):
    ...
    module_spec = importlib.util.spec_from_file_location("sft_physics_prerequisite_" + claim_id.replace("-", "_"), path)
...
def main() -> None:
    ...
    existing = {row["claim_id"] for row in manifest["claims"]}
    for spec in PREREQUISITE_SPECS:
    ...
    for spec in PREREQUISITE_SPECS:
        ...
        (package / "STATUS.md").write_text(
            ...
            f"- Engine receipt: `{census_row['receipt_hash']}`\n",
            encoding="utf-8",
        )
