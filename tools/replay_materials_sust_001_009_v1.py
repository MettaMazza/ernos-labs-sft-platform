#!/usr/bin/env python3
"""Exact read-only replay of all nine admitted Materials SUST receipts."""
...
from sft.materials.sust_001_009_laws_v1 import ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("SUST replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("SUST exact replay mismatch: " + claim_id)
