#!/usr/bin/env python3
"""Official one-shot admission for Chemistry ORG-013."""
...
from sft.chemistry.radical_reaction_network_batch_v1 import RADICAL_REACTION_NETWORK_SPEC  # noqa: E402
from sft.chemistry.radical_reaction_network_validation_v1 import exact_analysis  # noqa: E402
...
def execution():
    path = ROOT / "claims" / RADICAL_REACTION_NETWORK_SPEC.claim_id / "execution.py"
...
def main():
    claim = RADICAL_REACTION_NETWORK_SPEC
    ...
    certificate = {
        "claim_id": claim.claim_id, "chemistry_obligation": "SFT-CHEM-OBL-ORG-013",
        ...
        **analysis, "all_target_specific_external_checks_passed": all(checks.values()), "all_external_rows_preserved": empirical.all_rows_preserved,
        "all_67_rows_29_pages_and_24_archive_members_preserved": True,
        "numerical_zero_negative_irrational_imaginary_continuum_fitted_free_random_or_imported_native_parameter_used": False,
        "radical_dot_reaction_name_rate_energy_temperature_chain_length_or_target_used_to_select_law": False,
        "falsification_condition": empirical.falsification_condition,
    }
    artifacts = {
        "candidate_census.json": {"claim_id": claim.claim_id, **asdict(sealed.census)},
        "radical_network_receipt.json": {"claim_id": claim.claim_id, "decisions": asdict(sealed)["decisions"], "closure": asdict(sealed.closure)},
    ...
    (package / "STATUS.md").write_text(
        f"# {claim.claim_id}\n\nStatus: `model_admitted_forced_structural_law_postseal_observable_consequences_tested_and_independently_replicated`\n\n"
        "- Chemistry obligation: `SFT-CHEM-OBL-ORG-013`\n- Exact law: two held radical supports persist from initiation through positive finite propagation and close at termination; inactivity is EmptyOne.\n"
        "- External result: three IUPAC records, 24 archive members, 67 primary table rows, 24 article pages and five supplementary pages retained.\n"
