#!/usr/bin/env python3
"""Submit finite symmetry/conservation and least action to the frozen engine."""
...
from sft.physics.symmetry_action_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_symmetry_action", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load symmetry/action execution")
...
def main() -> None:
    ...
    receipt = EngineRepository(ROOT).execute_official(
        execution.program, execution.independent_validator, execution.source_files
    )
    ...
    row = next(
        item
        for item in json.loads(census_path.read_text(encoding="utf-8"))["claims"]
    ...
    (package / "STATUS.md").write_text(
        "\n".join(
            (
                ...
                "- Empirical status: exact finite structural correspondence; system-specific physical applications remain separately testable",
