...
from sft.mathematics.prob_001_016_laws_v1 import IDS as PROB,SPECS
...
def seals():
 for tool,status in (("verify_engine_seal.py","VALID_CANONICAL_ENGINE"),("verify_verification_authority_seal.py","VALID_CANONICAL_VERIFICATION_AUTHORITY")):
  ...
  if result.returncode or data.get("status")!=status:raise SystemExit("Mathematics PROB admission halted: protected seal invalid")
def load(cid):
 path=ROOT/"claims"/cid/"execution.py";spec=importlib.util.spec_from_file_location("prob_"+cid.replace("-","_"),path);module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);return module.build_execution(ROOT)
def main():
 ...
 for index,cid in enumerate(PROB,1):
  ...
  seals();existing.add(cid);print(f"[{index}/{len(PROB)}] admitted {cid}: {receipt.receipt_hash}",flush=True)
