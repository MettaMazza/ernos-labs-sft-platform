#!/usr/bin/env python3
"""Admit the complete Physics empirical Grand Lock through the untouched engine."""
...
def verify_engine() -> None:
    ...
    if result.returncode:
        raise SystemExit(result.stdout + result.stderr + "\nempirical Grand Lock admission halted")
...
def load_execution(claim_id: str):
    ...
    definition = importlib.util.spec_from_file_location("sft_physics_grand_lock_076", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load empirical Grand Lock execution")
...
def main() -> None:
    ...
    from sft.physics.grand_lock_empirical_v1 import CLAIM_ID, SPEC
    ...
    if missing:
        raise SystemExit("empirical Grand Lock dependencies are not admitted: " + ", ".join(missing))
    ...
    write_json(package / "registration.json", {
        ...
        "intended_certificate": "Complete 256-form reconciliation of every empirical receipt, source identity and adverse/scope record.",
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "- All 234 pre-lock empirical Physics claims are retained in one immutable vector.",
        "- All 147 distinct registered external source identities remain bound.",
        "- Every validation hash and every available separate measurement receipt remains bound.",
        "- Six older receipt shapes are disclosed exactly; none is silently upgraded.",
        "- All fourteen detected unfavorable-result or scope-boundary claims remain retained.",
        "- Observation remains empirical evidence and never selects a formal survivor.",
