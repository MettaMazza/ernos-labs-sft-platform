#!/usr/bin/env python3
"""Admit the complete VALID-001 through VALID-012 family sequentially."""
...
from sft.computation.valid_001_012_laws_v1 import IDS, SPECS
...
def seals():
    for tool, status in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Classical Computation VALID admission halted: protected seal invalid")
...
def main():
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "classical_computation_obligation": f"SFT-COMP-OBL-VALID-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/computation_valid_001_012_target_registry_v1.json",
                "empirical_protocol": "post-registry exact validation execution custody",
