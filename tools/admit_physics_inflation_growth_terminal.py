#!/usr/bin/env python3
"""Submit the inflation-growth terminal law to the canonical engine."""
...
from sft.physics.inflation_growth_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def verify_engine() -> None:
    ...
    if completed.returncode != 0 or json.loads(completed.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(completed.stdout + completed.stderr + "\ninflation-growth admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_inflation_growth_terminal", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load inflation-growth execution")
...
def registration() -> dict[str, object]:
    return {
        ...
        "intended_certificate": (
            "Complete 256-form engine census; exact least binary cover of generator-three space; exact primordial "
            "support partition; two-step perturbation trace; depth-independent component transport; implementation-"
            "distinct reconstruction; and all hostile controls."
        ),
...
def main() -> None:
    ...
    (package / "STATUS.md").write_text("\n".join((
        f"# {CLAIM_ID}", "",
        "Status: `independently_replicated_formal_empirical_comparison_separate`", "",
        "- Generator-three stable space volume 27 is first covered at binary depth five, support 32.",
        "- The complete support partition is exactly 31/32 scalar and 1/32 least tensor-pair.",
        "- The least perturbation grows exactly 1/4 to 1/2 to One.",
        "- Matter/radiation relative support grows by exactly the later scale carrier g.",
        "- Conventional logarithmic e-fold counts and observational values do not enter formal selection.",
