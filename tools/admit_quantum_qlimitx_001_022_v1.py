#!/usr/bin/env python3
"""Sequential untouched-engine admission for the complete QLIMITX family."""
...
def seals():
    ...
    for tool, status in expected:
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Quantum QLIMITX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("qlimitx_" + claim_id.replace("-", "_"), path)
...
The newest admitted claim is `{claim_id}`, receipt `{receipt_hash}`. QLIMITX is
...
{"Admit the next QLIMITX claim in frozen dependency order." if index < 22 else "Reconcile the completed QLIMITX family into reconciliation v11; do not resubmit it."}
...
def main():
    ...
    from sft.quantum_computation.qlimitx_001_022_laws_v1 import IDS, SPECS
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "quantum_computation_obligation": f"SFT-QUANTUM-OBL-QLIMITX-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/quantum_qlimitx_001_022_target_registry_v1.json",
                "empirical_protocol": "post-registry exhaustive classical-quantum correspondence/limit execution and explicit Physics/device handoff",
