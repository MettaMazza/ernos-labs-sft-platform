#!/usr/bin/env python3
"""Admit and materialize terminal atomic-precision successors."""
...
from sft.physics.atomic_precision_successor_laws_v1 import ATOMIC_PRECISION_SPECS  # noqa: E402
from sft.physics.atomic_precision_successor_validation_v1 import (  # noqa: E402
...
def load_execution(claim_id: str):
    ...
    definition = importlib.util.spec_from_file_location(f"sft_atomic_precision_{claim_id}", path)
    if definition is None or definition.loader is None:
        raise RuntimeError(f"cannot load atomic precision execution: {claim_id}")
    ...
    return {
        ...
        "dimension_unit_boundary": {
            ...
            "external_reference_protocol": (
                "Authoritative NIST snapshots are hash-bound in " + SOURCE_HASH
                + "; observation informed law formation, while the execution surface cannot access the Rydberg carrier or comparison target before sealing."
            ),
        ...
        "evaluation_protocol": {
            ...
            "metrics": [{
                ...
                "definition": "Verify every source hash, translate the immutable exact dimensionless relation with the registered Rydberg carrier, retain every interval endpoint and compare after seal release.",
                "unit_protocol": "External hertz inscriptions remain source records; only exact positive rational arithmetic enters the evaluator.",
                "all_rows": True,
            }],
...
def main() -> None:
    ...
    for spec in ATOMIC_PRECISION_SPECS:
        ...
        empirical = EMPIRICAL_SPEC_BY_ID[spec.claim_id]
        ...
        registration = {
            ...
            "intended_certificate": (
                f"One engine receipt requiring all {candidate_census['expected_cardinality']:,} typed forms, "
                "one survivor, implementation-distinct exact reconstruction and complete hostile controls, "
                "followed by capability-gated release of the complete NIST measurement interval."
            ),
        ...
        why = [
            ...
            spec.statement,
            "",
            "This uses the observational-derivation empirical prediction protocol. Observation informs the explicit frozen law; the target is then placed behind the capability boundary; target-inaccessible engine execution enumerates every form, retains the sole survivor and seals the exact prediction; only then are the complete registered NIST value, unit and uncertainty released for comparison.",
            "",
            "## DERIVATION",
            "",
        ...
        for axis in spec.axes:
            choice = axis.survivor
            why.append(f"- `{axis.key}`: `{choice.name}` — {choice.reason}")
        why.extend((
            "",
        ...
        why.extend(f"- `{witness.name}`: {witness.statement}" for witness in spec.witnesses)
        why.extend((
            "",
            f"The engine regenerates all {candidate_census['expected_cardinality']:,} forms, retains one survivor, passes four hostile controls and requires an implementation-distinct exact reconstruction. The NIST measurement capability opens only after the derivation seal.",
            "",
            "## EXCLUSIONS",
            "",
        ))
