#!/usr/bin/env python3
"""Submit the post-seal spin-statistics and condensation empirical successor."""
...
from sft.physics.spin_statistics_condensation_empirical_v1 import (  # noqa: E402
    CLAIM_ID,
    EXPERIMENT_ID,
    SOURCE_FILES,
    SOURCE_HASH,
...
from sft.physics.spin_statistics_condensation_validation_v1 import TARGET_IDS  # noqa: E402
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\nspin-statistics empirical admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_spin_statistics_condensation_empirical", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load spin-statistics empirical execution")
...
def experiment_registration():
    ...
    return {
        ...
        "development_observations": [
            ...
            "Formal Claim 045, its complete 256-form census, and its official engine receipt preceded the bound target vector.",
            "The apparatus-specific BEC scale and Pauli collision factor remain measured records rather than universal formal proof scalars.",
        ],
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("exact-spin-statistics-condensation-comparator/1", SPEC.falsification_condition)),
            "acceptance_condition": "Finite BEC occupation and cooling direction pass; direct Pauli blocking passes; the complete neutron-spinor interval contains 720 degrees; all scale limitations and adverse controls are retained.",
        ...
        "controls": [
            {
                ...
                "kind": kind,
                "expected_rejection": "Any premise, source, artifact, boundary, cooling-direction or spinor-interval violation halts.",
            }
        ...
        "row_retention_policy": "retain all three sources, the complete BEC and spinor intervals, every direction and response row, finite-population correction and all scale limitations",
...
def claim_registration():
    return {
        ...
        "intended_certificate": "Complete 256-form census, independent reconstruction, capability-closed custody, complete BEC/Pauli/spinor vector, exact interval comparison, scale limitations and adverse-control rejection.",
    ...
    (package / "STATUS.md").write_text(
        "\n".join(
            (
                ...
                "Status: `empirically_tested_and_independently_replicated`",
                "",
                "- Finite Bose-gas ground occupation, cooling direction and sharp transition pass.",
                "- Direct two-spin-state Pauli blocking passes.",
                "- The measured neutron-spinor interval `[666,742]` degrees contains the sealed two-turn `720`-degree return.",
                "- Apparatus-specific BEC and collision-response magnitudes remain external scale records, not law selectors.",
