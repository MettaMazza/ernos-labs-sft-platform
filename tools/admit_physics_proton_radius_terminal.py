#!/usr/bin/env python3
"""Admit and materialize the terminal probe-independent proton-radius law."""
...
from sft.physics.proton_radius_terminal_law_v1 import CLAIM_ID, EXPERIMENT_ID, SPEC  # noqa: E402
from sft.physics.proton_radius_terminal_validation_v1 import (  # noqa: E402
...
def verify_engine() -> None:
    ...
    if actual != ENGINE_TREE or dirty != 0:
        raise SystemExit("frozen engine identity changed; proton-radius admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_proton_radius_terminal", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load proton-radius execution")
...
def experiment_registration() -> dict[str, object]:
    ...
    source_record = json.loads(
        (ROOT / "experiments/external_sources/physics/snapshots/proton-radius-terminal-source-record.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    return {
        ...
        "development_observations": [
            "The V1/V2 proton-radius proposal and the radius puzzle were known before this reconstruction; no historical-blindness claim is made.",
            "The measured rows cannot select the ten-cell linear survivor because the eight-, nine- and ten-cell linear controls all overlap the current muonic interval.",
        ],
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(
                ("exact-proton-radius-terminal-comparator/1", FALSIFICATION_CONDITION)
            ),
            "acceptance_condition": (
                "The exact propagated interval lies in every registered current interval, the historical conflict "
                "and all structural alternatives remain, target access is post-seal, and the tampered target fails."
            ),
            "falsification_condition": FALSIFICATION_CONDITION,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "A named proton without admitted composition is rejected."},
            ...
            {"control_id": "BOUNDARY", "kind": "boundary", "expected_rejection": "Early target access, fitted length or probe correction is rejected."},
            {"control_id": "UNFAVORABLE-MEASUREMENT", "kind": "unfavorable_measurement", "expected_rejection": "A disjoint electronic-hydrogen interval rejects the correspondence."},
        ],
        "custody_protocol": {
...
def claim_registration() -> dict[str, object]:
    return {
        ...
        "intended_certificate": (
            ...
            "complete source custody, exact interval propagation and adverse-formula comparison."
        ),
...
def main() -> None:
    ...
    (package / "STATUS.md").write_text(
        "\n".join((
            ...
            "- Exact coefficient: `10069574419808/2519231977345 = 4(One-alpha_terminal/10)`.",
            "- Post-seal reduced-Compton translation: approximately `0.840621761 fm`.",
            "- The exact interval lies in all four registered current intervals.",
            "- Historical CODATA-2014 conflict and current extraction limitations remain explicit.",
            "- Measurement does not select ten cells: eight-, nine- and ten-cell linear alternatives overlap the current muonic interval.",
