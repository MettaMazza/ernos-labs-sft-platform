#!/usr/bin/env python3
"""Execute and materialize the Chemistry rovibronic-composition claim."""
...
from sft.chemistry.rovibronic_composition_batch import ROVIBRONIC_COMPOSITION_SPEC  # noqa: E402
...
def main() -> None:
    ...
    if not receipt.model_admitted:
        raise SystemExit(
            f"claim rejected at {receipt.halted_stage}; preserved receipt {receipt.receipt_hash}; "
            "a scientifically distinct successor is required before another attempt"
        )
    ...
    payloads = {
        ...
        "certificate.json": {
            ...
            "status": "model_admitted_observationally_derived_and_independently_replicated",
            ...
            "external_evidence_class": "exact_H2_D2_ratio_vector_plus_structural_rovibronic_correspondence",
            "absolute_molecular_property_claimed": False,
    ...
    (package / "STATUS.md").write_text(
        ...
        f"- Post-seal external validation: `{receipt.empirical_validation_hash}`\n"
        f"- Measurement receipt: `{empirical.measurement_receipt_hash}`\n"
        f"- Engine receipt: `{receipt.receipt_hash}`\n"
        "- Scope: nuclear-electronic and vibronic/rovibronic composition with the complete registered H2/D2 ratio vector; no absolute bond length or absolute frequency claim.\n",
        encoding="utf-8",
    )
    print(f"admitted {spec.claim_id}: {receipt.receipt_hash}")
    print(f"candidates: {len(sealed.census.candidates)}")
    print(f"external rows: {len(empirical.measurements)}")
