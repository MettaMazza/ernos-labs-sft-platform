#!/usr/bin/env python3
"""Exact read-only replay of all fourteen admitted SYMREP receipts."""
import json
import sys
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("SYMREP replay halted: census and manifest differ")
    ...
    for claim_id in IDS:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("SYMREP exact replay mismatch: " + claim_id)
