#!/usr/bin/env python3
"""Seal value-free KIN-012 source identities before target extraction."""
...
OUTPUT = ROOT / "experiments/external_sources/chemistry/kinetic_isotope_effect_target_identities_v1.json"
...
def common(document: str, record_class: str, record_identity: str) -> dict:
    return {
        ...
        "isotopologue_reaction_system_identity": SYSTEM,
...
def main() -> None:
    ...
    pdf_records = (
        ...
        ("reporting-summary.pdf", "complete-reporting-summary-page"),
    )
    page_counts = {}
    ...
    rows = tuple(
        row | {
            "target_id": f"SFT-CHEM-KIN012-COMPLETE-SOURCE-RECORD-{ordinal:03d}",
            "source_record_ordinal": ordinal,
        }
        for ordinal, row in enumerate(rows, start=1)
    )
    payload = {
        "schema": "sft-v3-kinetic-isotope-effect-target-identities/1",
        ...
        "complete_reporting_summary_page_count": page_counts["reporting-summary.pdf"],
        "complete_source_data_worksheet_count": len(sheet_records),
        "target_values_or_hashes_present": False,
        "all_isotopologue_rate_ratio_decay_production_condition_uncertainty_replicate_figure_table_workbook_cell_status_value_and_target_hash_values_absent": True,
        "rows": rows,
    }
