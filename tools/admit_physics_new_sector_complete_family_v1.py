#!/usr/bin/env python3
"""Admit one formal new-sector family member through the frozen engine."""
...
def verify_seals() -> None:
    for tool, expected in (
        ...
        if completed.returncode or json.loads(completed.stdout).get("status") != expected:
            raise SystemExit(completed.stdout + completed.stderr + "\nNew-sector admission halted")


def load_execution(claim_id: str):
    path = ROOT / "claims" / claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_new_sector_submission", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load new-sector execution")
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: admit_physics_new_sector_complete_family_v1.py CLAIM_ID")
    claim_id = sys.argv[1]
    verify_seals()
    ...
    if claim_id not in SPECS:
        raise SystemExit(f"unregistered new-sector claim: {claim_id}")
    spec = SPECS[claim_id]
    ...
    if missing:
        raise SystemExit("New-sector dependencies absent: " + ", ".join(missing))
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "- No search outcome, measured value, conventional force model or desired particle list entered the formal survivor decision.",
