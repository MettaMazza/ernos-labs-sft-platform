...
from sft.information_science.therm_001_010_laws_v1 import IDS, SPECS
...
def seals():
    for tool, status in (
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Information Science THERM admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("therm_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "information_science_obligation": f"SFT-INFO-OBL-THERM-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/information_science_therm_001_010_target_registry_v1.json",
                "empirical_protocol": "post-registry exact therm-observation custody",
