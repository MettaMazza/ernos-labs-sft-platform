#!/usr/bin/env python3
"""Reconcile the completed NANO family into the frozen Materials census."""
...
PREVIOUS = ROOT / "census/materials_discipline_current_reconciliation_v11.json"
OUT = ROOT / "census/materials_discipline_current_reconciliation_v12.json"
AUDIT = ROOT / "audits/MATERIALS_NANO_001_010_COMPLETION_2026-07-29.json"
CLAIMS = (
    "SFT-MAT-NANO-SIZE-SHAPE-DISTRIBUTION-001",
    "SFT-MAT-NANO-NANOWIRE-CONFINEMENT-002",
    "SFT-MAT-NANO-LAYER-STACKING-003",
    "SFT-MAT-NANO-QUANTUM-DOT-CONFINEMENT-004",
    "SFT-MAT-NANO-SURFACE-VOLUME-DOMINANCE-005",
    "SFT-MAT-NANO-PHASE-MELTING-BOUNDARY-006",
    "SFT-MAT-NANO-QUANTUM-COLLECTIVE-STATE-007",
    "SFT-MAT-NANO-MOIRE-SUPERSTRUCTURE-008",
    "SFT-MAT-NANO-NANOCOMPOSITE-INTERFACE-DENSITY-009",
    "SFT-MAT-NANO-AGGREGATION-DISPERSION-CUSTODY-010",
)
...
def main():
    ...
    if canonical(frozen) != frozen_identity or canonical(previous) != previous_identity or previous["current_closed_count"] != 204:
        raise SystemExit("NANO reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(CLAIMS, 1):
        ...
        obligation = f"SFT-MAT-OBL-NANO-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["materials_obligation"] != obligation:
            raise SystemExit("NANO reconciliation halt " + claim_id)
    ...
    families["NANO"] = rows
    value = {
        "schema": "sft-v3-materials-discipline-current-reconciliation/12",
        "date": "2026-07-29",
        ...
        "current_closed_count": 214,
        "current_open_count": 75,
        "current_completion_fraction": "214/289",
        "current_completion_percent": "74.0%",
    ...
    audit = {
        "schema": "sft-v3-materials-nano-completion/1",
        "date": "2026-07-29",
        "family": "NANO-001--010",
        ...
        "current_materials_progress": "214/289",
        "current_materials_percent": "74.0%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 214, "open": 75, "percent": "74.0%", "identity": value["reconciliation_identity"]}, indent=2))
