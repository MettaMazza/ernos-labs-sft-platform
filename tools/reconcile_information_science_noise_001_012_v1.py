...
PREVIOUS = ROOT / "census/information_science_discipline_current_reconciliation_v8.json"
OUT = ROOT / "census/information_science_discipline_current_reconciliation_v9.json"
AUDIT = ROOT / "audits/INFORMATION_SCIENCE_NOISE_001_012_COMPLETION_2026-07-29.json"

from sft.information_science.noise_001_012_laws_v1 import IDS
...
def main():
    ...
    if canonical(frozen_body) != frozen_identity or canonical(previous_body) != previous_identity or previous["current_closed_count"] != 114:
        raise SystemExit("NOISE reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        obligation = f"SFT-INFO-OBL-NOISE-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["information_science_obligation"] != obligation:
            raise SystemExit("NOISE reconciliation halt: " + claim_id)
        rows.append({
    ...
    families["NOISE"] = rows
    value = {
        "schema": "sft-v3-information-science-discipline-current-reconciliation/9",
        "date": "2026-07-29",
        ...
        "current_closed_count": 126,
        "current_open_count": 136,
        "current_completion_fraction": "126/262",
        "current_completion_percent": "48.1%",
    ...
    audit = {
        "schema": "sft-v3-information-science-noise-completion/1",
        "date": "2026-07-29",
        "family": "NOISE-001--012",
        ...
        "current_information_science_progress": "126/262",
        "current_information_science_percent": "48.1%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 126, "open": 136, "percent": "48.1%", "identity": value["reconciliation_identity"]}, indent=2))
