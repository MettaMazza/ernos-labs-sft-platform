#!/usr/bin/env python3
"""Reconcile the completed VALID family against the frozen Quantum census."""
...
PREVIOUS = ROOT / "census/quantum_computation_discipline_current_reconciliation_v11.json"
OUT = ROOT / "census/quantum_computation_discipline_current_reconciliation_v12.json"
AUDIT = ROOT / "audits/QUANTUM_COMPUTATION_VALID_001_012_COMPLETION_2026-07-29.json"
...
def main():
    ...
    if canonical(previous_body) != previous_identity or previous["current_closed_count"] != 270:
    ...
    registry = json.loads((ROOT / "census/quantum_valid_001_012_target_registry_v1.json").read_text())
    rows = []
    ...
    value = {"schema": "sft-v3-quantum-computation-discipline-current-reconciliation/12", "date": "2026-07-29", "frozen_census_identity": frozen_identity, "frozen_obligation_count": 288, "closed_at_freeze": 22, "predecessor_reconciliation_identity": previous_identity, "completed_families": completed, "current_closed_count": 282, "current_open_count": 6, "current_completion_fraction": "282/288", "current_completion_percent": "97.9%", "frozen_census_mutated": False, "extension_policy": frozen["extension_policy"]}
    value["reconciliation_identity"] = canonical(value)
    OUT.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
    audit = {"schema": "sft-v3-quantum-computation-valid-completion/1", "date": "2026-07-29", "family": "VALID", "family_completion": "12/12", "generated_candidate_count": 12 * 256, "unique_survivor_count": 12, "passed_control_count": 12 * 4, "post_registry_exact_observation_count": 12, "implementation_distinct_reconstruction_count": 12, "exact_receipt_replay": "12/12", "receipt_rows": rows, "protected_engine_or_verifier_changed": False, "current_quantum_computation_progress": "282/288", "current_quantum_computation_percent": "97.9%", "frozen_census_identity": frozen_identity, "reconciliation_identity": value["reconciliation_identity"]}
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"family": "12/12", "candidates": 12 * 256, "controls": 12 * 4, "current": "282/288", "percent": "97.9%", "identity": value["reconciliation_identity"]}, indent=2))
