#!/usr/bin/env python3
"""Exact read-only replay of all twelve admitted Materials VALID receipts."""
...
from sft.materials.valid_001_012_laws_v1 import IDS as ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("VALID replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("VALID exact replay mismatch: " + claim_id)
