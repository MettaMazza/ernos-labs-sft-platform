#!/usr/bin/env python3
"""Admit and materialize all matter/flavour successor claims."""
...
def load_execution(claim_id: str):
    ...
    definition = importlib.util.spec_from_file_location("sft_physics_matter_" + claim_id.replace("-", "_"), path)
...
def main() -> None:
    ...
    receipts = {}
    for position, item in enumerate(MATTER_FLAVOUR_SPECS, 1):
        if item.claim_id in existing:
            receipt = read_receipt(ROOT / existing[item.claim_id]["receipt_path"])
            print(f"[{position}/{len(MATTER_FLAVOUR_SPECS)}] retained {item.claim_id}: {receipt.receipt_hash}")
        else:
            ...
            print(f"[{position}/{len(MATTER_FLAVOUR_SPECS)}] admitted {item.claim_id}: {receipt.receipt_hash}")
            existing = {row["claim_id"]: row for row in json.loads(census_path.read_text(encoding="utf-8"))["claims"]}
    ...
    for position, item in enumerate(MATTER_FLAVOUR_SPECS, 1):
        ...
        print(f"[{position}/{len(MATTER_FLAVOUR_SPECS)}] materialized {item.claim_id}")
