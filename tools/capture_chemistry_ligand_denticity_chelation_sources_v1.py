#!/usr/bin/env python3
"""Capture the complete predeclared INORG-003 authority surface byte-for-byte."""
...
def fetch(uri: str) -> bytes:
    ...
    if len(payload) < 500:
        raise ValueError(f"INORG-003 response unexpectedly short: {uri}")
    return payload
...
def main() -> None:
    ...
    if spec.get("target_values_present") is not False:
        raise ValueError("INORG-003 prefetch specification is not value-free")
    ...
    inventory = {
        "schema": "sft-v3-ligand-denticity-chelation-source-inventory/1",
