#!/usr/bin/env python3
"""Exact read-only replay of all twelve admitted CALC receipts."""
import json,sys
...
from sft.mathematics.calc_001_012_laws_v1 import IDS as CALC
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("CALC replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in CALC);authority=AuthorityLedger()
 ...
 for cid in CALC:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("CALC exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(CALC)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
