#!/usr/bin/env python3
"""Capture the frozen INORG-004–017 source family exactly once.

The value-free identity registry is immutable input. This capture preserves
successful, adverse, absent and unresolved source rows without selecting or
interpreting any scientific outcome.
"""
...
SNAPSHOT_ROOT = ROOT / "experiments/external_sources/chemistry/snapshots/inorg-004-017-family-v1"
...
def main() -> None:
    if hash_file(REGISTRY) != REGISTRY_HASH:
        raise SystemExit("VOID_INVALID_HALTED: family identity registry changed")
    registry = json.loads(REGISTRY.read_text(encoding="utf-8"))
    rows = registry.get("sources", [])
    if registry.get("target_values_or_outcomes_present") is not False:
        raise SystemExit("VOID_INVALID_HALTED: family identity registry is not value-free")
    if len(rows) != 40 or len({row["source_id"] for row in rows}) != 40:
        raise SystemExit("VOID_INVALID_HALTED: family source identity census changed")
    if INVENTORY.exists():
        raise SystemExit("family source inventory already exists; preserved without replay")
    ...
    for ordinal, source in enumerate(rows, start=1):
        ...
        request = Request(
            source["uri"],
            headers={"User-Agent": "Ernos-Labs-SFT-v3-family-source-capture/1"},
        )
        try:
    ...
    inventory = {
        "schema": "sft-v3-chemistry-inorg-004-017-family-source-inventory/1",
