#!/usr/bin/env python3
"""Fail-closed gate frozen before the Medicine public release is submitted."""
...
def main() -> None:
    ...
    github = read_json(ROOT / "publication/medicine_foundation_github_metadata.json")
    release = read_json(ROOT / "publication/medicine_foundation_release.json")
    paper = ROOT / "publications/current/medicine/FROM_FOLD_TO_MEDICINE.md"
    readme = ROOT / "README.md"
    doi = metadata["doi"]
    match = re.fullmatch(r"10\.5281/zenodo\.(\d+)", doi)
    assert match, "invalid Medicine DOI"
    ...
    assert github["release_tag"] == "medicine-v1.0.0"
    ...
    assert release["github_push_authorized"] is True
    assert release["zenodo_publish_authorized"] is True
    assert release["zenodo_record"] == record_id
    assert release["doi"] == doi
    assert paper.read_bytes() == readme.read_bytes(), "GitHub landing page is not the exact Medicine paper"
    ...
    with ZipFile(RELEASE / NAMES[1]) as archive:
        ...
        evidence = json.loads(archive.read("medicine_foundation_evidence_map.json"))
        assert evidence["publication_authorized"] is True
        assert evidence["claim_count"] == 72
        assert evidence["candidate_count"] == 18_432
        assert evidence["control_count"] == 288
        assert evidence["prior_atom_count"] == 5
    print(f"Medicine published-release gate v1: PASS doi={doi} files=4 claims=72")
