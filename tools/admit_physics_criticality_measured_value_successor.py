#!/usr/bin/env python3
"""Admit the corrected criticality measured-value successor."""
...
from sft.physics.criticality_measured_value_successor_v1 import CLAIM_ID, EXPERIMENT_ID, SOURCE_FILES, SOURCE_HASH, SOURCE_IDS, SOURCE_PATH, SPEC  # noqa: E402
from sft.physics.criticality_measured_value_successor_validation_v1 import TARGET_IDS, source_hashes  # noqa: E402
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\ncriticality successor admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_criticality_measured_value_056", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load criticality successor execution")
...
def experiment_registration():
    ...
    return {
        ...
        "development_observations": [{"source_id": "V1-V2-CRITICALITY-CONTEXT", "role": "development_only", "content_hash": record["superseded_validation_receipt_hash"]}],
        ...
        "inputs": [{"input_id": "FORMAL-CRITICALITY-RECEIPT", "value_kind": "admitted-formal-receipt", "content_hash": record["formal_receipt_hash"]}],
        "withheld_targets": [{"target_id": TARGET_IDS[0], "source_id": SOURCE_IDS[0], "content_withheld_from_prediction": True}],
        "dimension_unit_boundary": {"derived_dimension_carriers": ["exact critical exponent vector", "exact cascade exponent vector", "generated universality class key"], "external_reference_protocol": "Measured exponents remain post-seal dimensionless records normalized only by their reported standard uncertainties.", "proof_value_policy": "positive-generated-counts-and-exact-ratios-only", "measurement_record_policy": "external-records-never-become-proof-scalars-or-law-selectors"},
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("exact-criticality-measured-value-successor/1", SPEC.falsification_condition)),
            "metrics": [
                {"metric_id": "MANGANITE-COMPLETE-CLASS-KEYS", "definition": "Every material retains transition order, interaction range, scaling relation and exponent vector.", "unit_protocol": "source-bound structural key", "all_rows": True},
                {"metric_id": "MANGANITE-COMPLETE-UNIT-RESIDUAL", "definition": "All fifteen exponent measurements contribute once to the exact mean squared normalized residual.", "unit_protocol": "exact rational unit-normalized squared residual", "all_rows": True},
                {"metric_id": "ERBIUM-TURBULENCE-VECTOR", "definition": "Complete independent critical and cascade measurements contain the sealed exact carriers.", "unit_protocol": "dimensionless exact rational intervals and route records", "all_rows": True},
            ],
            "acceptance_condition": "All five class keys are complete, the fifteen-value residual mean is below the One, and every independent erbium/turbulence check passes without rewarding a displaced material row.",
            "falsification_condition": SPEC.falsification_condition,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "A class not descending from Claim 047 must reject."},
            {"control_id": "TAMPERED-SOURCE", "kind": "tampered_source", "expected_rejection": "Any source hash or measurement row change must reject."},
            {"control_id": "TAMPERED-ARTIFACT", "kind": "tampered_artifact", "expected_rejection": "A changed census, trace or certificate must reject."},
            {"control_id": "BOUNDARY", "kind": "boundary", "expected_rejection": "Target access, row selection, uncertainty rescaling or mismatch-as-success must reject."},
            {"control_id": "DISPLACED-MEASUREMENT", "kind": "unfavorable_measurement", "expected_rejection": "A deliberately displaced vector or incomplete class key must reject rather than become a passing label."},
        ],
        ...
        "stop_condition": "Halt without admission on a changed source, incomplete class key, unit residual failure, excluded exponent, missing route, omitted row, target access, fitting, uncertainty rescaling, mismatch-as-success or failed control.",
...
def claim_registration():
    return {
        ...
        "intended_certificate": "Complete 256-form census, independent reconstruction, five complete material class keys, fifteen-value exact unit-residual ledger, independent erbium/turbulence checks and hostile controls.",
...
def main():
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "- All five manganite materials retain complete structural class keys and identify the mean-field class.",
        "- All fifteen exponent measurements enter once; their exact normalized residual mean is `5286961/10584000`, below the One.",
        "- The complete erbium vector, turbulence structure interval, and both physical spectrum routes pass.",
        "- La02 remains fully visible, but its individual displacement is not rewarded as a result.",
        "- No uncertainty was widened, no row deleted, and displaced controls reject.",
        "- This successor replaces Claim 048 for empirical closure while preserving both historical receipts.",
