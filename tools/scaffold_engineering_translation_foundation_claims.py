#!/usr/bin/env python3
"""Scaffold all Engineering Translation packages and validators."""
...
def registration(spec) -> dict:
    return {
        ...
        "intended_certificate": "Complete 256-form census, one survivor, depth-independent positive-finite reconstruction and post-seal source-bound Engineering comparison preserving every adverse, absent, unresolved and failed row.",
        "empirical_protocol": f"experiments/engineering_translation/{spec.experiment_id}/registration.json",
...
def experiment(spec) -> dict:
    ...
    return {
        ...
        "evaluation_protocol": {
            "acceptance_condition": "The registered target opens only after the complete branch derivation seal; the source-bound consequence matches; failures and missingness remain; application, performance, design preference and status never select the law.",
...
def build_execution(root: Path) -> ClaimExecution:
    spec=next(x for x in ENGINEERING_SPECS if x.claim_id=={spec.claim_id!r})
    ...
    return ClaimExecution(program=GeneratedEmpiricalEngineeringProgram(spec,source_hash),independent_validator=ExternalCommandValidator({(spec.claim_id.lower()+'-independent-python/1')!r},(sys.executable,str(validator)),validator.parent,(validator,)),source_files=source_files,empirical_validator=BlindEngineeringBoundaryValidator(root,spec))
'''
...
def main() -> None:
    for spec in ENGINEERING_SPECS:
        ...
        exp = ROOT / "experiments/engineering_translation" / spec.experiment_id
        write(exp / "registration.json", json.dumps(experiment(spec), indent=2, sort_keys=True) + "\n")
    checkpoint = json.loads(CHECKPOINT.read_text())
    checkpoint.update({"status": "complete_claim_packages_scaffolded_not_admitted", "claim_package_count": len(ENGINEERING_SPECS), "independent_validator_count": len(ENGINEERING_SPECS), "next_exact_operation": "admit_" + ENGINEERING_SPECS[0].claim_id})
    CHECKPOINT.write_text(json.dumps(checkpoint, indent=2, sort_keys=True) + "\n")
    print(f"scaffolded Engineering claims={len(ENGINEERING_SPECS)}")
