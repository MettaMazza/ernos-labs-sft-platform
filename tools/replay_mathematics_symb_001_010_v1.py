#!/usr/bin/env python3
"""Exact read-only replay of all ten admitted SYMB receipts."""
import json,sys
...
from sft.mathematics.symb_001_010_laws_v1 import IDS as SYMB
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("SYMB replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in SYMB);authority=AuthorityLedger()
 ...
 for cid in SYMB:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("SYMB exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(SYMB)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
