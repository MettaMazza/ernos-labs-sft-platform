#!/usr/bin/env python3
"""Submit the Fold baryogenesis dependency law to the frozen engine."""
...
from sft.physics.baryogenesis_dependency_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_baryogenesis_dependency", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load baryogenesis dependency execution")
...
def main() -> None:
    ...
    (package / "STATUS.md").write_text("\n".join((f"# {CLAIM_ID}", "", "Status: `independently_replicated_with_inherited_postseal_comparison`", "", "- The complete three-condition process census is independently replicated.", "- The terminal CKM and baryon-to-photon dependencies supply the already sealed physical CP carrier and abundance comparison.", "- The result is a universal dependency law, not a claim about one cosmic history or a blind historical discovery.", f"- Closure: `{certificate['closure_scope']}`", f"- Derivation seal: `{certificate['derivation_seal_hash']}`", f"- Independent validation: `{certificate['external_validation_hash']}`", f"- Engine receipt: `{receipt.receipt_hash}`", f"- Receipt path: `{row['receipt_path']}`", "")), encoding="utf-8")
