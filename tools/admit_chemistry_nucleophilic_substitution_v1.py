...
from sft.chemistry.nucleophilic_substitution_batch_v1 import NUCLEOPHILIC_SUBSTITUTION_SPEC, PRIMARY_PATH  # noqa: E402
from sft.chemistry.nucleophilic_substitution_validation_v1 import _source_rows, exact_analysis  # noqa: E402
...
def load_execution():
    path = ROOT / "claims" / NUCLEOPHILIC_SUBSTITUTION_SPEC.claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("org007_execution", path)
    module = importlib.util.module_from_spec(definition); definition.loader.exec_module(module)
    return module.build_execution(ROOT)


def main() -> None:
    spec = NUCLEOPHILIC_SUBSTITUTION_SPEC
    ...
    claim_row = next(row for row in json.loads(claims_path.read_text())["claims"] if row["claim_id"] == spec.claim_id)
    package = ROOT / "claims" / spec.claim_id
    analysis, checks = exact_analysis(_source_rows(ROOT), json.loads((ROOT / PRIMARY_PATH).read_text()))
    certificate = {
        "claim_id": spec.claim_id, "chemistry_obligation": "SFT-CHEM-OBL-ORG-007",
        "status": "model_admitted_forced_structural_law_postseal_externally_tested_and_independently_replicated",
        ...
        "engine_receipt_path": claim_row["receipt_path"], "closure_scope": receipt.closure_status,
        ...
        "conventional_mechanism_name_rate_energy_solvent_substrate_or_external_structure_used_to_generate_or_select_law": False,
    ...
    experiment = json.loads(experiment_path.read_text()); experiment["status"] = "measured_postseal_complete"
    write_json(experiment_path, experiment)
    (package / "STATUS.md").write_text(
        f"# {spec.claim_id}\n\nStatus: `model_admitted_forced_structural_law_postseal_externally_tested_and_independently_replicated`\n\n"
        f"- Chemistry obligation: `SFT-CHEM-OBL-ORG-007`\n"
        f"- Exact law: incoming held pair forms the new bond; displaced bond pair remains on the leaving carrier; every occurrence is conserved.\n"
        f"- Complete path family: one-transition exchange and cleavage-then-formation; formation-before-cleavage halts.\n"
        f"- External surface: nine complete records; seven exact payloads opened post-seal and two development-observed records explicitly disclosed.\n"
        f"- Exact structure vector: `CBr + [OH-] -> CO + [Br-]`; source and terminal inventories both `Br1 C1 H4 O1`.\n"
