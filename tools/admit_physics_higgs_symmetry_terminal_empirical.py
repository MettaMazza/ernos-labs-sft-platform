#!/usr/bin/env python3
"""Admit the post-seal terminal Higgs comparison through the sealed engine."""
...
from sft.physics.higgs_symmetry_terminal_validation_v1 import TARGET_IDS, source_hashes
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\nHiggs terminal empirical admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_Higgs_symmetry_terminal_empirical_066", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load Higgs terminal empirical execution")
...
def experiment_registration():
    ...
    return {
        ...
        "development_observations": [{"source_id": "V1-V2-HIGGS-CONTEXT", "role": "development_only", "content_hash": sha256_identity("prior-obligation-only")}],
        ...
        "inputs": [{"input_id": "HIGGS-TERMINAL-FORMAL-RECEIPT", "value_kind": "admitted-formal-receipt", "content_hash": record["formal_receipt_hash"]}],
        ...
        "dimension_unit_boundary": {
            "derived_dimension_carriers": ["terminal Higgs mass ratio", "native self-coupling"],
            "external_reference_protocol": "The PDG electroweak scale supplies the dimensional GeV reference only after the exact ratio seals.",
            "proof_value_policy": "positive exact whole and fractional Fold forms only",
            "measurement_record_policy": "reported signed coordinates remain typed external directions and magnitudes, never negative Fold proof scalars",
        },
        "prediction_protocol": {
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("exact-Higgs-terminal-comparator/1", SPEC.falsification_condition)),
            "metrics": [
                {"metric_id": "HIGGS-MASS-AGGREGATE", "definition": "Test the exact predicted GeV value against the complete PDG listed-average interval.", "unit_protocol": "exact rational GeV", "all_rows": True},
                {"metric_id": "HIGGS-MASS-INDIVIDUAL", "definition": "Retain exact ATLAS and CMS offsets and every reported uncertainty without relabelling.", "unit_protocol": "exact rational GeV and exact uncertainty ratio", "all_rows": True},
                {"metric_id": "HIGGS-SELF-COUPLING", "definition": "Test normalized unity against the complete direct-search interval while retaining the precision boundary.", "unit_protocol": "typed signed-coordinate interval with positive exact magnitudes", "all_rows": True},
            ],
            "acceptance_condition": "The aggregate mass interval contains the sealed prediction, direct self-coupling bounds contain normalized unity, and all individual offsets and capability boundaries remain visible.",
            "falsification_condition": SPEC.falsification_condition,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "A law not descending from Claim 065 rejects."},
            ...
            {"control_id": "BOUNDARY", "kind": "boundary", "expected_rejection": "Target access, fitting, omission or evidential relabelling rejects."},
            {"control_id": "OUTSIDE-MASS", "kind": "unfavorable_measurement", "expected_rejection": "An aggregate interval excluding the exact mass prediction rejects."},
            {"control_id": "OUTSIDE-COUPLING", "kind": "unfavorable_measurement", "expected_rejection": "A direct interval excluding normalized unity rejects."},
            {"control_id": "ERASED-INDIVIDUAL-OFFSET", "kind": "unfavorable_measurement", "expected_rejection": "Deleting an individual offset rejects completeness."},
        ],
        "custody_protocol": {
        ...
        "row_retention_policy": "retain-aggregate-individual-uncertainty-and-coupling-boundary-rows",
        "stop_condition": "Halt without admission on changed evidence, aggregate exclusion, coupling exclusion, omitted individual row, target access, fitting, signed-coordinate import or failed control.",
...
def main():
    ...
    write_json(package / "registration.json", {
        ...
        "intended_certificate": "Complete 256-form census, exact terminal mass comparison, complete aggregate and individual rows, direct self-coupling boundary and hostile exclusion controls.",
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "",
        "- The exact terminal ratio predicts 125.266104978... GeV after the post-seal PDG dimensional reference is applied.",
        "- The prediction lies inside the complete PDG 2025 listed-average uncertainty interval.",
        "- The ATLAS and CMS individual offsets and uncertainties remain explicit in the certificate.",
        "- The native self-coupling is exact; the latest direct normalized interval contains unity but remains broad.",
        "- Outside-mass, outside-coupling and erased-offset controls reject.",
