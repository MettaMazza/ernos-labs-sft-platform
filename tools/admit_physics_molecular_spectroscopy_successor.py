#!/usr/bin/env python3
"""Admit and materialize the terminal molecular-spectroscopy successor."""
...
from sft.physics.molecular_spectroscopy_successor_laws_v1 import (  # noqa: E402
    MOLECULAR_SPECTROSCOPY_SPEC,
)
from sft.physics.molecular_spectroscopy_successor_validation_v1 import (  # noqa: E402
...
def load_execution():
    path = ROOT / "claims" / MOLECULAR_SPECTROSCOPY_SPEC.claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_molecular_spectroscopy_terminal", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load terminal molecular-spectroscopy execution")
    ...
    return {
        ...
        "dimension_unit_boundary": {
            ...
            "external_reference_protocol": (
                "Hash-bound NIST H2/D2 snapshots remain capability-closed while exact ladder and ratio relations seal; source inscriptions are released only to the exact post-seal comparator."
            ),
        ...
        "evaluation_protocol": {
            ...
            "metrics": [{
                ...
                "definition": "Verify both source hashes, propagate every displayed-resolution endpoint outward, retain all four ratio intervals and compare only after seal release.",
                "unit_protocol": "External inverse-centimetre and angstrom inscriptions remain source records; only exact positive rational arithmetic enters the evaluator.",
                "all_rows": True,
            }],
            "acceptance_condition": "All four sealed ratios are contained, both exact ladders hold, both isotope directions hold and every tampered row is rejected.",
            "falsification_condition": spec.falsification_condition,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "Incomplete molecular carrier is rejected."},
        ...
        "stop_condition": "Halt after every registered source, ratio and adverse-control row is evaluated once, or immediately on any violation.",
        "source_hashes": {
...
def main() -> None:
    spec = MOLECULAR_SPECTROSCOPY_SPEC
    ...
    registration = {
        ...
        "empirical_protocol": f"experiments/physics/{MOLECULAR_EMPIRICAL_SPEC.experiment_id}/registration.json",
        "excluded_inputs": list(spec.exclusions),
        "intended_certificate": (
            f"One engine receipt requiring all {candidate_census['expected_cardinality']:,} typed forms, one survivor, "
            "implementation-distinct exact reconstruction and complete hostile controls, followed by capability-gated "
            "release of the complete NIST H2/D2 displayed-resolution vector."
        ),
    ...
    experiment_path = ROOT / "experiments/physics" / MOLECULAR_EMPIRICAL_SPEC.experiment_id / "registration.json"
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "- Observation informed the explicit frozen law; capability-gated execution sealed the exact relation vector before NIST target release.",
    ...
    why = [
        ...
        spec.statement,
        "",
        "This uses the disclosed observational-derivation empirical prediction protocol. Observation informs the explicit frozen law; the measurement vector is then placed behind the capability boundary; target-inaccessible engine execution enumerates every form, retains the sole survivor and seals the exact relation; only then are both complete NIST source rows released for exact comparison.",
        "",
        "## DERIVATION",
        "",
        f"Grammar boundary: {spec.grammar_boundary}",
        "",
        f"The complete {len(spec.axes)}-axis grammar contains {candidate_census['expected_cardinality']:,} forms. Exactly one survives:",
        "",
        f"`{survivor}`",
        "",
    ]
    for axis in spec.axes:
        why.append(f"- `{axis.key}`: `{axis.survivor.name}` — {axis.survivor.reason}")
    why.extend((
        "",
        ...
        "- A separate exact implementation reconstructs both ladders and all four ratios without measurement access.",
        "- Both NIST component hashes and every displayed digit are retained.",
        "- Exact outward interval propagation contains all four sealed ratios.",
        "- The earlier electronic-over-molecular hierarchy remains, while its rejected equal-quarter numerical reading remains adverse evidence.",
        "- A deliberately changed measurement label is rejected.",
        "",
        "## EXCLUSIONS",
        "",
        *(f"- {item}" for item in spec.exclusions),
        "",
    ))
