#!/usr/bin/env python3
"""Submit finite Fold quantum gravity to the frozen engine."""
...
from sft.physics.finite_quantum_gravity_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_finite_quantum_gravity", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load finite quantum-gravity execution")
...
def main() -> None:
    ...
    (package / "STATUS.md").write_text("\n".join((f"# {CLAIM_ID}", "", "Status: `independently_replicated_with_inherited_postseal_comparison`", "", "- One finite lattice jointly composes quantum support, rank-two gravity, loop closure and the horizon ledger.", "- Existing admitted wave, finite-loop and horizon claims supply the inherited post-seal comparison boundary.", "- The claim is explicitly finite and does not assert direct graviton detection or a continuum completion.", f"- Closure: `{certificate['closure_scope']}`", f"- Derivation seal: `{certificate['derivation_seal_hash']}`", f"- Independent validation: `{certificate['external_validation_hash']}`", f"- Engine receipt: `{receipt.receipt_hash}`", f"- Receipt path: `{row['receipt_path']}`", "")), encoding="utf-8")
