"""Project one admitted empirical claim into complete derivation and measurement evidence."""
...
def load_execution(path: Path):
    definition = importlib.util.spec_from_file_location("sft_empirical_materialization", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("empirical execution module cannot be loaded")
...
def main() -> None:
    ...
    if receipt != read_receipt(ROOT / row["receipt_path"]):
        raise RuntimeError("fresh empirical receipt does not match admitted evidence")
    ...
    payloads = {
        ...
        "elimination_receipt.json": {"claim_id": args.claim_id, "decisions": asdict(sealed)["decisions"], "closure": asdict(sealed.closure)},
        "controls.json": {"claim_id": args.claim_id, "controls": asdict(sealed)["controls"]},
        "empirical_validation.json": {"claim_id": args.claim_id, **asdict(empirical)},
        "certificate.json": {
    ...
    for name, payload in payloads.items():
        (package / name).write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(f"materialized empirical {args.claim_id}: {len(sealed.census.candidates)} candidates; {len(empirical.measurements)} measurement rows")
