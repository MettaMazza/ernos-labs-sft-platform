#!/usr/bin/env python3
"""Exact read-only replay of the twelve admitted Materials MAGSC receipts."""
import json
...
from sft.materials.magsc_001_012_laws_v1 import ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("MAGSC replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("MAGSC exact replay mismatch: " + claim_id)
