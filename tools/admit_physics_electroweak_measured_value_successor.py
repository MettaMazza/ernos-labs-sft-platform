#!/usr/bin/env python3
"""Admit the clean electroweak measured-value successor."""
...
from sft.physics.electroweak_measured_value_successor_v1 import CLAIM_ID, EXPERIMENT_ID, SOURCE_FILES, SOURCE_HASH, SOURCE_IDS, SOURCE_PATH, SPEC  # noqa: E402
from sft.physics.electroweak_measured_value_successor_validation_v1 import TARGET_IDS, source_hashes  # noqa: E402
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\nelectroweak successor admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_electroweak_measured_value_053", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load electroweak successor execution")
...
def experiment_registration():
    ...
    return {
        ...
        "development_observations": [
            {"source_id": "V1-V2-ELECTROWEAK-MEASURED-VALUE-CONTEXT", "role": "development_only", "content_hash": record["superseded_validation_receipt_hash"]},
        ],
        ...
        "inputs": [{"input_id": "FORMAL-ELECTROWEAK-TERMINAL-RECEIPT", "value_kind": "admitted-formal-receipt", "content_hash": record["formal_receipt_hash"]}],
        "withheld_targets": [{"target_id": TARGET_IDS[0], "source_id": SOURCE_IDS[0], "content_withheld_from_prediction": True}],
        "dimension_unit_boundary": {"derived_dimension_carriers": ["dimensionless on-shell sine-squared Fold fraction", "dimensionless compatible W/Z squared reconstruction"], "external_reference_protocol": "Mass units cancel only in the post-seal like-dimension ratio; no external unit enters formal forcing.", "proof_value_policy": "positive-generated-counts-and-exact-ratios-only", "measurement_record_policy": "external-records-never-become-proof-scalars-or-law-selectors"},
        ...
        "evaluation_protocol": {"evaluator_id": EXPERIMENT_ID + "-post-seal-evaluator", "comparison_implementation_hash": sha256_identity(("exact-electroweak-measured-value-successor/1", SPEC.falsification_condition)), "metrics": [{"metric_id": "DIRECT-ON-SHELL-INTERVAL", "definition": "Exact membership of the sealed terminal share in the complete direct PDG interval.", "unit_protocol": "dimensionless exact rational interval", "all_rows": True}, {"metric_id": "COMPATIBLE-WZ-INTERVAL", "definition": "Exact membership of the sealed complement in the complete outward W/Z squared interval.", "unit_protocol": "like-dimension mass ratio squared after seal", "all_rows": True}], "acceptance_condition": "Both like-typed measured-value intervals contain the sealed exact relation; every source and methodology row remains unchanged; no inconsistent aggregate is promoted to a physical target.", "falsification_condition": SPEC.falsification_condition},
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "A relation not descending from the admitted terminal formal claim must reject."},
            {"control_id": "TAMPERED-SOURCE", "kind": "tampered_source", "expected_rejection": "Any source hash or registered row change must reject."},
            {"control_id": "TAMPERED-ARTIFACT", "kind": "tampered_artifact", "expected_rejection": "A changed candidate census, trace or certificate must reject."},
            {"control_id": "BOUNDARY", "kind": "boundary", "expected_rejection": "Target access, fitting, uncertainty rescaling or aggregate-as-law conflation must reject."},
            {"control_id": "DISPLACED-MEASUREMENT", "kind": "unfavorable_measurement", "expected_rejection": "A deliberately displaced direct angle or compatible W value must reject rather than become a passing label."},
        ],
        ...
        "stop_condition": "Halt without admission on a changed source, missed like-typed interval, altered method record, target access, fitting, uncertainty rescaling or failed control.",
...
def claim_registration():
    return {
        ...
        "intended_certificate": "Complete 256-form census, independent reconstruction, post-seal direct on-shell and compatible W/Z interval comparisons, source-consistency boundary and hostile controls.",
...
def main():
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "- The exact forced terminal on-shell value lies inside the complete direct PDG interval.",
        "- Its exact complement lies inside the complete compatible-input W/Z squared interval.",
        "- No uncertainty was widened and no mismatch was admitted as closure.",
        "- The inconsistent all-input aggregate is retained unchanged as a measurement-method record, not an SFT result.",
        "- This claim supersedes the earlier mismatch-as-success validation for branch-closure purposes without deleting its immutable receipt.",
