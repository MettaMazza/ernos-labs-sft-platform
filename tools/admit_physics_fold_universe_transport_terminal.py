#!/usr/bin/env python3
"""Submit composite Fold-orbit transport boundary to the frozen engine."""
...
from sft.physics.fold_universe_transport_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_fold_universe_transport", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load Fold-universe transport execution")
...
def main() -> None:
    ...
    registration["status"] = "independently_replicated_with_adverse_physical_transport_boundary"
    ...
    (package / "STATUS.md").write_text("\n".join((f"# {CLAIM_ID}", "", "Status: `independently_replicated_with_adverse_physical_transport_boundary`", "", "- Exact composite-orbit bijection, Fold commutation and least-common-multiple recurrence are retained.", "- The prior literal signalling/travel interpretation is resolved adversely: the native denominator never changes and correlation does not steer the paired component.", "- A future physical transport claim would require its own generated causal operation and observational discriminator.", f"- Closure: `{certificate['closure_scope']}`", f"- Derivation seal: `{certificate['derivation_seal_hash']}`", f"- Independent validation: `{certificate['external_validation_hash']}`", f"- Engine receipt: `{receipt.receipt_hash}`", f"- Receipt path: `{row['receipt_path']}`", "")), encoding="utf-8")
