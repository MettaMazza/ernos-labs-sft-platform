#!/usr/bin/env python3
"""Admit the post-seal strong-CP/baryon comparison through the sealed engine."""
...
from sft.physics.strong_cp_baryon_stability_validation_v1 import TARGET_IDS, source_hashes
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\nstrong-CP/baryon empirical admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_strong_cp_baryon_stability_empirical_064", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load strong-CP/baryon empirical execution")
...
def experiment_registration():
    ...
    return {
        ...
        "development_observations": [{"source_id": "V1-V2-STRONG-CP-PROTON-STABILITY-CONTEXT", "role": "development_only", "content_hash": sha256_identity("prior-obligation-only")}],
        ...
        "inputs": [{"input_id": "STRONG-CP-BARYON-STABILITY-FORMAL-RECEIPT", "value_kind": "admitted-formal-receipt", "content_hash": record["formal_receipt_hash"]}],
        ...
        "dimension_unit_boundary": {
            "derived_dimension_carriers": ["aligned One strong phase", "empty-One EDM carrier", "baryon One invariant"],
            "external_reference_protocol": "EDM uncertainties and proton lifetime limits remain post-seal measurement records.",
            "proof_value_policy": "positive-generated-counts-exact-ratios-and-empty-One-absence-only",
            "measurement_record_policy": "null estimates and finite limits retain their source-defined evidential type",
        },
        "prediction_protocol": {
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("exact-strong-cp-baryon-stability-comparator/1", SPEC.falsification_condition)),
            "metrics": [
                {"metric_id": "DIRECT-NEDM", "definition": "Retain central status, both uncertainty magnitudes and finite upper limit without proof-value relabelling.", "unit_protocol": "source-bound exact rational magnitude", "all_rows": True},
                {"metric_id": "PROTON-DECAY-VECTOR", "definition": "Retain all six mode limits and every candidate interpretation.", "unit_protocol": "source-bound complete mode vector", "all_rows": True},
            ],
            "acceptance_condition": "No direct row reports a confirmed nonempty neutron EDM or statistically significant proton decay, and every evidential boundary remains intact.",
            "falsification_condition": SPEC.falsification_condition,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "A law not descending from Claim 063 rejects."},
            ...
            {"control_id": "CONFIRMED-EDM", "kind": "unfavorable_measurement", "expected_rejection": "A confirmed nonempty neutron EDM rejects alignment."},
            {"control_id": "CONFIRMED-PROTON-DECAY", "kind": "unfavorable_measurement", "expected_rejection": "A confirmed proton decay rejects stability."},
        ],
        "custody_protocol": {
        ...
        "row_retention_policy": "retain-every-registered-uncertainty-candidate-and-mode-row",
        "stop_condition": "Halt without admission on changed evidence, confirmed contradictory signal, omitted row, target access, fitting, evidential relabelling or failed control.",
...
def main():
    ...
    write_json(package / "registration.json", {
        ...
        "intended_certificate": "Complete 256-form census, direct neutron-EDM vector, six proton modes, complete candidate custody and hostile confirmed-signal controls.",
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "",
        "- The complete direct neutron-EDM record remains consistent with the sealed aligned-One prediction.",
        "- All six registered proton-decay modes retain their finite limits and candidate interpretations.",
        "- No registered mode reports statistically significant proton decay.",
        "- Null estimates, background candidates and finite limits retain their exact evidential types.",
        "- Confirmed-EDM and confirmed-proton-decay controls both reject.",
