#!/usr/bin/env python3
"""Submit the scattering partition/path law to the frozen engine."""
...
from sft.physics.scattering_partition_path_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_scattering_partition_path", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load scattering partition/path execution")
...
def main() -> None:
    ...
    (package / "STATUS.md").write_text("\n".join((f"# {CLAIM_ID}", "", "Status: `independently_replicated`", "", f"- Closure: `{certificate['closure_scope']}`", "- Empirical status: exact structural law; existing admitted Rutherford/Compton comparison supplies the downstream scattering correspondence", f"- Derivation seal: `{certificate['derivation_seal_hash']}`", f"- Independent validation: `{certificate['external_validation_hash']}`", f"- Engine receipt: `{receipt.receipt_hash}`", f"- Receipt path: `{row['receipt_path']}`", "")), encoding="utf-8")
