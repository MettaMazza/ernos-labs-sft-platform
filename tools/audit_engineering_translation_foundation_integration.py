#!/usr/bin/env python3
"""Complete integration audit for the Engineering and Collective Sciences foundation."""
...
from sft.engineering_translation.generated_law import ENGINEERING_BLUEPRINTS, candidate_forms
...
def main() -> None:
    ...
    ids = [x.claim_id for x in ENGINEERING_BLUEPRINTS]
    missing = [x for x in ids if x not in admitted]
    if missing:
        raise ValueError(f"unadmitted Engineering claims: {missing}")
    if len(ids) != 72 or sum(len(candidate_forms(x)) for x in ENGINEERING_BLUEPRINTS) != 18432:
        raise ValueError("Engineering candidate census differs")
    if (targets["claim_count"], targets["passed_claim_count"], targets["unresolved_claim_count"]) != (72, 72, 0):
        raise ValueError("Engineering external target census incomplete")
    if targets["all_unfavorable_absent_and_failed_rows_preserved"] is not True:
        raise ValueError("Engineering adverse or failed rows were lost")
    if targets["application_performance_or_status_used_as_law_proof"] is not False:
        raise ValueError("Engineering application, performance or status was used as law proof")
    ...
    if len(atom_map) != prior_count or any(x not in admitted for x in atom_map.values()):
        raise ValueError("Engineering prior atom reconciliation incomplete")
    receipt_rows = []
    for claim_id in ids:
    ...
    result = {
        "schema": "sft-v3-engineering-translation-foundation-integration-audit/1",
    ...
    path = ROOT / "audits/engineering_translation_foundation_integration.json"
    ...
    print(f"Engineering integration: 72/72, 18432 candidates, {len(atom_map)}/{len(atom_map)} prior atoms, hash={result['integration_hash']}")
