...
from sft.mathematics.alext_001_010_laws_v1 import IDS as ORDER,SPECS
...
def seals():
 for tool,status in (("verify_engine_seal.py","VALID_CANONICAL_ENGINE"),("verify_verification_authority_seal.py","VALID_CANONICAL_VERIFICATION_AUTHORITY")):
  ...
  if result.returncode or data.get("status")!=status:raise SystemExit("Mathematics ALEXT admission halted: protected seal invalid")
def load(cid):
 path=ROOT/"claims"/cid/"execution.py";spec=importlib.util.spec_from_file_location("alext_"+cid.replace("-","_"),path);module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);return module.build_execution(ROOT)
def main():
 ...
 for index,cid in enumerate(ORDER,1):
  ...
  certificate["mathematics_obligation"]=f"SFT-MATH-OBL-ALEXT-{spec.claim_id.rsplit('-',1)[-1]}"
  payload["registration.json"]["pre_source_target_registry"]="census/mathematics_alext_001_010_target_registry_v1.json"
