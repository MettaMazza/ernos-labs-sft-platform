"""Materialize the first immutable Bonding and Molecular Organization batch."""
...
from sft.chemistry.bonding_molecular_batch_1 import (  # noqa: E402
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in BONDING_MOLECULAR_BATCH_1_SPECS if item.claim_id == {spec.claim_id!r})
    source_files = (
...
def extended_note(spec) -> str:
    return note(spec).replace(
        "required fragments of the byte-sealed official IUPAC term",
        "required fragments of the byte-sealed official IUPAC definition and numbered notes",
    )
...
def main() -> None:
    ...
    for spec in BONDING_MOLECULAR_BATCH_1_SPECS:
        ...
        write(package / "WHY_DERIVATION_CHECK.md", extended_note(spec))
        ...
        record["evaluation_protocol"]["metrics"][0]["definition"] = (
            ...
            "official term definition and numbered notes, then compare the held label with the sealed "
            "Fold prediction."
        )
        write(
