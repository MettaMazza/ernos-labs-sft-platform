#!/usr/bin/env python3
"""Submit the post-seal quantum-support Bell empirical successor."""
...
from sft.physics.quantum_support_uncertainty_empirical_v1 import (  # noqa: E402
    CLAIM_ID,
    EXPERIMENT_ID,
    SOURCE_FILE,
    SOURCE_FILE_HASH,
...
from sft.physics.quantum_support_uncertainty_validation_v1 import TARGET_IDS  # noqa: E402
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\nquantum-support Bell empirical admission halted")


def load_execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_quantum_support_bell_empirical", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load quantum-support Bell empirical execution")
...
def experiment_registration():
    ...
    return {
        ...
        "development_observations": [
            ...
            "Formal Claim 049, its complete formal censuses, and its official engine receipt preceded target retrieval.",
            "The source's measurement-independence assumption and deterministic-model limitation remain retained as evidence boundaries.",
        ],
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("exact-quantum-support-Bell-comparator/1", SPEC.falsification_condition)),
            "acceptance_condition": "The complete S interval exceeds two; trial, significance, timing and memory rows pass; every assumption and interpretation boundary remains explicit.",
        ...
        "controls": [
            {
                ...
                "kind": kind,
                "expected_rejection": "Any premise, source, artifact, interval, timing, assumption or interpretation-boundary violation halts.",
            }
        ...
        "custody_protocol": {
            ...
            "target_commitment_hash": sha256_identity((SOURCE_HASH, SOURCE_FILE_HASH, TARGET_IDS)),
            "release_requires_matching_seal": True,
        },
        "target_access_policy": "structurally-denied-before-seal",
        "row_retention_policy": "retain the complete S interval, trials, significance, timing, memory, setting method, measurement-independence assumption and every interpretation boundary",
        "source_hashes": {SOURCE_PATH: SOURCE_HASH, SOURCE_FILE: SOURCE_FILE_HASH},
...
def claim_registration():
    return {
        ...
        "intended_certificate": "Complete 256-form census, independent reconstruction, capability-closed custody, exact S and timing comparisons, assumption retention and adverse-control rejection.",
...
def main():
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "Status: `empirically_tested_and_independently_replicated`",
        "",
        "- The complete measured S interval lies above the local-factorization bound two.",
        "- More than one million trials, the reported significance, space-like timing and memory control are retained.",
        "- Measurement independence remains a declared assumption rather than a hidden premise.",
        "- The result rejects local factorization under the declared conditions, not determinism as a whole.",
        "- No ontic randomness is imported and no support-count law is relabelled as measured variance.",
