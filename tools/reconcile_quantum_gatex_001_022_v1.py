#!/usr/bin/env python3
"""Reconcile the complete admitted GATEX family against the frozen census."""
import hashlib
import json
...
PREVIOUS = ROOT / "census/quantum_computation_discipline_current_reconciliation_v3.json"
OUT = ROOT / "census/quantum_computation_discipline_current_reconciliation_v4.json"
AUDIT = ROOT / "audits/QUANTUM_COMPUTATION_GATEX_001_022_COMPLETION_2026-07-29.json"
...
def receipt_identity(path):
    ...
    if recorded != computed:
        raise SystemExit("GATEX stored receipt identity mismatch: " + path.name)
    return computed


def main():
    if OUT.exists() or AUDIT.exists():
        raise SystemExit("GATEX reconciliation already exists")
    ...
    if canonical(previous_body) != previous_identity or previous["current_closed_count"] != 68:
    ...
    if len(obligations) != 22:
        raise SystemExit("GATEX obligation count changed")
    live = {row["claim_id"]: row for row in json.loads((ROOT / "census/claims.json").read_text())["claims"]}
    registry = json.loads((ROOT / "census/quantum_gatex_001_022_target_registry_v1.json").read_text())
    rows = []
    for claim_id, obligation in zip(registry["claim_ids"], obligations):
        ...
        if (row is None or not row.get("model_admitted") or receipt_identity(ROOT / row["receipt_path"]) != row["receipt_hash"] or certificate.get("engine_receipt_hash") != row["receipt_hash"] or certificate.get("quantum_computation_obligation") != obligation["obligation_id"] or certificate.get("candidate_count") != 256 or certificate.get("unique_survivor_count") != 1 or not certificate.get("controls_passed") or len(census["candidates"]) != 256 or len(controls) != 4 or not all(control["passed"] for control in controls) or not empirical.get("all_rows_preserved") or not empirical.get("passed")):
            raise SystemExit("GATEX reconciliation halt: " + claim_id)
    ...
    completed["GATEX"] = rows
    value = {"schema": "sft-v3-quantum-computation-discipline-current-reconciliation/4", "date": "2026-07-29", "frozen_census_identity": frozen_identity, "frozen_obligation_count": 288, "closed_at_freeze": 22, "predecessor_reconciliation_identity": previous_identity, "completed_families": completed, "current_closed_count": 90, "current_open_count": 198, "current_completion_fraction": "90/288", "current_completion_percent": "31.3%", "frozen_census_mutated": False, "extension_policy": frozen["extension_policy"]}
    value["reconciliation_identity"] = canonical(value)
    OUT.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
    audit = {"schema": "sft-v3-quantum-computation-gatex-completion/1", "date": "2026-07-29", "family": "GATEX", "family_completion": "22/22", "generated_candidate_count": 22 * 256, "unique_survivor_count": 22, "passed_control_count": 22 * 4, "post_registry_exact_observation_count": 22, "implementation_distinct_reconstruction_count": 22, "exact_receipt_replay": "22/22", "receipt_rows": rows, "protected_engine_or_verifier_changed": False, "current_quantum_computation_progress": "90/288", "current_quantum_computation_percent": "31.3%", "frozen_census_identity": frozen_identity, "reconciliation_identity": value["reconciliation_identity"]}
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"family": "22/22", "candidates": 22 * 256, "controls": 22 * 4, "current": "90/288", "percent": "31.3%", "identity": value["reconciliation_identity"]}, indent=2))
