#!/usr/bin/env python3
"""Reconcile the completed QSIMX family against the frozen Quantum census."""
...
PREVIOUS = ROOT / "census/quantum_computation_discipline_current_reconciliation_v8.json"
OUT = ROOT / "census/quantum_computation_discipline_current_reconciliation_v9.json"
AUDIT = ROOT / "audits/QUANTUM_COMPUTATION_QSIMX_001_024_COMPLETION_2026-07-29.json"
...
def receipt_identity(path):
    ...
    if recorded != computed:
        raise SystemExit("QSIMX stored receipt identity mismatch: " + path.name)
    return computed


def main():
    if OUT.exists() or AUDIT.exists():
        raise SystemExit("QSIMX reconciliation already exists")
    ...
    if canonical(previous_body) != previous_identity or previous["current_closed_count"] != 202:
    ...
    if len(obligations) != 24:
        raise SystemExit("QSIMX obligation count changed")
    live = {row["claim_id"]: row for row in json.loads((ROOT / "census/claims.json").read_text())["claims"]}
    registry = json.loads((ROOT / "census/quantum_qsimx_001_024_target_registry_v1.json").read_text())
    rows = []
    for claim_id, obligation in zip(registry["claim_ids"], obligations):
        ...
        if (row is None or not row.get("model_admitted") or receipt_identity(ROOT / row["receipt_path"]) != row["receipt_hash"] or certificate.get("engine_receipt_hash") != row["receipt_hash"] or certificate.get("quantum_computation_obligation") != obligation["obligation_id"] or certificate.get("candidate_count") != 256 or certificate.get("unique_survivor_count") != 1 or not certificate.get("controls_passed") or len(census["candidates"]) != 256 or len(controls) != 4 or not all(control["passed"] for control in controls) or not empirical.get("all_rows_preserved") or not empirical.get("passed")):
            raise SystemExit("QSIMX reconciliation halt: " + claim_id)
    ...
    completed["QSIMX"] = rows
    value = {"schema": "sft-v3-quantum-computation-discipline-current-reconciliation/9", "date": "2026-07-29", "frozen_census_identity": frozen_identity, "frozen_obligation_count": 288, "closed_at_freeze": 22, "predecessor_reconciliation_identity": previous_identity, "completed_families": completed, "current_closed_count": 226, "current_open_count": 62, "current_completion_fraction": "226/288", "current_completion_percent": "78.5%", "frozen_census_mutated": False, "extension_policy": frozen["extension_policy"]}
    value["reconciliation_identity"] = canonical(value)
    OUT.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
    audit = {"schema": "sft-v3-quantum-computation-qsimx-completion/1", "date": "2026-07-29", "family": "QSIMX", "family_completion": "24/24", "generated_candidate_count": 24 * 256, "unique_survivor_count": 24, "passed_control_count": 24 * 4, "post_registry_exact_observation_count": 24, "implementation_distinct_reconstruction_count": 24, "exact_receipt_replay": "24/24", "receipt_rows": rows, "protected_engine_or_verifier_changed": False, "current_quantum_computation_progress": "226/288", "current_quantum_computation_percent": "78.5%", "frozen_census_identity": frozen_identity, "reconciliation_identity": value["reconciliation_identity"]}
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"family": "24/24", "candidates": 24 * 256, "controls": 24 * 4, "current": "226/288", "percent": "78.5%", "identity": value["reconciliation_identity"]}, indent=2))
