#!/usr/bin/env python3
"""Admit the complete ALGX-001 through ALGX-031 family sequentially."""
from dataclasses import asdict
import importlib.util,json,subprocess,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:sys.path.insert(0,str(ROOT))
from sft.computation.algx_001_031_laws_v1 import IDS,SPECS
...
def seals():
 for tool,status in (("verify_engine_seal.py","VALID_CANONICAL_ENGINE"),("verify_verification_authority_seal.py","VALID_CANONICAL_VERIFICATION_AUTHORITY")):
  ...
  if result.returncode or data.get("status")!=status:raise SystemExit("Classical Computation ALGX admission halted: protected seal invalid")
def load(claim_id):
 path=ROOT/"claims"/claim_id/"execution.py";module_spec=importlib.util.spec_from_file_location("algx_"+claim_id.replace("-","_"),path);module=importlib.util.module_from_spec(module_spec);module_spec.loader.exec_module(module);return module.build_execution(ROOT)
def main():
