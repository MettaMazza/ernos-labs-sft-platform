#!/usr/bin/env python3
"""Scaffold all pre-source-sealed foundational Biology claim packages."""
...
from sft.biology.generated_law import BIOLOGY_SPECS, PRE_SOURCE_SEAL_PATH, experiment_registration_record, prediction_program_document, validate_pre_source_seal  # noqa: E402
...
EXACT_COUNT_CLAIMS = {
    "SFT-BIO-NUCLEOTIDE-ALPHABET-001": "four generated DNA coding symbols",
    "SFT-BIO-CODON-001": "three ordered positions and sixty-four codon words",
    "SFT-BIO-CODON-BOX-001": "sixteen first-two-position boxes with four codons in each box",
}
...
def claim_registration(spec) -> dict[str, object]:
    return {
        ...
        "intended_certificate": "Complete content-specific 256-form census, unique survivor, depth-independent certificate, independent reconstruction and post-seal purpose-matched biological comparison.",
        "empirical_protocol": f"experiments/biology/{spec.experiment_id}/registration.json",
...
def experiment_registration(spec) -> dict[str, object]:
    ...
    return {
        ...
        "complete_branch_pre_source_seal": {"path": PRE_SOURCE_SEAL_PATH, "hash": validate_pre_source_seal(ROOT), "all_75_predictions_sealed_before_source_selection": True},
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("source-fragment-extraction-and-exact-biology-label-equality", spec.experiment_id)),
            "metrics": [{"metric_id": "source-derived-structural-correspondence", "definition": "Require every registered fragment in every byte-sealed source and compare its reconstructed categorical record with the sealed consequence.", "unit_protocol": f"Exact structural result: {exact}." if exact else "Conditional categorical biological relation; specimen- or population-dependent magnitudes remain bounded records.", "all_rows": True}],
        ...
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "A form missing the biological carrier is rejected."},
...
def build_execution(root: Path) -> ClaimExecution:
    spec = next(item for item in BIOLOGY_SPECS if item.claim_id == {spec.claim_id!r})
    source_files = (
...
def note(spec) -> str:
    ...
    exact = EXACT_COUNT_CLAIMS.get(spec.claim_id)
    magnitude = f"This claim seals and externally checks **{exact}**." if exact else "This claim closes a conditional biological relation; it does not turn an organism-, specimen-, population- or method-dependent magnitude into a universal constant."
    return f"""# {spec.title}
...
def main() -> None:
    for spec in BIOLOGY_SPECS:
        ...
        experiment = ROOT / "experiments/biology" / spec.experiment_id
