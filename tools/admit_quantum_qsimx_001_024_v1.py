#!/usr/bin/env python3
"""Sequential untouched-engine admission for the complete QSIMX family."""
...
def seals():
    ...
    for tool, status in expected:
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Quantum QSIMX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("qsimx_" + claim_id.replace("-", "_"), path)
...
The newest admitted claim is `{claim_id}`, receipt `{receipt_hash}`. QSIMX is
...
{"Admit the next QSIMX claim in frozen dependency order." if index < 24 else "Reconcile the completed QSIMX family into reconciliation v9; do not resubmit it."}
...
def main():
    ...
    from sft.quantum_computation.qsimx_001_024_laws_v1 import IDS, SPECS
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "quantum_computation_obligation": f"SFT-QUANTUM-OBL-QSIMX-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/quantum_qsimx_001_024_target_registry_v1.json",
                "empirical_protocol": "post-registry exhaustive quantum simulation/verification execution and explicit owning-domain handoff custody",
