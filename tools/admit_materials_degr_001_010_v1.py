#!/usr/bin/env python3
"""Admit all ten DEGR claims sequentially through the untouched engine."""
...
MANIFEST_ID = "sha256:d81d754e89db4f32d56e97972b4b7adef211a0c81d38498f0dd51140935e495a"
...
def seals():
    for tool, expected in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or payload.get("status") != expected:
            raise SystemExit("Materials DEGR admission halted: protected seal invalid")


def execution(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("materials_degr_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    if present:
        raise SystemExit("Materials DEGR admission refuses replay: " + ", ".join(present))
    for index, claim_id in enumerate(ORDER, 1):
        ...
        if missing:
            raise SystemExit(f"Materials DEGR admission halted: missing dependencies {claim_id}: {missing}")
        ...
        comparison_count = next(item["comparison_count"] for item in json.loads((ROOT / "experiments/external_sources/materials/degr_001_010_v1/complete_evidence_vector_v1.json").read_text())["claims"] if item["claim_id"] == claim_id)
