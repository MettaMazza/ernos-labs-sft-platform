#!/usr/bin/env python3
"""Exact read-only replay of the eight admitted Materials CRYS receipts."""
...
from sft.materials.crys_001_008_laws_v1 import ORDER
...
def main() -> None:
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("CRYS replay halted: census and execution manifest differ")
    index_by_id = {row["claim_id"]: index for index, row in enumerate(census)}
    first = min(index_by_id[claim_id] for claim_id in ORDER)
    ...
    for claim_id in ORDER:
        ...
        exact = receipt == stored and receipt.receipt_hash == row["receipt_hash"]
        if not exact:
            raise SystemExit("CRYS exact replay mismatch: " + claim_id)
