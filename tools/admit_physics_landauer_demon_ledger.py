#!/usr/bin/env python3
"""Submit the erasure/demon ledger to the frozen engine."""
...
from sft.physics.landauer_demon_ledger_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_landauer_demon", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load erasure/demon execution")
...
def main() -> None:
    ...
    registration["status"] = "independently_replicated_formal_postseal_measurement_pending"
    ...
    (package / "STATUS.md").write_text("\n".join((f"# {CLAIM_ID}", "", "Status: `independently_replicated_formal_postseal_measurement_pending`", "", f"- Closure: `{certificate['closure_scope']}`", "- Empirical status: formal Fold erasure/demon ledger closed; dimensional Landauer comparison remains post-seal", f"- Derivation seal: `{certificate['derivation_seal_hash']}`", f"- Independent validation: `{certificate['external_validation_hash']}`", f"- Engine receipt: `{receipt.receipt_hash}`", f"- Receipt path: `{row['receipt_path']}`", "")), encoding="utf-8")
