#!/usr/bin/env python3
"""Freeze Materials HAND paired empirical records after registry freeze."""
import hashlib
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
REGISTRY = ROOT / "census/materials_hand_001_006_dependency_registry_v1.json"
OUTPUT = ROOT / "experiments/external_sources/materials/hand_001_006_complete_handoff_vector_v1.json"
...
def main():
    if OUTPUT.exists():
        raise SystemExit("Materials HAND v1 vector already frozen")
    registry = read(REGISTRY)
    body = dict(registry)
    identity = body.pop("registry_identity")
    if canonical(body) != identity or registry["target_content_present"]:
        raise SystemExit("Materials HAND v1 registry changed")
    ...
    payload = {
        "schema": "sft-v3-materials-hand-001-006-evidence-vector/1",
        "date": "2026-07-29",
