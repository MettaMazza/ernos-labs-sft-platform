#!/usr/bin/env python3
"""Admit the Consciousness return family without changing protected code."""
...
EMPIRICAL = "SFT-CONSC-VALIDATION-NONORDINARY-COMPLETE-FAMILY-002"
...
def seals() -> None:
    for tool, status in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if run.returncode or data.get("status") != status:
            raise SystemExit(run.stdout + run.stderr + "\nConsciousness admission halted")


def load(claim_id: str):
    path = ROOT / "claims" / claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("consc_return_" + claim_id.replace("-", "_"), path)
...
def manifest(claim_id: str) -> None:
    ...
    if claim_id not in {x["claim_id"] for x in data["claims"]}:
        data["claims"].append({"claim_id": claim_id, "execution_file": f"claims/{claim_id}/execution.py"})
...
def materialize(spec, execution, receipt, captured) -> None:
    ...
    payloads = {
        ...
        "registration.json": {
            "$schema": "../../governance/claim.schema.json", "branch": "consciousness_cognitive_science", "claim_id": spec.claim_id,
            "title": spec.title, "statement": spec.exact_result, "dependencies": list(spec.dependencies), "excluded_inputs": list(spec.exclusions),
    ...
    if empirical:
        ...
        from sft.consciousness.nonordinary_external_v1 import external_registration_record
        write_json(ROOT / "experiments/consciousness/SFT-EXP-CONSC-VALIDATION-NONORDINARY-COMPLETE-FAMILY-002/registration.json", {**external_registration_record(), "status": "authoritatively_corresponded"})
...
def admit(order) -> None:
    ...
    from sft.consciousness.nonordinary_laws_v1 import SPECS
    ...
    for index, claim_id in enumerate(order, 1):
        if claim_id in existing:
            raise SystemExit(f"already admitted: {claim_id}")
        spec = SPECS[claim_id]
        ...
        if missing:
            raise SystemExit(f"{claim_id} dependencies absent: {missing}")
...
def main() -> None:
    seals()
    if "--formal-only" in sys.argv:
        admit(FORMAL)
    elif "--empirical-only" in sys.argv:
        admit((EMPIRICAL,))
    else:
        admit(FORMAL + (EMPIRICAL,))
