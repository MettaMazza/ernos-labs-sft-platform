#!/usr/bin/env python3
"""Exact read-only replay of all eight admitted XINT receipts."""
import json,sys
...
from sft.mathematics.xint_001_008_laws_v1 import IDS as XINT
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("XINT replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in XINT);authority=AuthorityLedger()
 ...
 for cid in XINT:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("XINT exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(XINT)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
