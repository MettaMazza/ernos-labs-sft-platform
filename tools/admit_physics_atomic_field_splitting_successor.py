#!/usr/bin/env python3
"""Admit and materialize terminal atomic field-splitting completion."""
...
from sft.physics.atomic_field_splitting_successor_laws_v1 import ATOMIC_FIELD_SPLITTING_SPEC  # noqa: E402
from sft.physics.atomic_field_splitting_successor_validation_v1 import (  # noqa: E402
...
def load_execution():
    path = ROOT / "claims" / ATOMIC_FIELD_SPLITTING_SPEC.claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_atomic_field_splitting_terminal", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load terminal atomic field-splitting execution")
...
def experiment_registration() -> dict[str, object]:
    spec = ATOMIC_FIELD_SPLITTING_EMPIRICAL_SPEC
    ...
    commitment = sha256_identity((
        SOURCE_HASH,
        tuple((row.target_id, row.source_id, row.source_locator) for row in spec.target_rows),
    ))
    return {
        ...
        "development_observations": [
            {"source_id": SOURCE_ID, "role": "development_only", "content_hash": SOURCE_HASH}
        ],
        ...
        "dimension_unit_boundary": {
            ...
            "external_reference_protocol": "NIST/NBS dimensional coefficients and operating values remain external records; formal execution uses only exact normalized positive carriers.",
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity((
                "exact-atomic-field-splitting-comparison", spec.experiment_id, spec.falsification_condition,
            )),
            "metrics": [{
                "metric_id": "complete-Zeeman-Stark-vector",
                "definition": "Compare every registered multiplicity, field order, displayed coefficient, operating cross-check, uncertainty status and scaling control exactly.",
                "unit_protocol": "Dimensional inscriptions remain external source records; response-order and scaling decisions use exact positive fractions.",
                "all_rows": True,
            }],
            "acceptance_condition": "Every formal relation and complete registered NIST/NBS row passes while dimensional coefficients remain external carriers.",
            "falsification_condition": spec.falsification_condition,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "Incomplete angular or degeneracy carriers are rejected."},
            {"control_id": "TAMPERED-SOURCE", "kind": "tampered_source", "expected_rejection": "Any changed aggregate source record is rejected."},
            {"control_id": "TAMPERED-ARTIFACT", "kind": "tampered_artifact", "expected_rejection": "Changed prediction or trace is rejected."},
            {"control_id": "BOUNDARY", "kind": "boundary", "expected_rejection": "Target access or forbidden proof values are rejected."},
            {"control_id": "UNFAVORABLE-MEASUREMENT", "kind": "unfavorable_measurement", "expected_rejection": "Changed multiplicity, field order, value or uncertainty status fails."},
        ],
        "custody_protocol": {
        ...
        "stop_condition": "Halt after every registered relation, displayed value, scaling control and custody row is evaluated once, or immediately on any violation.",
        "source_hashes": {
...
def main() -> None:
    spec = ATOMIC_FIELD_SPLITTING_SPEC
    empirical = ATOMIC_FIELD_SPLITTING_EMPIRICAL_SPEC
    ...
    write_json(package / "registration.json", {
        ...
        "candidate_grammar": {
            ...
            "completeness_certificate": candidates["completeness_certificate_hash"],
        ...
        "intended_certificate": "All 1,024 typed forms, one survivor, independent reconstruction, hostile controls and complete NIST/NBS Zeeman/Stark vector.",
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "- The NIST/NBS dimensional coefficients remain external unit carriers and do not select the formal survivor.",
    ...
    why = [
        f"# {spec.claim_id}: WHY / DERIVATION / CHECK",
        "",
        "## WHY",
        "",
        spec.statement,
        "",
        "Observation informed the explicit law; target-inaccessible enumeration and sealing precede the complete NIST/NBS release.",
        "",
        "## DERIVATION",
        "",
        f"Grammar boundary: {spec.grammar_boundary}",
        "",
        f"The ten-axis grammar contains {candidates['expected_cardinality']:,} forms. Exactly one survives:",
        "",
        f"`{survivor}`",
        "",
    ]
    ...
    why.extend((
        "",
        f"Base: {spec.induction_base}",
        "",
        f"Successor: {spec.induction_step}",
        "",
        f"Exact result: {spec.exact_result}",
        "",
        "## CHECK",
        "",
        "- Independent code reconstructs complete orientation multiplicity and both exact field orders.",
        "- NIST weak-field Zeeman multiplicity and linear response match after sealing.",
        "- NBS hydrogen and NIST cesium retain the distinct linear and quadratic Stark classes.",
        "- The displayed cesium voltage, spacing and field cross-lock exactly without floating arithmetic.",
        "",
        "## EXCLUSIONS",
        "",
    ))
