#!/usr/bin/env python3
"""Exact read-only replay of all six admitted Materials HAND receipts."""
...
from sft.materials.hand_001_006_laws_v1 import IDS as ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("HAND replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("HAND exact replay mismatch: " + claim_id)
