...
PREVIOUS = ROOT / "census/information_science_discipline_current_reconciliation_v3.json"
OUT = ROOT / "census/information_science_discipline_current_reconciliation_v4.json"
AUDIT = ROOT / "audits/INFORMATION_SCIENCE_SOURCE_001_014_COMPLETION_2026-07-29.json"

from sft.information_science.source_001_014_laws_v1 import IDS
...
def main():
    ...
    if canonical(frozen_body) != frozen_identity or canonical(previous_body) != previous_identity or previous["current_closed_count"] != 38:
        raise SystemExit("SOURCE reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        obligation = f"SFT-INFO-OBL-SOURCE-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["information_science_obligation"] != obligation:
            raise SystemExit("SOURCE reconciliation halt: " + claim_id)
        rows.append({
    ...
    families["SOURCE"] = rows
    value = {
        "schema": "sft-v3-information-science-discipline-current-reconciliation/4",
        "date": "2026-07-29",
        ...
        "current_closed_count": 52,
        "current_open_count": 210,
        "current_completion_fraction": "52/262",
        "current_completion_percent": "19.8%",
    ...
    audit = {
        "schema": "sft-v3-information-science-source-completion/1",
        "date": "2026-07-29",
        "family": "SOURCE-001--014",
        ...
        "current_information_science_progress": "52/262",
        "current_information_science_percent": "19.8%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 52, "open": 210, "percent": "19.8%", "identity": value["reconciliation_identity"]}, indent=2))
