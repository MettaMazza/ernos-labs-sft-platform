#!/usr/bin/env python3
"""Exact read-only replay of all twelve admitted CAT receipts."""
import json,sys
...
from sft.mathematics.cat_001_012_laws_v1 import IDS as CAT
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("CAT replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in CAT);authority=AuthorityLedger()
 ...
 for cid in CAT:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("CAT exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(CAT)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
