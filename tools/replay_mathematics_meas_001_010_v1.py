#!/usr/bin/env python3
"""Exact read-only replay of all ten admitted MEAS receipts."""
import json,sys
...
from sft.mathematics.meas_001_010_laws_v1 import IDS as MEAS
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("MEAS replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in MEAS);authority=AuthorityLedger()
 ...
 for cid in MEAS:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("MEAS exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(MEAS)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
