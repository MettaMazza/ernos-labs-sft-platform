#!/usr/bin/env python3
"""Build the deterministic local Medicine foundation archival bundle."""
...
VERSION = "1.0.0"
RELEASE = ROOT / "output/release" / f"medicine-{VERSION}"
PDF_NAME = "00_From-Fold-to-Medicine_Medicine-and-Health-Sciences-Foundation-Paper-001-v1.0.pdf"
ZIP_NAME = "01_Ernos-Labs-SFT-Medicine-Foundation-Evidence-and-Source-v1.0.0.zip"
MD_NAME = "02_From-Fold-to-Medicine_Medicine-and-Health-Sciences-Foundation-Paper-001-v1.0.md"
...
def evidence_paths() -> tuple[Path, ...]:
    explicit = (
        ROOT / "sft/medicine",
        ROOT / "docs/branch_roadmaps/09-medicine.md",
        ROOT / "audits/medicine_v1_v2_atomic_ownership.json",
        ROOT / "audits/medicine_v1_v2_atomic_ownership.md",
        ROOT / "census/medicine_continuation_checkpoint.json",
        ROOT / "census/medicine_prior_obligations.json",
        ...
        ROOT / "tests/test_medicine_foundation.py",
    )
    tools = tuple(sorted((ROOT / "tools").glob("*medicine*")))
    claims = tuple(sorted((ROOT / "claims").glob("SFT-MED-*")))
    receipts = tuple(sorted((ROOT / "receipts/engine/model_admitted").glob("SFT-MED-*.json")))
...
def write_deterministic_zip(target: Path) -> int:
    ...
    evidence_map = {
        ...
        "control_count": 288,
        "prior_atom_count": 5,
        "captured_external_source_count": 11,
        "failed_external_source_transport_count": 2,
    ...
    with ZipFile(target, "w", compression=ZIP_DEFLATED, compresslevel=9) as archive:
        ...
        info = ZipInfo("medicine_foundation_evidence_map.json", FIXED_TIME)
        info.compress_type = ZIP_DEFLATED
        info.external_attr = 0o100644 << 16
...
def main() -> None:
    ...
    (ROOT / "publication/medicine_foundation_release.json").write_text(
