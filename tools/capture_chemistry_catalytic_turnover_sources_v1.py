#!/usr/bin/env python3
"""Capture the complete pre-registered KIN-010 source surface without parsing targets."""
...
FILENAMES = (
    "article.html", "article.pdf", "supplementary-information.pdf", "supplementary-data.zip",
    "supplementary-video.mp4", "source-data-figure-1.zip", "source-data-figure-2.zip",
    "source-data-figure-3.zip", "source-data-figure-4.zip", "source-data-figure-6.zip",
    "zenodo-record-metadata.json", "zenodo-complete-source-data.zip",
)
...
def main() -> None:
    if sha_file(SPEC_PATH) != SPEC_HASH:
        raise ValueError("KIN-010 prefetch specification changed")
    spec = json.loads(SPEC_PATH.read_text())
    requests = tuple(spec.get("complete_source_requests", ()))
    if len(requests) != len(FILENAMES):
        raise ValueError("KIN-010 complete source request census changed")
    ...
    inventory = {
        "schema": "sft-v3-catalytic-turnover-source-inventory/1",
        "article_doi": spec["article_doi"], "source_data_repository_doi": spec["source_data_repository_doi"],
    ...
    print(json.dumps({
        ...
        "complete_archive_count": len(archive_topologies),
