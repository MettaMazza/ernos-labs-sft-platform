#!/usr/bin/env python3
"""Officially admit and materialize Chemistry PROP-006."""
...
from sft.chemistry.molecular_polarizability_batch_v1 import MOLECULAR_POLARIZABILITY_SPEC  # noqa: E402
from sft.chemistry.molecular_polarizability_validation_v1 import _source_rows  # noqa: E402
...
def load_execution():
    path = ROOT / "claims" / MOLECULAR_POLARIZABILITY_SPEC.claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_chemistry_prop_006", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load PROP-006 execution package")
...
def main() -> None:
    ...
    quantitative = tuple({
        ...
        "formula": row["formula"],
        "name": row["name"],
        "molecular_state": row["molecular_state"],
        "conformation": row["conformation"],
        "inscription_angstrom_cubed": row["inscription"],
        "exact_positive_value_angstrom_cubed": exact_pair(row["vault_value"]),
    ...
    payloads = {
        ...
        "certificate.json": {
            "claim_id": spec.claim_id,
            "chemistry_obligation": "SFT-CHEM-OBL-PROP-006",
            ...
            "exact_component_law": "component response = positive induced-dipole distinction / positive electric distinction",
            "exact_isotropic_law": "isotropic response = Junction of three distinct held-axis responses / three",
            "exact_successor_law": "equal field acts scale response and field by one common positive count and preserve the ratio",
            "complete_external_rows": len(source_rows),
            "complete_reference_cohorts": len({str(row["reference"]) for row in source_rows}),
    ...
    print(f"external rows: {len(source_rows)}; reference cohorts: {len({str(row['reference']) for row in source_rows})}")
