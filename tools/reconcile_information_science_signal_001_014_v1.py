...
PREVIOUS = ROOT / "census/information_science_discipline_current_reconciliation_v5.json"
OUT = ROOT / "census/information_science_discipline_current_reconciliation_v6.json"
AUDIT = ROOT / "audits/INFORMATION_SCIENCE_SIGNAL_001_014_COMPLETION_2026-07-29.json"

from sft.information_science.signal_001_014_laws_v1 import IDS
...
def main():
    ...
    if canonical(frozen_body) != frozen_identity or canonical(previous_body) != previous_identity or previous["current_closed_count"] != 68:
        raise SystemExit("SIGNAL reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        obligation = f"SFT-INFO-OBL-SIGNAL-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["information_science_obligation"] != obligation:
            raise SystemExit("SIGNAL reconciliation halt: " + claim_id)
        rows.append({
    ...
    families["SIGNAL"] = rows
    value = {
        "schema": "sft-v3-information-science-discipline-current-reconciliation/6",
        "date": "2026-07-29",
        ...
        "current_closed_count": 82,
        "current_open_count": 180,
        "current_completion_fraction": "82/262",
        "current_completion_percent": "31.3%",
    ...
    audit = {
        "schema": "sft-v3-information-science-signal-completion/1",
        "date": "2026-07-29",
        "family": "SIGNAL-001--014",
        ...
        "current_information_science_progress": "82/262",
        "current_information_science_percent": "31.3%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 82, "open": 180, "percent": "31.3%", "identity": value["reconciliation_identity"]}, indent=2))
