#!/usr/bin/env python3
"""Official one-shot admission for Chemistry ORG-012."""
...
from sft.chemistry.pericyclic_reaction_batch_v1 import PERICYCLIC_REACTION_SPEC  # noqa: E402
from sft.chemistry.pericyclic_reaction_validation_v1 import exact_analysis  # noqa: E402
...
def load_execution():
    path = ROOT / "claims" / PERICYCLIC_REACTION_SPEC.claim_id / "execution.py"
...
def main() -> None:
    ...
    certificate = {
        "claim_id": spec.claim_id,
        "chemistry_obligation": "SFT-CHEM-OBL-ORG-012",
        ...
        **analysis,
        ...
        "all_32_primary_rows_and_all_44_archive_members_preserved": True,
        "all_28_reported_ratios_preserve_both_relative_classes": True,
        "five_second_class_preferences_and_one_equal_row_preserved_as_unfavorable_controls": True,
        "numerical_zero_negative_irrational_imaginary_continuum_fitted_free_random_or_imported_native_parameter_used": False,
        "orbital_rule_reaction_name_energy_yield_ratio_temperature_condition_or_target_used_to_select_law": False,
        "falsification_condition": empirical.falsification_condition,
    }
    artifacts = {
        "candidate_census.json": {"claim_id": spec.claim_id, **asdict(sealed.census)},
        "pericyclic_receipt.json": {
            "claim_id": spec.claim_id,
    ...
    (package / "STATUS.md").write_text(
        f"# {spec.claim_id}\n\n"
        "Status: `model_admitted_forced_structural_law_postseal_observable_consequences_tested_and_independently_replicated`\n\n"
        "- Chemistry obligation: `SFT-CHEM-OBL-ORG-012`\n"
        "- Exact law: one complete retained closed transition cycle forces four face assignments and exactly two global-complement relative classes without observed-product selection.\n"
        "- External surface: two complete IUPAC records and the complete official 44-member PMC8162770 package.\n"
        "- Measured result: all 32 primary rows retained; 28 reported ratios retain both classes; 22 first-class, five second-class and one equal preference; four unresolved rows retained.\n"
        "- Completeness: all 12 article pages, 203 supplementary pages, the CIF, every archive member and all unfavorable or absent rows remain present.\n"
