#!/usr/bin/env python3
"""Read-only exact replay of the twelve admitted Information Science base claims."""
import json
import sys
...
IDS = tuple(spec.claim_id for spec in SPECS)
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("Information Science base replay halted: census and manifest differ")
    ...
    for claim_id in IDS:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("Information Science base exact replay mismatch: " + claim_id)
