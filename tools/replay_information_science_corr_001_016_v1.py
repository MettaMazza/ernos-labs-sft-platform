#!/usr/bin/env python3
"""Exact read-only replay of all sixteen admitted CORR receipts."""
import json
import sys
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("CORR replay halted: census and manifest differ")
    ...
    for claim_id in IDS:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("CORR exact replay mismatch: " + claim_id)
