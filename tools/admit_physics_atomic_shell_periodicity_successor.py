#!/usr/bin/env python3
"""Admit and materialize terminal atomic shell periodicity completion."""
...
from sft.physics.atomic_shell_periodicity_successor_laws_v1 import ATOMIC_SHELL_PERIODICITY_SPEC  # noqa: E402
from sft.physics.atomic_shell_periodicity_successor_validation_v1 import (  # noqa: E402
...
def load_execution():
    path = ROOT / "claims" / ATOMIC_SHELL_PERIODICITY_SPEC.claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_atomic_shell_periodicity_terminal", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load terminal atomic periodicity execution")
...
def experiment_registration() -> dict[str, object]:
    spec = ATOMIC_SHELL_PERIODICITY_EMPIRICAL_SPEC
    ...
    target_package_hash = sha256_identity((SOURCE_HASH, tuple((row.target_id, row.source_id, row.source_locator) for row in spec.target_rows)))
    return {
        ...
        "dimension_unit_boundary": {
            ...
            "external_reference_protocol": "IUPAC count coordinates and NIST eV inscriptions remain source records; only exact positive rational interval comparison occurs post-seal.",
        ...
        "evaluation_protocol": {
            ...
            "comparison_implementation_hash": sha256_identity(("exact-rational-periodicity-comparison", spec.experiment_id, spec.falsification_condition)),
            "metrics": [{
                "metric_id": "complete-periodicity-vector",
                "definition": "Compare every closure, width, configuration, period envelope, reset, local dip, uncertainty and source identity exactly.",
                "unit_protocol": "External eV inscriptions are exact source records and all comparison arithmetic uses positive fractions.",
                "all_rows": True,
            }],
            "acceptance_condition": "All generated coordinates and every registered favorable and adverse NIST relation pass.",
            "falsification_condition": spec.falsification_condition,
        },
        "controls": [
            {"control_id": "FALSE-PREMISE", "kind": "false_premise", "expected_rejection": "Incomplete shell support is rejected."},
            {"control_id": "TAMPERED-SOURCE", "kind": "tampered_source", "expected_rejection": "Any changed source record or IUPAC PDF is rejected."},
            ...
            {"control_id": "UNFAVORABLE-MEASUREMENT", "kind": "unfavorable_measurement", "expected_rejection": "A failed envelope/reset or erased local dip fails the complete comparison."},
        ],
        "custody_protocol": {
        ...
        "stop_condition": "Halt after every registered coordinate, configuration, interval and adverse row is evaluated once, or immediately on any violation.",
        "source_hashes": {
            SOURCE_PATH: SOURCE_HASH,
            aggregate["sources"]["periodic_table"]["snapshot_path"]: aggregate["sources"]["periodic_table"]["snapshot_hash"],
...
def main() -> None:
    spec = ATOMIC_SHELL_PERIODICITY_SPEC
    experiment_path = ROOT / "experiments/physics" / ATOMIC_SHELL_PERIODICITY_EMPIRICAL_SPEC.experiment_id / "registration.json"
    ...
    write_json(package / "registration.json", {
        ...
        "branch": "physics",
        "candidate_grammar": {"boundary": spec.grammar_boundary, "completeness_certificate": candidate_census["completeness_certificate_hash"], "generator": spec.generation_rule},
        "claim_id": spec.claim_id,
        "dependencies": list(spec.dependencies),
        "empirical_protocol": f"experiments/physics/{ATOMIC_SHELL_PERIODICITY_EMPIRICAL_SPEC.experiment_id}/registration.json",
        "excluded_inputs": list(spec.exclusions),
        "intended_certificate": "All 1,024 typed forms, one survivor, independent reconstruction, hostile controls and complete IUPAC/NIST periodicity vector.",
    ...
    (package / "STATUS.md").write_text("\n".join((
        ...
        "- Shell capacity, period width and ionization-envelope claims are distinct; all local dips remain adverse evidence.",
    ...
    why = [
        ...
        "Observation informed the explicit frozen law. The IUPAC/NIST vector is capability-closed; exhaustive target-inaccessible enumeration and sealing precede release.",
        "", "## DERIVATION", "", f"Grammar boundary: {spec.grammar_boundary}", "",
        f"The complete ten-axis grammar contains {candidate_census['expected_cardinality']:,} forms. Exactly one survives:", "", f"`{survivor}`", "",
    ]
    why.extend(f"- `{axis.key}`: `{axis.survivor.name}` — {axis.survivor.reason}" for axis in spec.axes)
    why.extend(("", f"Base: {spec.induction_base}", "", f"Successor: {spec.induction_step}", "", f"Exact result: {spec.exact_result}", "", "## CHECK", "",
        "- Independent code reconstructs the arbitrary-shell identity and cover-order closure walk.",
        "- All seven IUPAC period widths and registered NIST configurations match.",
        "- Six period envelopes rise, six boundary resets fall, and three local dips remain visible.",
        "- All interval decisions use exact fractions and the Rn display-resolution boundary is explicit.",
        "", "## EXCLUSIONS", ""))
