#!/usr/bin/env python3
"""Submit the general composite confining-sector law to the frozen engine."""
...
from sft.physics.composite_confining_sector_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_composite_confining_sector", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load composite-sector execution")
...
def verify_engine() -> None:
    ...
    if payload["status"] != "VALID_CANONICAL_ENGINE":
        raise SystemExit("runtime engine seal changed; admission halted")
...
def claim_registration() -> dict[str, object]:
    return {
        ...
        "intended_certificate": (
            "Complete 2,048-form enumeration, independent exact reconstruction, depth-independent "
            "odd-support inverse, exhaustive orbit partition, antipodal closure, corrected whole pair-count "
            "law and the inherited post-seal eight-carrier discriminator."
        ),
        ...
        "status": "independently_replicated_with_inherited_postseal_eight_carrier_discriminator",
        "title": SPEC.title,
    }
...
def main() -> None:
    ...
    (package / "STATUS.md").write_text("\n".join((
        f"# {CLAIM_ID}", "",
        "Status: `independently_replicated_with_inherited_postseal_eight_carrier_discriminator`", "",
        "- The arbitrary-even-sector denominator, orbit, antipode, pair-count and coupling laws are depth-independent.",
        "- Sectors 8, 12, 18, 24 and 30 are completely enumerated without importing their V1 orbit tables.",
        "- The V1 pair counts five and fourteen are retained with the exact corrected whole formula `(s-2)/2`.",
        "- The admitted PDG force-sector anchor supplies the post-seal eight-carrier correspondence; the remaining forced sectors are explicit standing predictions.",
