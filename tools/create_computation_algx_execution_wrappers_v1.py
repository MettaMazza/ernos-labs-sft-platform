#!/usr/bin/env python3
"""Mechanically create the fixed ALGX claim execution entry points."""
...
def main():
 for claim_id in IDS:
  ...
  if path.exists():raise SystemExit("ALGX execution wrapper already exists: "+claim_id)
  path.parent.mkdir(parents=True,exist_ok=True);path.write_text(TEMPLATE.format(claim_id=claim_id))
 print(f"created {len(IDS)} ALGX execution entry points")
