...
PREVIOUS = ROOT / "census/information_science_discipline_current_reconciliation_v11.json"
OUT = ROOT / "census/information_science_discipline_current_reconciliation_v12.json"
AUDIT = ROOT / "audits/INFORMATION_SCIENCE_COARSE_001_012_COMPLETION_2026-07-29.json"

from sft.information_science.coarse_001_012_laws_v1 import IDS
...
def main():
    ...
    if canonical(frozen_body) != frozen_identity or canonical(previous_body) != previous_identity or previous["current_closed_count"] != 158:
        raise SystemExit("COARSE reconciliation predecessor changed")
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        obligation = f"SFT-INFO-OBL-COARSE-{index:03d}"
        if not row["model_admitted"] or certificate["engine_receipt_hash"] != row["receipt_hash"] or certificate["information_science_obligation"] != obligation:
            raise SystemExit("COARSE reconciliation halt: " + claim_id)
        rows.append({
    ...
    families["COARSE"] = rows
    value = {
        "schema": "sft-v3-information-science-discipline-current-reconciliation/12",
        "date": "2026-07-29",
        ...
        "current_closed_count": 170,
        "current_open_count": 92,
        "current_completion_fraction": "170/262",
        "current_completion_percent": "64.9%",
    ...
    audit = {
        "schema": "sft-v3-information-science-coarse-completion/1",
        "date": "2026-07-29",
        "family": "COARSE-001--012",
        ...
        "current_information_science_progress": "170/262",
        "current_information_science_percent": "64.9%",
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"closed": 170, "open": 92, "percent": "64.9%", "identity": value["reconciliation_identity"]}, indent=2))
