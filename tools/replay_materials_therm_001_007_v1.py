#!/usr/bin/env python3
"""Exact read-only replay of the seven admitted Materials THERM receipts."""
import json
...
from sft.materials.therm_001_007_laws_v1 import ORDER
...
def main():
    ...
    if [row["claim_id"] for row in census] != [row["claim_id"] for row in manifest]:
        raise SystemExit("THERM replay halted: census and execution manifest differ")
    ...
    for claim_id in ORDER:
        ...
        if receipt != stored or receipt.receipt_hash != row["receipt_hash"]:
            raise SystemExit("THERM exact replay mismatch: " + claim_id)
