#!/usr/bin/env python3
"""Admit QCPLXX-001 through QCPLXX-026 sequentially."""
...
from sft.quantum_computation.qcplxx_001_026_laws_v1 import IDS, SPECS
...
def seals():
    for tool, status in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or data.get("status") != status: raise SystemExit("Quantum QCPLXX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("qcplxx_" + claim_id.replace("-", "_"), path)
