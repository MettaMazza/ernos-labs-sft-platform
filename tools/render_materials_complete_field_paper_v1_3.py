#!/usr/bin/env python3
"""Render the exhaustive 289-law Materials manuscript to an archival PDF."""
...
SOURCE = ROOT / "publications/successors/materials/FROM_FOLD_TO_MATERIALS_PAPER_001_V1_3.md"
OUTPUT = ROOT / "output/pdf/from-fold-to-materials-branch-paper-001-v1.3.pdf"
METADATA = ROOT / "publication/materials_zenodo_metadata_v1_3_draft.json"


def cover(authorized: bool, doi: str):
    title = ParagraphStyle("MaterialsCoverTitle", fontName="Helvetica-Bold", fontSize=27, leading=32, textColor=base.ACCENT_DARK, alignment=TA_CENTER)
    subtitle = ParagraphStyle("MaterialsCoverSubtitle", fontName="Helvetica", fontSize=13, leading=18, textColor=base.INK, alignment=TA_CENTER)
    kicker = ParagraphStyle("MaterialsCoverKicker", fontName="Helvetica-Bold", fontSize=9, leading=12, textColor=base.ACCENT, alignment=TA_CENTER)
    author = ParagraphStyle("MaterialsCoverAuthor", fontName="Times-Roman", fontSize=12, leading=18, textColor=base.INK, alignment=TA_CENTER)
    note = ParagraphStyle("MaterialsCoverNote", fontName="Times-Roman", fontSize=9, leading=13, textColor=base.MUTED, alignment=TA_CENTER, leftIndent=18 * mm, rightIndent=18 * mm)
    warning = ParagraphStyle("MaterialsCoverWarning", fontName="Helvetica-Bold", fontSize=9, leading=13, textColor=base.ACCENT_DARK, alignment=TA_CENTER)
    return [
        Spacer(1, 18 * mm),
        Paragraph("SMITHIAN FOLD THEORY - MATERIALS SCIENCE BRANCH PAPER 001", kicker),
        Paragraph("From Fold to Materials", title),
        Spacer(1, 7 * mm),
        Paragraph("An Exact, Parameter-Free and Machine-Closed Reconstruction of Materials Science from Smithian Fold Theory", subtitle),
        Spacer(1, 10 * mm),
...
def main() -> None:
    ...
    document = BaseDocTemplate(
        ...
        topMargin=21 * mm, bottomMargin=18 * mm, title="From Fold to Materials",
        author="Maria Smith", subject="Completed Smithian Fold Theory Materials Science branch",
