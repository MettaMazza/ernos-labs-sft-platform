#!/usr/bin/env python3
"""Admit all eight EXT claims sequentially through the untouched engine."""
...
MANIFEST_ID = "sha256:254d1b470728c78ff3466f11923cd0fe9911722771ae7b9e7be0fd99d9a614e4"
...
def seals():
    for tool, expected in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or payload.get("status") != expected:
            raise SystemExit("Materials EXT admission halted: protected seal invalid")


def execution(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("materials_ext_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    if present:
        raise SystemExit("Materials EXT admission refuses replay: " + ", ".join(present))
    for index, claim_id in enumerate(ORDER, 1):
        ...
        if missing:
            raise SystemExit(f"Materials EXT admission halted: missing dependencies {claim_id}: {missing}")
        ...
        comparison_count = next(item["comparison_count"] for item in json.loads((ROOT / "experiments/external_sources/materials/ext_001_008_v1/complete_evidence_vector_v1.json").read_text())["claims"] if item["claim_id"] == claim_id)
        write_json(package / "certificate.json", {
            ...
            "captured_authoritative_source_count": 8,
