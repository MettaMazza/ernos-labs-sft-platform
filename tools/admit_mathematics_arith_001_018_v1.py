...
from sft.mathematics.arith_001_018_laws_v1 import IDS as ORDER,SPECS
...
def seals():
 for tool,status in (("verify_engine_seal.py","VALID_CANONICAL_ENGINE"),("verify_verification_authority_seal.py","VALID_CANONICAL_VERIFICATION_AUTHORITY")):
  ...
  if result.returncode or data.get("status")!=status:raise SystemExit("Mathematics ARITH admission halted: protected seal invalid")
def load(cid):
 path=ROOT/"claims"/cid/"execution.py";spec=importlib.util.spec_from_file_location("arith_"+cid.replace("-","_"),path);module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module);return module.build_execution(ROOT)
def main():
