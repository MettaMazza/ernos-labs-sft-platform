#!/usr/bin/env python3
"""Admit all twelve COMP claims sequentially through the untouched engine."""
...
MANIFEST_ID = "sha256:4c9e18cd3526f3a66b4170821c1916786d53d33192c2f49b770a0fac7dcc945a"
...
def seals():
    for tool, expected in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or payload.get("status") != expected:
            raise SystemExit("Materials COMP admission halted: protected seal invalid")


def execution(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("materials_comp_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    if present:
        raise SystemExit("Materials COMP admission refuses replay: " + ", ".join(present))
    for index, claim_id in enumerate(ORDER, 1):
        ...
        if missing:
            raise SystemExit(f"Materials COMP admission halted: missing dependencies {claim_id}: {missing}")
        ...
        comparison_count = next(item["comparison_count"] for item in json.loads((ROOT / "experiments/external_sources/materials/comp_001_012_v1/complete_evidence_vector_v1.json").read_text())["claims"] if item["claim_id"] == claim_id)
