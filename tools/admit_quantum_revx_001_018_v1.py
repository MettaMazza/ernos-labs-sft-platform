#!/usr/bin/env python3
"""Admit the complete REVX-001 through REVX-018 family sequentially."""
...
from sft.quantum_computation.revx_001_018_laws_v1 import IDS, SPECS
...
def seals():
    for tool, status in (
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Quantum REVX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("revx_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "quantum_computation_obligation": f"SFT-QUANTUM-OBL-REVX-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/quantum_revx_001_018_target_registry_v1.json",
                "empirical_protocol": "post-registry exact reversible-computation execution custody",
