#!/usr/bin/env python3
"""Sequential untouched-engine admission for the complete HAND family."""
...
def seals():
    ...
    for tool, status in expected:
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Quantum HAND admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("hand_" + claim_id.replace("-", "_"), path)
...
The newest admitted claim is `{claim_id}`, receipt `{receipt_hash}`. HAND is
**{index}/6** admitted. Every preceding Quantum family through VALID remains
complete and must not be repeated.

## Exact next operation

{"Admit the next HAND claim in frozen dependency order." if index < 6 else "Reconcile the completed HAND family into reconciliation v13; do not resubmit it."}
...
def main():
    ...
    from sft.quantum_computation.hand_001_006_laws_v1 import IDS, SPECS
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "quantum_computation_obligation": f"SFT-QUANTUM-OBL-HAND-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/quantum_hand_001_006_target_registry_v1.json",
                "empirical_protocol": "post-registry complete single-owner cross-branch handoff and open-extension custody",
