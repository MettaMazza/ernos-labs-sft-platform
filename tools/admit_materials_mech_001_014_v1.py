#!/usr/bin/env python3
"""Admit all fourteen MECH claims sequentially through the untouched engine."""
...
REGISTRY_ID = "sha256:a35bbb4297261a17b7d543c83935f944aceb2a038717c89c6c973cb29a70d476"
MANIFEST_ID = "sha256:aa53d03b01a7593112a1a4a0575e0acc2e4a7d72e94dbf098124a4467f69d9c5"
VECTOR_ID = "sha256:796ae288a892f41a0415a8cf133089eca567368ed2ea48e6b5ff31e5f83e7c72"
...
def seals():
    for tool, expected in (("verify_engine_seal.py", "VALID_CANONICAL_ENGINE"), ("verify_verification_authority_seal.py", "VALID_CANONICAL_VERIFICATION_AUTHORITY")):
        ...
        if result.returncode or payload.get("status") != expected:
            raise SystemExit("Materials MECH admission halted: protected seal invalid")


def execution(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("materials_mech_" + claim_id.replace("-", "_"), path)
...
def main():
    ...
    if present:
        raise SystemExit("Materials MECH admission refuses replay: " + ", ".join(present))
    for index, claim_id in enumerate(ORDER, 1):
        ...
        if missing:
            raise SystemExit(f"Materials MECH admission halted: missing dependencies {claim_id}: {missing}")
        ...
        write_json(package / "certificate.json", {"claim_id": claim_id, "materials_obligation": spec.obligation_id, "status": "model_admitted_authoritatively_corresponded_and_independently_replicated", "source_manifest_hash": run.program.registration.source_hash, "derivation_seal_hash": sealed.seal_hash, "independent_implementation_hash": independent.implementation_hash, "independent_certificate_hash": independent.certificate_hash, "independently_recomputed": True, "external_validation_hash": receipt.external_validation_hash, "empirical_validation_hash": receipt.empirical_validation_hash, "measurement_receipt_hash": empirical.measurement_receipt_hash, "external_data_source_ids": list(empirical.data_source_ids), "all_external_rows_preserved": empirical.all_rows_preserved, "falsification_condition": empirical.falsification_condition, "engine_receipt_hash": receipt.receipt_hash, "engine_receipt_path": row["receipt_path"], "exact_result": spec.exact_result, "closure_scope": receipt.closure_status, "controls_passed": True, "candidate_count": len(sealed.census.candidates), "unique_survivor_count": sum(decision.survives for decision in sealed.decisions), "free_parameters": [], "imported_axioms": [], "target_registry_identity": REGISTRY_ID, "source_custody_manifest_identity": MANIFEST_ID, "complete_evidence_vector_identity": VECTOR_ID, "captured_authoritative_source_count": 8, "external_comparison_count": next(item["comparison_count"] for item in json.loads((ROOT / "experiments/external_sources/materials/mech_001_014_v1/complete_evidence_vector_v1.json").read_text())["claims"] if item["claim_id"] == claim_id)})
