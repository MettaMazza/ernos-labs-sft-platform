#!/usr/bin/env python3
"""Official one-shot admission for Chemistry ORG-008."""
...
from sft.chemistry.electrophilic_substitution_batch_v1 import (  # noqa: E402
    ELECTROPHILIC_SUBSTITUTION_SPEC,
    PRIMARY_PATH,
)
from sft.chemistry.electrophilic_substitution_validation_v1 import (  # noqa: E402
    _source_rows,
    exact_analysis,
)
...
def load_execution():
    path = ROOT / "claims" / ELECTROPHILIC_SUBSTITUTION_SPEC.claim_id / "execution.py"
    definition = importlib.util.spec_from_file_location("org008_execution", path)
...
def main() -> None:
    spec = ELECTROPHILIC_SUBSTITUTION_SPEC
    ...
    claim_row = next(
        row
        for row in json.loads(claims_path.read_text())["claims"]
        if row["claim_id"] == spec.claim_id
    )
    package = ROOT / "claims" / spec.claim_id
    primary = json.loads((ROOT / PRIMARY_PATH).read_text())
    analysis, checks = exact_analysis(ROOT, _source_rows(ROOT), primary)
    certificate = {
        "claim_id": spec.claim_id,
        "chemistry_obligation": "SFT-CHEM-OBL-ORG-008",
        ...
        **analysis,
        ...
        "conventional_mechanism_rate_energy_solvent_substrate_or_external_target_used_to_generate_or_select_law": False,
    ...
    registration["candidate_grammar"]["completeness_certificate"] = (
        sealed.census.completeness_certificate_hash
    )
    ...
    (package / "STATUS.md").write_text(
        ...
        "- Exact law: the entering EmptyOne acceptor receives the complete aromatic donor pair; the leaving pair restores the opened aromatic incidence.\n"
        "- Complete path family: direct exchange or addition followed by recurrence restoration; cleavage-first halts.\n"
        "- External surface: four complete sources including every page of the 452-page supplementary PDF.\n"
        "- Complete optimization surface: all 25 Table S1 rows, including 14 displayed zero-yield rows, nine displayed full-yield rows, and the 12 and 93 rows.\n"
        "- Radical, phenoxonium, recovered-starting-material, inconsistent-condition, unpromoted and inseparable-mixture records are retained.\n"
