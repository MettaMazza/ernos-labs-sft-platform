#!/usr/bin/env python3
"""Execute and materialize the first molecular-electronic Chemistry claim."""
...
def main() -> None:
    ...
    spec = ELECTRONIC_STRUCTURE_BATCH_1_SPECS[0]
    existing = {
        row["claim_id"]: row
    ...
    payloads = {
        ...
        "certificate.json": {
            ...
            "external_evidence_class": "categorical_observational_derivation_postseal_correspondence",
    ...
    (package / "STATUS.md").write_text(
        ...
        f"- Post-seal external correspondence: `{receipt.empirical_validation_hash}`\n"
        f"- External measurement receipt: `{empirical.measurement_receipt_hash}`\n"
        f"- Engine receipt: `{receipt.receipt_hash}`\n"
        "- Numerical measurement claimed: `false`; this is the structural prerequisite for later quantitative molecular-state laws.\n",
        encoding="utf-8",
    )
