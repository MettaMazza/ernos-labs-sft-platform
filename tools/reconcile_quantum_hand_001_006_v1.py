#!/usr/bin/env python3
"""Reconcile the completed HAND family against the frozen Quantum census."""
...
PREVIOUS = ROOT / "census/quantum_computation_discipline_current_reconciliation_v12.json"
OUT = ROOT / "census/quantum_computation_discipline_current_reconciliation_v13.json"
AUDIT = ROOT / "audits/QUANTUM_COMPUTATION_HAND_001_006_COMPLETION_2026-07-29.json"
...
def receipt_identity(path):
    ...
    if recorded != computed:
        raise SystemExit("HAND stored receipt identity mismatch: " + path.name)
    return computed


def main():
    if OUT.exists() or AUDIT.exists():
        raise SystemExit("HAND reconciliation already exists")
    ...
    if canonical(previous_body) != previous_identity or previous["current_closed_count"] != 282:
        raise SystemExit("Quantum Computation predecessor reconciliation changed")
    obligations = [row for row in frozen["obligations"] if row["family"] == "HAND"]
    if len(obligations) != 6:
        raise SystemExit("HAND obligation count changed")
    live = {row["claim_id"]: row for row in json.loads((ROOT / "census/claims.json").read_text())["claims"]}
    registry = json.loads((ROOT / "census/quantum_hand_001_006_target_registry_v1.json").read_text())
    rows = []
    ...
    completed["HAND"] = rows
    value = {"schema": "sft-v3-quantum-computation-discipline-current-reconciliation/13", "date": "2026-07-29", "frozen_census_identity": frozen_identity, "frozen_obligation_count": 288, "closed_at_freeze": 22, "predecessor_reconciliation_identity": previous_identity, "completed_families": completed, "current_closed_count": 288, "current_open_count": 0, "current_completion_fraction": "288/288", "current_completion_percent": "100.0%", "frozen_census_mutated": False, "extension_policy": frozen["extension_policy"]}
    value["reconciliation_identity"] = canonical(value)
    OUT.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
    audit = {"schema": "sft-v3-quantum-computation-hand-completion/1", "date": "2026-07-29", "family": "HAND", "family_completion": "6/6", "generated_candidate_count": 6 * 256, "unique_survivor_count": 6, "passed_control_count": 6 * 4, "post_registry_exact_observation_count": 6, "implementation_distinct_reconstruction_count": 6, "exact_receipt_replay": "6/6", "receipt_rows": rows, "protected_engine_or_verifier_changed": False, "current_quantum_computation_progress": "288/288", "current_quantum_computation_percent": "100.0%", "frozen_census_identity": frozen_identity, "reconciliation_identity": value["reconciliation_identity"]}
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"family": "6/6", "candidates": 6 * 256, "controls": 6 * 4, "current": "288/288", "percent": "100.0%", "identity": value["reconciliation_identity"]}, indent=2))
