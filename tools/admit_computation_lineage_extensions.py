#!/usr/bin/env python3
"""Execute and admit only the three Classical Computation lineage extensions."""
...
def load_execution(claim_id: str):
    ...
    module_spec = importlib.util.spec_from_file_location("sft_comp_lineage_" + claim_id.replace("-", "_"), path)
    if module_spec is None or module_spec.loader is None: raise RuntimeError(f"cannot load {path}")
...
def main() -> None:
    ...
    existing_rows = {row["claim_id"]: row for row in json.loads((ROOT / "census/claims.json").read_text(encoding="utf-8"))["claims"]}
    for spec in LINEAGE_SPECS:
        if spec.claim_id in existing_rows and existing_rows[spec.claim_id].get("model_admitted"):
            print("retained", spec.claim_id, existing_rows[spec.claim_id]["receipt_hash"]); continue
        ...
        print("admitted", spec.claim_id, receipt.receipt_hash)

    manifest_path = ROOT / "census/execution_manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8")); existing = {item["claim_id"] for item in manifest["claims"]}
    for spec in LINEAGE_SPECS:
        if spec.claim_id not in existing: manifest["claims"].append({"claim_id": spec.claim_id, "execution_file": f"claims/{spec.claim_id}/execution.py"})
