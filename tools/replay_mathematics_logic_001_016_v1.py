#!/usr/bin/env python3
"""Exact read-only replay of all sixteen admitted LOGIC receipts."""
import json,sys
...
from sft.mathematics.logic_001_016_laws_v1 import IDS as LOGIC
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("LOGIC replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in LOGIC);authority=AuthorityLedger()
 ...
 for cid in LOGIC:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("LOGIC exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(LOGIC)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
