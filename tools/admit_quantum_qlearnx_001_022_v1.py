#!/usr/bin/env python3
"""Sequential untouched-engine admission for the complete QLEARNX family."""
...
def seals():
    ...
    for tool, status in expected:
        ...
        if result.returncode or data.get("status") != status:
            raise SystemExit("Quantum QLEARNX admission halted: protected seal invalid")


def load(claim_id):
    path = ROOT / "claims" / claim_id / "execution.py"
    module_spec = importlib.util.spec_from_file_location("qlearnx_" + claim_id.replace("-", "_"), path)
...
The newest admitted claim is `{claim_id}`, receipt `{receipt_hash}`. QLEARNX is
**{index}/24** admitted. The preceding BASE, REVX, QSTATEX, GATEX, QALGX,
QCPLXX, QCOMMX and QCODEX families remain complete and must not be repeated.

## Exact next operation

{"Admit the next QLEARNX claim in frozen dependency order." if index < 22 else "Reconcile the completed QLEARNX family into reconciliation v10; do not resubmit it."}
...
def main():
    ...
    from sft.quantum_computation.qlearnx_001_022_laws_v1 import IDS, SPECS
    ...
    for index, claim_id in enumerate(IDS, 1):
        ...
        certificate = {
            "claim_id": claim_id,
            "quantum_computation_obligation": f"SFT-QUANTUM-OBL-QLEARNX-{index:03d}",
        ...
        payload = {
            ...
            "registration.json": {
                ...
                "pre_source_target_registry": "census/quantum_qlearnx_001_022_target_registry_v1.json",
                "empirical_protocol": "post-registry exhaustive quantum-learning execution, held-out target custody and explicit owning-domain handoff",
