#!/usr/bin/env python3
"""Submit the Fold lattice-operator family to the frozen engine."""
...
from sft.physics.lattice_operator_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def load_execution():
    ...
    definition = importlib.util.spec_from_file_location("sft_lattice_operator_terminal", path)
    if definition is None or definition.loader is None:
        raise RuntimeError("cannot load lattice-operator execution")
...
def main() -> None:
    ...
    (package / "STATUS.md").write_text("\n".join((f"# {CLAIM_ID}", "", "Status: `independently_replicated_with_inherited_postseal_comparison`", "", "- The exact stencil, point-source, causal-cone and phase-mode family is independently reconstructed.", "- Existing admitted dispersion, Maxwell and cubic-support claims supply the inherited post-seal physical comparison boundary.", "- No irrational conventional mode value is evaluated or imported as a proof scalar.", f"- Closure: `{certificate['closure_scope']}`", f"- Derivation seal: `{certificate['derivation_seal_hash']}`", f"- Independent validation: `{certificate['external_validation_hash']}`", f"- Engine receipt: `{receipt.receipt_hash}`", f"- Receipt path: `{row['receipt_path']}`", "")), encoding="utf-8")
