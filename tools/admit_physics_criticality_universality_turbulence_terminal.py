...
from sft.physics.criticality_universality_turbulence_terminal_law_v1 import CLAIM_ID, SPEC  # noqa: E402
...
def verify_engine():
    ...
    if result.returncode or json.loads(result.stdout).get("status") != "VALID_CANONICAL_ENGINE":
        raise SystemExit(result.stdout + result.stderr + "\ncriticality admission halted")


def execution():
    path = ROOT / "claims" / CLAIM_ID / "execution.py"
    definition = importlib.util.spec_from_file_location("sft_criticality_universality_turbulence", path)
...
def registration():
    return {
        ...
        "intended_certificate": "Complete 256-form law census; exact perfect-power critical and cascade witnesses; scaling identities; generated class census; independent reconstruction and hostile controls.",
    ...
    (package / "STATUS.md").write_text(
        "\n".join(
            (
                ...
                "Status: `independently_replicated_formal_empirical_comparison_separate`",
                "",
                "- Binary critical carriers are beta=1/2, nu=1/2, gamma=1 and delta=3; alpha and eta are typed empty records.",
                "- Widom, Rushbrooke and Fisher close exactly.",
                "- Generator-three cascade carriers are 2/3 and falling 5/3.",
                "- The declared grammar contains two distinct generated universality classes.",
