#!/usr/bin/env python3
"""Exact read-only replay of all six admitted HAND receipts."""
import json,sys
...
from sft.mathematics.hand_001_006_laws_v1 import IDS as HAND
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("HAND replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in HAND);authority=AuthorityLedger()
 ...
 for cid in HAND:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("HAND exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(HAND)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
