#!/usr/bin/env python3
"""Exact read-only replay of all twelve admitted DYN receipts."""
import json,sys
...
from sft.mathematics.dyn_001_012_laws_v1 import IDS as DYN
...
def main():
 ...
 if [x["claim_id"] for x in census]!=[x["claim_id"] for x in manifest]:raise SystemExit("DYN replay halted: census and manifest differ")
 indices={x["claim_id"]:i for i,x in enumerate(census)};first=min(indices[x] for x in DYN);authority=AuthorityLedger()
 ...
 for cid in DYN:
  ...
  if receipt!=stored or receipt.receipt_hash!=row["receipt_hash"]:raise SystemExit("DYN exact replay mismatch: "+cid)
  authority.admit(receipt);results.append(cid);print(f"[{len(results)}/{len(DYN)}] exact replay {cid}: {receipt.receipt_hash}",flush=True)
