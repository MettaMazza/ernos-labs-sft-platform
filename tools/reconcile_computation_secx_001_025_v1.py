#!/usr/bin/env python3
"""Reconcile the complete admitted SECX family against the frozen census."""
import hashlib
import json
...
PREVIOUS = ROOT / "census/computation_discipline_current_reconciliation_v7.json"
OUT = ROOT / "census/computation_discipline_current_reconciliation_v8.json"
AUDIT = ROOT / "audits/COMPUTATION_SECX_001_025_COMPLETION_2026-07-29.json"
...
def receipt_identity(path):
    ...
    if recorded != computed:
        raise SystemExit("SECX stored receipt identity mismatch: " + path.name)
    return computed


def main():
    if OUT.exists() or AUDIT.exists():
        raise SystemExit("SECX reconciliation already exists")
    ...
    if canonical(body) != previous_identity or previous["current_closed_count"] != 275:
        raise SystemExit("Classical Computation predecessor reconciliation changed")
    obligations = [row for row in frozen["obligations"] if row["family"] == "SECX"]
    if len(obligations) != 25:
        raise SystemExit("SECX obligation count changed")
    live = {row["claim_id"]: row for row in json.loads((ROOT / "census/claims.json").read_text())["claims"]}
    registry = json.loads((ROOT / "census/computation_secx_001_025_target_registry_v1.json").read_text())
    rows = []
    for claim_id, obligation in zip(registry["claim_ids"], obligations):
        ...
        if row is None or not row.get("model_admitted") or receipt_identity(ROOT / row["receipt_path"]) != row["receipt_hash"] or certificate.get("engine_receipt_hash") != row["receipt_hash"] or certificate.get("classical_computation_obligation") != obligation["obligation_id"] or certificate.get("candidate_count") != 256 or certificate.get("unique_survivor_count") != 1 or not certificate.get("controls_passed") or len(census["candidates"]) != 256 or len(controls) != 4 or not all(control["passed"] for control in controls) or not empirical.get("all_rows_preserved") or not empirical.get("passed"):
            raise SystemExit("SECX reconciliation halt: " + claim_id)
        rows.append({
    ...
    completed["SECX"] = rows
    value = {
        "schema": "sft-v3-classical-computation-discipline-current-reconciliation/8",
        "date": "2026-07-29",
        ...
        "current_closed_count": 300,
        "current_open_count": 69,
        "current_completion_fraction": "300/369",
        "current_completion_percent": "81.3%",
    ...
    audit = {
        "schema": "sft-v3-classical-computation-secx-completion/1",
        "date": "2026-07-29",
        "family": "SECX",
        ...
        "current_classical_computation_progress": "300/369",
        "current_classical_computation_percent": "81.3%",
        "frozen_census_identity": frozen_identity,
        "reconciliation_identity": value["reconciliation_identity"],
    }
    AUDIT.write_text(json.dumps(audit, indent=2, sort_keys=True) + "\n")
    print(json.dumps({"family": "25/25", "candidates": 25 * 256, "controls": 25 * 4, "current": "300/369", "percent": "81.3%", "identity": value["reconciliation_identity"]}, indent=2))
