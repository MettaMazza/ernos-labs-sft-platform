#!/usr/bin/env python3
"""Build and verify the Foundation branch publication evidence bundle.
...
PDF_PATH = ROOT / "output/pdf/from-nothing-to-fold-foundation-branch-paper-001.pdf"
EVIDENCE_MAP_PATH = ROOT / "publications/current/foundation/evidence_map.json"
MANIFEST_PATH = ROOT / "publications/current/foundation/manifest.json"
PUBLICATION_RECEIPT_PATH = ROOT / "publications/current/foundation/publication_receipt.json"

PAPER_SECTIONS = {
    "SFT-ROOT-THERE-IS-NO-NOTHING": "5",
    "SFT-FOUNDATION-ONE-001": "6",
    "SFT-FOUNDATION-COUNT-001": "7",
    "SFT-FOUNDATION-PART-001": "8",
    "SFT-FOUNDATION-FOLD-001": "9",
    "SFT-FOUNDATION-PART-EQUIVALENCE-001": "10",
    "SFT-FOUNDATION-FOLD-ASSEMBLY-001": "11",
    "SFT-FOUNDATION-FORM-GRAMMAR-001": "12",
    "SFT-FOUNDATION-FORM-ENFORCEMENT-001": "13",
    "SFT-FOUNDATION-MEASURED-VALUE-BOUNDARY-001": "14",
}
...
def load_inventory() -> BranchInventory:
    ...
    if calculated != payload["inventory_hash"]:
        raise ValueError("foundation inventory identity does not match its content")
...
def build_evidence_map() -> dict[str, Any]:
    ...
    if tuple(census) != inventory.required_claim_ids:
        raise ValueError("foundation census order does not equal the frozen inventory")
    ...
    return {
        "schema": "sft-v3-foundation-paper-evidence-map/1",
        "branch_id": BRANCH_ID,
        "inventory": {
...
def verify_written_bundle() -> dict[str, Any]:
    ...
    if read_json(PUBLICATION_RECEIPT_PATH) != expected_receipt:
        raise ValueError("foundation publication receipt is stale or has been altered")
    return {
...
def main() -> int:
    result = build_bundle()
    print("SFT FOUNDATION PUBLICATION GATE: PASS")
