"""Focused ANAL-006--008 tests; the heavy repository suite remains deferred."""
...
from sft.chemistry.nmr_family_batch_v1 import SPECS
...
@pytest.mark.parametrize("claim", SPECS, ids=("anal006", "anal007", "anal008"))
...
def test_complete_external_reconstruction(claim):
    ...
    assert analysis["complete_family_source_count"] == 10
    assert analysis["complete_family_pdf_pages"] == 24
    assert analysis["complete_family_html_documents"] == 5
    assert analysis["complete_family_source_bytes"] == 1527681
    assert analysis["complete_family_measured_row_count"] == 1633
    assert analysis["implementation_distinct_raw_vector_reconstruction_passed"]
