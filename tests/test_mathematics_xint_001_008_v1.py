...
from sft.mathematics.xint_001_008_external_v1 import load
from sft.mathematics.xint_001_008_laws_v1 import IDS,OBS,SPECS
from sft.mathematics.generated_law import candidate_records,survivor_id
ROOT=Path(__file__).resolve().parents[1]
def test_complete_family_products():
 assert len(IDS)==len(SPECS)==8
...
def test_observations():assert len(OBS)==8 and all(v for _,v in OBS.values())
def test_registry_vector():
 r,v=load(ROOT);assert r["target_content_present"] is False;assert len(r["claim_ids"])==v["record_count"]==8;assert v["outcomes_opened_only_after_registry_freeze"]
