"""Focused ANAL-009--011 tests; the heavy repository suite remains deferred."""
...
from sft.chemistry.photoluminescence_family_batch_v1 import SPECS
...
@pytest.mark.parametrize("claim", SPECS, ids=("anal009", "anal010", "anal011"))
...
def test_complete_external_reconstruction(claim):
    ...
    assert analysis["complete_family_source_count"] == 16
    assert analysis["complete_family_pdf_pages"] == 73
    assert analysis["complete_family_html_documents"] == 6
    assert analysis["complete_family_xml_documents"] == 2
    assert analysis["complete_family_workbooks"] == 2
    assert analysis["complete_family_source_bytes"] == 3184576
