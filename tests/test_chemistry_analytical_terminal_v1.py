"""Focused whole-subfield ANAL-012--022 tests; heavy verification stays deferred."""
...
from sft.chemistry.analytical_terminal_batch_v1 import SPECS
from sft.chemistry.analytical_terminal_validation_v1 import exact_analysis


ROOT = Path(__file__).resolve().parents[1]
IDS = tuple(f"anal{number:03d}" for number in range(12, 23))
...
def test_complete_external_reconstruction(claim):
    ...
    assert analysis["complete_family_source_count"] == 218
    assert analysis["complete_family_pdf_pages"] == 173
    assert analysis["complete_family_html_documents"] == 7
    assert analysis["complete_family_source_bytes"] == 22221914
