...
from sft.mathematics.hand_001_006_external_v1 import load
from sft.mathematics.hand_001_006_laws_v1 import IDS,OBS,SPECS
...
def test_observations():assert len(OBS)==6 and all(v for _,v in OBS.values())
def test_registry_vector():
 r,v=load(ROOT);assert r["target_content_present"] is False;assert len(r["claim_ids"])==v["record_count"]==6;assert v["outcomes_opened_only_after_registry_freeze"]
